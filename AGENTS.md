...
- **Closed vocabularies (RDR status, and future ones) are CHECKED TABLES, not prose.** The RDR lifecycle domain lives at `src/nexus/tables/rdr-lifecycle.toml` (packaged, loaded via `load_packaged_table`; byte-identical plugin copy at `conexus/resources/tables/rdr-lifecycle.toml`), never hand-copied into a doc. `docs/tables/` holds repo-only tables (e.g. the future release-choreography table) whose consumers are `scripts/`, never shipped. `nx rdr set-status` and `nx rdr preamble rdr-audit` enforce against the table directly; a lint test asserts every prose copy of the vocabulary (e.g. `docs/rdr/AGENTS.md`) matches the table's domain exactly.
