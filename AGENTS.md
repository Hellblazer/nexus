...
- **⛔ `develop` is UNRELEASABLE until the service-stack install + user-migration story ships (since RDR-155 P4a, 2026-06-10).** Post-P4a, T3 serving routes exclusively through the PG16+pgvector+Java nexus-service stack; a PyPI release cut from develop would require every user to hand-assemble PG16, the pgvector extension, a 134MB service JAR with no distribution channel, `nx init --service`, and Voyage key plumbing. Do NOT cut a release (and do NOT start RDR-155 P4b — the final Chroma deletion, which also deletes the migration tool itself) until release-blocker bead `nexus-luxe6` closes: (1) epic `nexus-pebfx` install collapse, (2) `nx upgrade` migration orchestration (conexus RDR-001), (3) conexus xr7.8.9 go-live, (4) the two-release deprecation window — migration-capable release first, Chroma deletion only in the release after. Authoritative record: T2 `nexus/release-boundary-since-p4a`. Engine-side production migration is complete (`nexus_rdr/155-production-migration-complete`); Chroma sources are untouched rollback targets.
