...
- **Single-document hook chain** (RDR-070). Register with `register_post_store_hook(fn)`; fired by `fire_post_store_hooks(doc_id, collection, content)` at `src/nexus/mcp/core.py:887` inside the MCP `store_put` tool. For ad-hoc / agent-triggered enrichment that lands one document at a time. Current consumer: `taxonomy_assign_hook` (incremental topic assignment).
...
**Drift guard**: `tests/test_hook_drift_guard.py` uses `ast.walk` to assert that no source file outside `src/nexus/mcp_infra.py` (definitions) and `src/nexus/mcp/core.py` (the single registration site) imports or references `taxonomy_assign_batch_hook` or `chash_dual_write_batch_hook` directly. New per-document batch consumers register through the chain. Direct calls fail CI.
