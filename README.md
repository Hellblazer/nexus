...
**If `nx self install` says this nx is not running from a generation**, this box is still on the older uv-tool layout — upgrade it with `uv tool upgrade conexus` instead, which retains the spec you installed with. (That refusal also suggests `scripts/reinstall-tool.sh`. Ignore it unless you have a repo checkout: the message is written for a developer standing in one, and the same refusal fires for a uv-tool install where that script does not apply — nexus-op2r1.) `nx doctor`'s *Generation layout* row tells you which one you have. On either layout, **do not** upgrade with `uv tool install conexus --force` / `uv tool install conexus`: that *resets* the install and **drops `[local]`**, silently downgrading your embedder from 768-dim to 384-dim, which dimension-mismatches existing 768-dim collections and makes search return nothing. On a uv-tool box, recover with `uv tool install --reinstall "conexus[local]"`. On a generation box, that same command is worse than useless — it rebuilds the uv tree and takes the shims back, so `nx` resolves through uv again instead of through `current`; `nx doctor` flags it and `nx self install` repairs it.
