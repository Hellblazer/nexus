...
The plugin provides 16 specialized agents (including analytical operators for structured extraction, summarization, ranking, and comparison), 33 skills covering the RDR lifecycle, analytical query pipelines, and development workflows, session hooks for automatic context initialization, and 25 MCP tools split across two focused servers — `nexus` (15 core tools: search, store, memory, scratch, plans, consolidation) and `nexus-catalog` (10 catalog tools: search, show, link, resolve, stats, etc.) — with 6 admin operations exposed via CLI only. Agents search indexed code before proposing changes, check prior RDR decisions before designing new features, and coordinate through standard pipelines (plan → implement → review → test) with built-in quality gates.
