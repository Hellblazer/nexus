...
- `conexus/hooks/scripts/mailbox_drain.py` (nexus-galkv.6, RDR-208 Phase 2 Step 3): after a session's own mailbox, the drain also reads `<config>/tuple-watch/cleared.<session id>` and empties every mailbox it names, deleting the record only once every named mailbox's claim loop ended empty, a fresh probe shows no live row, and its pending file is empty; a record older than 7 days is pruned. The address's pending-file read-modify-write is now lock-protected, closing a race two processes draining the same address could hit.
