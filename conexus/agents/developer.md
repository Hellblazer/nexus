...
- **Project-scoped decisions / findings** (T2, future sessions this project) → `mcp__plugin_conexus_nexus__memory_put(content=..., project="<repo>", title=..., agent="developer")`. Omit `ttl` — a decision or finding is permanent unless there is a reason it should expire. The `agent` kwarg attributes this write to the developer role so `nx tier-status` slices by agent (nexus-9clx).
...
Call `nx_plan_audit` MCP tool if ANY of:
