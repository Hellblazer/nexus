...
3. **Declaration-completeness (Gap 1)**: `source tests/e2e/lib/expectations.sh; expectations_undeclared <session_id>` — names every START with no EXPECT row of its type. Pairing is by `agent_type` == the EXPECT row's subagent_type with N-of-type credit (nexus-qc4p1 writes the row from the PreToolUse dispatch hook; since CC 2.1.251 the payload carries no dispatch name at all — nexus-houpu), so 2 STARTs of a type with 1 EXPECT row leaves exactly one UNDECLARED, and a START the hook never saw (a Workflow-tool agent, an agent a hook spawned) is UNDECLARED unless it was hand-declared at dispatch time. Any UNDECLARED → bead. **Reads its EXIT CODE, not just stdout**: 0 clean; 1 BLINDSPOT — EXPECT rows present but zero STARTs walked, the audit examined nothing (nexus-mk3tw, re-aimed); 2 undeclared>0 deficit; 3 no ledger file. Census: `expectations_census <session_id>` — the SCRIPTED per-agent classification + ROWS/CLASSIFIED summary (nexus-hybv1: never hand-count; `BLOCKED_RESOLVED` = guard success, bare `BLOCKED_UNRESOLVED` = real idle); it is the report, shares only the rc=1 rule, never exits 2. Paste its two summary lines into the handoff.
