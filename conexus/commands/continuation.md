...
3. **Declaration-completeness (Gap 1)**: `nx-hook expectations_undeclared <session_id>` — names every START with no EXPECT row of its type. Pairing is by `agent_type` == the EXPECT row's subagent_type with N-of-type credit (nexus-qc4p1 writes the row from the PreToolUse dispatch hook; since CC 2.1.251 the payload carries no dispatch name at all — nexus-houpu), so 2 STARTs of a type with 1 EXPECT row leaves exactly one UNDECLARED, and a START the hook never saw (an agent a hook spawned) is UNDECLARED unless it was hand-declared at dispatch time. A Workflow-tool agent (`agent_type` exactly `workflow-subagent`) is the one named exception, since nexus-silj0 (Sam's ruling, 2026-09-23): it gets its own bucket instead, reported on a `WORKFLOW\tchecked=<n>` line and excluded from the `undeclared` deficit, because the Workflow tool's own fan-out count isn't knowable at PreToolUse time and no EXPECT row can be written for it in advance. Any OTHER UNDECLARED → bead. **Reads its EXIT CODE, not just stdout**: 0 clean; 1 BLINDSPOT — EXPECT rows present but zero STARTs walked, the audit examined nothing (nexus-mk3tw, re-aimed); 2 undeclared>0 deficit; 3 no ledger file. Census: `nx-hook expectations_census <session_id>` — the SCRIPTED per-agent classification + ROWS/CLASSIFIED summary (nexus-hybv1: never hand-count; `BLOCKED_RESOLVED` = guard success, bare `BLOCKED_UNRESOLVED` = real idle); it is the report, shares only the rc=1 rule, never exits 2; Workflow-tool STARTs are bucketed there too. Paste its two summary lines into the handoff.
4. **Payload-shape tripwire**: after any Claude Code version change, rerun `./tests/cc-validation/runner.sh --scenario 27` — it pins the SubagentStart payload the ledger keys on (opaque `a<hex>` `agent_id`, `agent_type` == `subagent_type`, no dispatch name anywhere) and that a named background dispatch is recognised by type.
