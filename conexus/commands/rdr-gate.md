...
- Run all three gate layers in sequence:
  - **Layer 1 (Structural)**: Use the Section Structure and Section Summaries above to check completeness (required headings present, no empty sections); when the repo ships `docs/writing-style.md`, the preamble also runs `nx prose lint` on the RDR and prints a BLOCKED line on any finding (override: `--skip-prose`, recorded in the audit trail), which stops the gate the same way an incomplete section does. **If no research findings exist** and `--skip-research` was NOT passed, report **BLOCKED** and stop; do not proceed to Layer 2 or 3. If `--skip-research` was passed, note the override and continue.
