...
RDR-205 Phase 5 consumer: `mailbox/<address>` tuple-space messaging.
...
- Address: the harness id `subagent-start.sh` injects (its "Claimant id: `<id>`" line). The mailbox address is `mailbox/<id>`. The instance-name form of the address is Phase 6.
- Send: `tuple_out` to the recipient's address, with a SENDER-MINTED message id as the nonce. `mcp__plugin_conexus_nexus__tuple_out(subspace="mailbox/<address>", keys={"to": "<address>"}, dims={"from": "<your id>"}, body="...", nonce="<sender-minted nonce>")`.
