...
A send to a peer's NAME lands in that peer's own session mailbox: `mailbox_send` resolves the name to a session id, so the peer sees it pushed over the Claude Code channel when launched with a channel flag, or at its next prompt through the drain hook otherwise, never sooner. The name resolves only while the peer holds it, and a peer holds it by calling `tuple_subscribe("mailbox/<name>")` once (RDR-208 Phase 3, bead nexus-galkv.20: that call arms the name in the session directory and nothing more; the `mailbox/<name>` subspace itself is never delivered). When a mailbox request goes to a peer that may not have subscribed or reached the channel, also send a one-line `SendMessage` pointing at it, or relay through your human. `mailbox_send` resolves the peer's NAME against the live session directory at send time: a resume that changed the peer's name, or a peer whose watcher has stopped, is a REFUSAL naming the failure: never a silent send to a name nobody reads any more. `ListAgents` is still how you learn a peer's name in the first place, or its current one after a refusal; you no longer need to re-check it immediately before every send purely to dodge staleness. Two live sessions holding the same name is also a refusal, naming both session ids; `nx tuple directory NAME` shows who holds a name and its session id(s): resend to the one you mean.
...
- A mailbox request: an `ack` consumes the request and carries its only reply, so never ack to say "received". Claim it with `tuple_in`, unless the drain hook already rendered its body with the channel notification, in which case act on it and claim nothing (RDR-213: the waiter itself never claims). Finish now: renew at half the lease while working, then `tuple_ack` with a `reply` carrying the result. Defer answering it (still your claim, just not the final answer yet): `tuple_ack` with a `reply` saying it is tracked and when to expect the answer, then send the result later as a new mailbox message with `kind="reply"` and the same `correlation_id`. Not ready to even look at it yet, and it was only a re-announced reference: `tuple_release` it, never `tuple_nack`: a nack counts as a failed attempt and three dead-letter the message, while release counts nothing and leaves it available for the next announcement.
...
- Use the locks the project documents (its AGENTS.md or CLAUDE.md names them) and nothing else. Local file-based capacity locks (the build lease, etc.) stay out of scope here — those are per-machine and per-project, never a tuple. The tuple space's own cross-session mutual-exclusion primitive is the `lock/<resource>` template (RDR-211): `tuple_in` to take it, `tuple_renew` while holding it, `tuple_release` to hand it back clean (`tuple_ack` is refused on a lock claim, naming `release`, since an acked lock row would stay unobtainable until the sweep purges it) — see `/conexus:mailbox`'s "Queue and lock claims" rule. Never invent a NEW tuple-space consumer for a lock or anything else: a use this template doesn't cover needs its own RDR, exactly as RDR-211 needed one to add it.
