...
A peer sees NAME-addressed mail pushed over the Claude Code channel once it has subscribed that name (`tuple_subscribe("mailbox/<name>")`), or at its next prompt once that subscribe has registered the name for the drain hook. A peer that never subscribed, or was never launched with the development-channel flag, still gets it at the next prompt through the drain hook, never sooner. When a mailbox request goes to a peer that may not have subscribed or reached the channel, also send a one-line `SendMessage` pointing at it, or relay through your human. `mailbox_send` resolves the peer's NAME against the live session directory at send time: a resume that changed the peer's name, or a peer whose watcher has stopped, is a REFUSAL naming the failure: never a silent send to a name nobody reads any more. `ListAgents` is still how you learn a peer's name in the first place, or its current one after a refusal; you no longer need to re-check it immediately before every send purely to dodge staleness. Two live sessions holding the same name is also a refusal, naming both session ids; `nx tuple directory NAME` shows who holds a name and its session id(s): resend to the one you mean.
