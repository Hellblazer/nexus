...
- User invokes `/conexus:rdr-accept`
...
7. **Planning handoff** (default: yes) — Auto-detect complexity:
   ...
   - **If yes — execute the full chain (3 sequential dispatches + catalog enrichment, orchestrated by this skill):**
     ...
     5. **Catalog links** (if catalog initialized): Search for related RDRs and create `relates` links:
        - `mcp__plugin_conexus_nexus-catalog__search(query="<rdr-title-keywords>", content_type="rdr")`
        - For each result that is NOT this RDR: `mcp__plugin_conexus_nexus-catalog__link(from_tumbler="<this-rdr-title>", to_tumbler="<related-rdr-tumbler>", link_type="relates", created_by="rdr-accept")`
...
2. `mcp__plugin_conexus_nexus__nx_plan_audit` — validates plan against codebase (RDR-080: no agent spawn)
3. `mcp__plugin_conexus_nexus__nx_enrich_beads` — enriches beads with execution context (RDR-080: no agent spawn)
...
- [ ] mcp__plugin_conexus_nexus__nx_plan_audit called and result verified (if user accepts)
- [ ] mcp__plugin_conexus_nexus__nx_enrich_beads called and beads enriched (if user accepts)
...
- **Agent dispatch + MCP calls**: strategic-planner (Agent tool) → `mcp__plugin_conexus_nexus__nx_plan_audit` → `mcp__plugin_conexus_nexus__nx_enrich_beads` (optional, user-confirmed, orchestrated by this skill — RDR-080)
