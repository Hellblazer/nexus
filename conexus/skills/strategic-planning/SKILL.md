...
- **T3 knowledge**: Validated architectural decisions via store_put tool: content="# Decision: {topic}\n{rationale}", collection="<subject>", title="decision-planner-{topic}", tags="decision,planning"
- **T2 memory**: Continuation state via memory_put tool: content="content", project="{project}", title="continuation-state.md", ttl=14 — a genuine expiring row (a resume snapshot, not a decision of record); `ttl` is an integer number of days here, not a string
