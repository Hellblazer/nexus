...
| **Verification** | `config.py` (verification section), `hooks/stop_verification.py`, `hooks/pre_close_verification.py`, `conexus/hooks/scripts/read_verification_config.py` | Opt-in mechanical enforcement: Stop hook (session-end checks), PreToolUse hook (bd-close gate), standalone config reader. Both hooks are in the wheel since [RDR-215](rdr/rdr-215-plugin-hooks-as-nx-verbs.md), served as the `hook_stop_verification` and `hook_pre_close_verification` MCP tools; the config reader stays plugin-resident. See [Verification config](configuration.md#verification) |
