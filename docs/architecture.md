...
A Linda tuple space over Postgres, engine-side — `out`/`rd`/`rdp`/`in`/`inp`/`ack`/`nack`/`registry`/`subspace_list`/`subspace_stats` under `/v1/tuples`. A Python client (`db.tuples` / `HttpTupleStore`) makes this a ninth store alongside the eight in the table above, but its row there, and the `nx tuple` CLI / `tuple_*` MCP tools built on it, are Phase 2 client documentation owned separately (nexus-0miq7.2). This subsection covers the engine internals only: the schema, the claim SQL, park/wake, and the sweep. See [Tuple space](tuple-space.md) for the full operational reference.
