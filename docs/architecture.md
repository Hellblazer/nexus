...
CLI (cli.py)            MCP Server (mcp_server.py)
  ...
  └──────────┬──────────────┘
    ...
    │           (code spans under 64 chars merge into a neighbour, never dropped: chunk_floor.py, nexus-x50jb; prose is not floored)
...
**The MCP server's own registry is a `LockedHookRegistry` too, since 7.55.3 (nexus-dgvsz).** It was a plain `HookRegistry` for as long as that was safe by construction: FastMCP calls a sync `@mcp.tool()` body directly from its async dispatch, so every sync tool body — `store_put` included — held the one event-loop thread for its whole duration and two fires could not interleave. That is no longer true. Sync tool bodies now run through `asyncio.to_thread` (`mcp/_sdk_patches.py`, so short-timeout hook calls are not starved behind a multi-second tool call and answered late into a torn-down stdio transport), which makes concurrent fires of the same hook reachable from the MCP path exactly as they already were from the bulk indexer.
...
`taxonomy_assign_batch_hook` reads only the batch's chunk chashes: assignment is computed and persisted server-side by `POST /v1/taxonomy/assignments/assign_from_chashes` (nexus-yu9w5), from the `chunks` rows the same flush just upserted and the `taxonomy_centroids` rows, so `contents`, `embeddings` and `metadatas` are accepted for the shared call shape and not read. One hook body covers both the bulk path and the single-document path; there is no separate single-doc taxonomy hook to keep in sync. Since 7.49.0 (nexus-r0vkh) the hook serializes against concurrent fires of itself under `LockedHookRegistry` (it had opted out in 2026-08 on the claim that per-flush chash sets share no rows, which is true of `topic_assignments` and false of `nexus.topics`, where the FK's KEY SHARE and the doc_count recount trigger put every concurrent fire on the same rows); the engine bounds its side of the call with a transaction-local `statement_timeout` and `lock_timeout` (engine-service-v0.1.125, `NX_TAXONOMY_ASSIGN_*_TIMEOUT_MS`) and retries once after a lock timeout. A bounded call that fails is recorded on the taxonomy tripwire and never fails the index write.
...
`src/nexus/db/t2/` is a Python package split into nine domain-specific
...
| Tuple space       | `HttpTupleStore`             | `db.tuples`             | Linda tuple space (`out`/`rd`/`in`/`ack`/`nack`/`renew`/`release`/`registry`/`subspace_list`/`subspace_stats`) over `/v1/tuples` ([RDR-205](rdr/rdr-205-linda-tuple-space-over-postgres.md), [RDR-206](rdr/rdr-206-tuple-claim-renew-and-reply-in-ack.md), [RDR-211](rdr/rdr-211-board-queue-and-lock-tuple-templates.md)). See [Tuple space](#tuple-space-rdr-205) below and [Tuple Space](tuple-space.md) for the full reference |

`T2Database` is a composing facade: it constructs the nine stores in
order (memory → plans → taxonomy → telemetry → chash_index →
document_aspects → aspect_queue → document_highlights → tuples), re-exposes the memory-domain public
methods as thin delegates for backward compatibility, and runs
cross-domain operations like `expire()` over all of them. The
chash_index, taxonomy, document_aspects, aspect_queue, and tuples
domains are accessed directly via their attributes -- no facade
delegates exist for them. The facade holds no connection of its own;
every operation runs through a specific domain store's HTTP client.
...
### Tuple space ([RDR-205](rdr/rdr-205-linda-tuple-space-over-postgres.md), [RDR-206](rdr/rdr-206-tuple-claim-renew-and-reply-in-ack.md), [RDR-211](rdr/rdr-211-board-queue-and-lock-tuple-templates.md))

A Linda tuple space over Postgres — `out`/`rd`/`rdp`/`in`/`inp`/`ack`/`nack`/`renew`/`release`/`wait`/`registry`/`subspace_list`/`subspace_stats`/`park_stats` under `/v1/tuples` (`renew` and `ack`'s optional `reply` are RDR-206; `release`, `wait`, and `park_stats` are RDR-211). This subsection covers the engine internals: the schema, the claim SQL, park/wake, and the sweep. RDR-211 also adds three generic coordination templates alongside the ledger, mailbox, and directory templates -- a broadcast board, a shared work queue, and a mutual-exclusion lock (six templates total) -- and a per-template `lock: true` flag (set on the `lock/<resource>` template only) that moves the tuple's own expiry to now-plus-retention on claim and renew, ahead of the lease clamp, and refuses `ack` on that claim (naming `release` instead), since a consumed lock row would otherwise be unobtainable until the sweep purges it; see [T2 Domain Stores § Engine: tuple space](#tuple-space-rdr-205) above and [Tuple space](tuple-space.md) for the full template reference. The Python client is `db.tuples` / `HttpTupleStore` (the ninth row in the table above); its `nx tuple` CLI and `tuple_*` MCP tools are documented in [CLI Reference](cli-reference.md#nx-tuple) and [MCP Servers](mcp-servers.md#tuple-space-t2-adjacent-rdr-205). See [Tuple space](tuple-space.md) for the full operational reference covering both halves.

**Three tables** (`tuples-001-baseline.xml`, `tuples-002-sweep-indexes.xml`), two RLS-scoped, one not:

- `nexus.tuples` — the space itself. `ENABLE`+`FORCE ROW LEVEL SECURITY` with a `tenant_isolation` policy (`tenant_id = current_setting('nexus.tenant', true)`, both `USING` and `WITH CHECK`), the same shape every other tenant-scoped table in this schema uses. `autovacuum_vacuum_scale_factor = 0.01` (the first per-table storage parameter in the changelog — claim/ack/nack/expire churn warrants reclaiming well before the 20% default).
- `nexus.tuple_claim_log` — append-only `claim`/`ack`/`nack`/`expire`/`dead` audit trail, one row per transition. Same RLS shape and storage parameter as `nexus.tuples`. `tuple_id` is a nullable FK to `nexus.tuples(id)` `ON DELETE SET NULL` — a purged tuple's log rows survive it, denormalizing `subspace`/`template` onto each row so it reads standalone after that.
- `nexus.tuple_tenants` — sweep-cursor bookkeeping, one row per tenant that has ever called `out`, upserted at write time. **No RLS**, same posture as `service_tokens`/`install_pings`: it names tenants and holds no tenant-owned data, and the sweep that reads it must see every tenant's row in one enumeration, not one tenant's RLS-scoped slice.

**The claim statement.** `TupleRepository.claimOnce` runs, per attempt, one `SELECT ... FOR NO KEY UPDATE SKIP LOCKED LIMIT 1` against the row-life availability predicate (`consumed_at IS NULL AND expires_at > now() AND claim_state IS DISTINCT FROM 'dead' AND (claim_state IS NULL OR lease_until < now())`), then an `UPDATE` setting `claim_state`/`claimant`/`claim_id`/`lease_until`, then an `INSERT` into the claim log — one tenant-scoped transaction. `FOR NO KEY UPDATE` rather than a plain `FOR UPDATE`: the update touches no key column, and the weaker lock lets the claim log's FK coexist without a lock upgrade inside the transaction (mirrors `AspectRepository.claimNext`'s shape, not its `claimBatch`, whose repeated-single-claim loop is a different shape from this bounded dead-letter re-run). A candidate whose own lease has lapsed is released and re-selected in the same pass, up to `NX_TUPLE_CLAIM_PASSES` (default 8) passes before the call gives up and returns the probe result.

**The waiter.** `TupleWaitRegistry` holds one `ReentrantLock`+`Condition` per live `(tenant, subspace)`, in a concurrent map. `out` signals the group for its subspace only after its own transaction commits — never from inside it, and never any other subspace's waiters. A blocking `rd`/`in` registers before its first query (so a write landing between that query and the first park is never lost), then loops: query, release the connection, park until signalled or a one-second timer, repeat. Each group carries a monotonic generation bumped on every `signalAll`; a waiter compares its own last-observed generation on entry to close the lost-wakeup window between registration and the first park. Per-claimant and engine-wide park caps (`ParkCapExceededException`) are checked only at the moment of actually parking, never against a call whose own first non-blocking probe already found a match.

**The sweep, beside the token loop.** A second task on the same `sweepScheduler` that runs the T1 crash-safety token sweep (`NexusService.runScheduledSweep`), at the same `SWEEP_INTERVAL_HOURS` (six), sharing that scheduler's single thread. It enumerates `nexus.tuple_tenants` directly (a plain `DataSource`-backed `DSLContext`, not through `TenantScope` — the table carries no RLS and the enumeration runs before any tenant is chosen), least-recently-swept first, and runs three arms per tenant against a shared per-tenant batch cap split into per-arm shares — release lapsed claims, purge expired/consumed-past-retention tuples, purge old claim-log rows — so a large backlog in one arm can no longer starve the other two within the same visit. A tenant is stamped `last_swept_at = now()` only when every arm finishes cleanly; a cut-short tenant keeps its old stamp and sorts first next run. See [Tuple space § The sweep](tuple-space.md#the-sweep) for the exact env vars, defaults, and the incomplete-cause classification.
...
| **Verification** | `config.py` (verification section), `hooks/stop_verification.py`, `hooks/pre_close_verification.py`, `hooks/verification_config.py` | Opt-in mechanical enforcement: Stop hook (session-end checks), PreToolUse hook (bd-close gate), config reader. All three are in the wheel; the reader moved there at nexus-b5ugt, because an `mcp_tool` hook runs inside `nx-mcp` and gets no usable `CLAUDE_PLUGIN_ROOT`, so a plugin-resident reader read `{}` and the Stop gate verified nothing. The Stop hook is served as the `hook_stop_verification` MCP tool; the **close gate is wired on the COMMAND tier** (`nx-hook pre-close-verification`, nexus-17i1n) because an `mcp_tool` hook cannot return a verdict at all — see `nexus.mcp.hooks.DECIDING_HOOKS`. See [Verification config](configuration.md#verification) |
| **MCP Servers** | `mcp/core.py`, `mcp/catalog.py`, `mcp/connect_marker.py`, `mcp_infra.py`, `mcp_server.py` (shim) | Multi-server FastMCP architecture ([RDR-062](rdr/rdr-062-mcp-interface-tiering.md), [RDR-139](rdr/rdr-139-devonthink-mcp-semantic-linking-sync.md)). `nexus` core server (52 tools: storage, retrieval, operators, orchestration) + `nexus-catalog` (10 tools: catalog and link graph). (The RDR-139 Layer A' `nx-mcp-devonthink` proxy was retired 2026-07-07, nexus-goypg: clients connect to DEVONthink's own MCP server directly; its `dt_incorporate` composite lives on as `nx dt incorporate`.) Short-name convention: catalog tools drop the redundant `catalog_` prefix since the server namespace already provides context. Six destructive / maintenance operations are intentionally kept CLI-only. Backward-compat shim at `mcp_server.py` re-exports every function. `query()` has catalog-aware routing (author, content_type, subtree, follow_links, depth); singletons and test injection live in `mcp_infra.py`. **For the full tool catalog see [MCP Servers](mcp-servers.md).** |
...
| **Support** | `config.py`, `registry.py`, `corpus.py`, `session.py`, `hooks/`, `ttl.py`, `formatters.py`, `types.py`, `errors.py`, `retry.py`, `bounded_subprocess.py`, `commands/_helpers.py` | Configuration, naming, formatting, session lifecycle, transient-error retry. `bounded_subprocess.py`: the timeout-bounded `subprocess.run` every call site goes through, enforced by a lint ratchet (nexus-t10nc). `_helpers.py`: shared CLI helpers (e.g. `default_db_path()`). (`_provision.py` — ChromaDB Cloud database provisioning — was DELETED at RDR-155 P4b P2; it had zero src callers.) |
| **Engine: tuple space** | `service/src/main/java/dev/nexus/service/db/TupleRepository.java`, `.../db/TupleWaitRegistry.java`, `.../db/TupleException.java` + its twelve subtypes, `.../http/TupleHandler.java`, `.../tuples/` (`TemplateRegistry.java`, `TemplateSchema.java`, `TemplateSchemaParser.java`, `MiniYaml.java`, `TemplateRegistryException.java`) | Java, engine-side ([RDR-205](rdr/rdr-205-linda-tuple-space-over-postgres.md), [RDR-206](rdr/rdr-206-tuple-claim-renew-and-reply-in-ack.md) for `renew` and `ack`'s reply). `TupleRepository` owns the fourteen operations (`out`, `rd`, `rdp`, `in`, `inp`, `ack`, `nack`, `renew`, `release`, `wait`, `registry`, `subspace_list`, `subspace_stats`, `park_stats`) and the claim/sweep SQL; `TupleWaitRegistry` is the per-subspace park/wake mechanism; `TupleHandler` is the `/v1/tuples` HTTP surface, one route per operation; the `tuples/` package loads, validates, and digests the six templates (`ledger/<session_id>`, `mailbox/<address>`, `directory/<name>`, and RDR-211's `board/<topic>`, `queue/<name>`, `lock/<resource>`) from classpath YAML at boot. See [T2 Domain Stores § Tuple space](#tuple-space-rdr-205) above and [Tuple space](tuple-space.md) for the full reference |
| **Client: tuple space** | `db/t2/http_tuple_store.py` (`HttpTupleStore`, `db.tuples`), `commands/tuple_cmd.py` (`nx tuple`), `mcp/core.py` (thirteen `tuple_*` tools), `health.py` (six `tuples.*` doctor rows) | Python client ([RDR-205](rdr/rdr-205-linda-tuple-space-over-postgres.md) Phase 2, [RDR-206](rdr/rdr-206-tuple-claim-renew-and-reply-in-ack.md) Phase 2 for `renew` and `ack(reply=)`, [RDR-211](rdr/rdr-211-board-queue-and-lock-tuple-templates.md) for `release`, subscriptions, and the doctor rows). The 8 KB pre-send guard and the twelve-typed-error mapping live in `http_tuple_store.py`; `nx tuple` and the MCP tools are thin callers with no HTTP of their own. See [Tuple space § Client surface](tuple-space.md#client-surface) |
| **Client: mailbox push delivery** | `mcp/subscriptions.py` (`SubscriptionSet`: per-session mailbox/board subscriptions, T1-persisted, plus the RDR-208 `directory/<name>` lease for a leased name -- NOT a second delivered mailbox, and, since RDR-208 Phase 3 (bead nexus-galkv.20), no per-session registration file at all), `mcp/channel.py` (the lifespan waiter: parks `wait` over every subscription from lifespan start, no gate, and pushes a reference; each mailbox stays in the spec every tick with an `announce={interval_s, max}` field, and the engine returns only claimable rows that are due, stamping `announced_at`/`announce_count` in the same statement, so the client keeps no mailbox cursor; a board spec carries `announce.subscriber` (this session's id, `max=1`) and the engine keeps the per-subscriber stamp in `nexus.tuple_deliveries` (bead nexus-q82tk), so the client keeps no cursor for any shape), `mailbox_arm.py` (the SessionStart subscribe instruction and its bounded, cached tuple-surface probe), `session_marker.py` (the `tuple-watch/session.<claude_pid>` marker `hooks/`'s `session_start` writes on every source), `hooks/mailbox_drain.py` (the `UserPromptSubmit` drain hook, `nx-hook mailbox-drain`, unconditional floor) | Python client (RDR-211 nexus-rplay.14, superseding [epic nexus-6konb](tuple-space.md#push-delivery-rdr-211-the-channel); RDR-213: the waiter needs no proof and holds no claim). The session's own nexus MCP server parks a `wait` over its subscriptions and pushes a reference through the Claude Code channel; the notification also wakes the `UserPromptSubmit` drain hook, which usually claims, acks and renders the body in that same prompt before the session's turn; a session without the plugin's hooks claims the reference itself with `tuple_in`. The drain hook claims, acks and renders at each prompt regardless, for the session-id mailbox only -- a leased name has no delivery floor at all (RDR-208 Phase 3). See [Tuple space § Push delivery](tuple-space.md#push-delivery-rdr-211-the-channel) |
