...
### Tuple space ([RDR-205](rdr/rdr-205-linda-tuple-space-over-postgres.md))

A Linda tuple space over Postgres — `out`/`rd`/`rdp`/`in`/`inp`/`ack`/`nack`/`registry`/`subspace_list`/`subspace_stats` under `/v1/tuples`. This subsection covers the engine internals: the schema, the claim SQL, park/wake, and the sweep. The Python client is `db.tuples` / `HttpTupleStore` (the ninth row in the table above); its `nx tuple` CLI and `tuple_*` MCP tools are documented in [CLI Reference](cli-reference.md#nx-tuple) and [MCP Servers](mcp-servers.md#tuple-space-t2-adjacent-rdr-205). See [Tuple space](tuple-space.md) for the full operational reference covering both halves.
