...
| Tuple space       | `HttpTupleStore`             | `db.tuples`             | Linda tuple space (`out`/`rd`/`in`/`ack`/`nack`/`registry`/`subspace_list`/`subspace_stats`) over `/v1/tuples` ([RDR-205](rdr/rdr-205-linda-tuple-space-over-postgres.md)). See [Tuple space](#tuple-space-rdr-205) below and [Tuple Space](tuple-space.md) for the full reference |
...
### Tuple space ([RDR-205](rdr/rdr-205-linda-tuple-space-over-postgres.md), [RDR-206](rdr/rdr-206-tuple-claim-renew-and-reply-in-ack.md))

A Linda tuple space over Postgres — `out`/`rd`/`rdp`/`in`/`inp`/`ack`/`nack`/`renew`/`registry`/`subspace_list`/`subspace_stats` under `/v1/tuples` (`renew` and `ack`'s optional `reply` are RDR-206). This subsection covers the engine internals: the schema, the claim SQL, park/wake, and the sweep. The Python client is `db.tuples` / `HttpTupleStore` (the ninth row in the table above); its `nx tuple` CLI and `tuple_*` MCP tools are documented in [CLI Reference](cli-reference.md#nx-tuple) and [MCP Servers](mcp-servers.md#tuple-space-t2-adjacent-rdr-205). See [Tuple space](tuple-space.md) for the full operational reference covering both halves.
