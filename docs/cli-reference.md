...
| `--check-assignments` | Cross-collection ("projection") topic-assignment audit (nexus-v4pj4): since engine-service-v0.1.132 the projection pass of `assign_from_chashes_<dim>` picks each chunk's nearest FOREIGN-collection centroid via an HNSW `LATERAL` (approximate) instead of an exact join — measured equal to exact under production insertion order, but a wrong pick would be silent. Samples chunks per collection that carry a stored `assigned_by='projection'` row, re-fetches the stored vector and every live foreign centroid, and recomputes the exact nearest centroid with the same tie-break the engine's own `ORDER BY distance, topic_id` uses. `--assignments-sample N` (default 20, max 300), `--assignments-collection NAME` (repeatable; default: every collection with a cross-collection projection assignment), `--assignments-seed` (default: today's UTC date). **Exit 1** when any sampled row disagrees beyond a float-noise tolerance, any collection could not be probed, or nothing was compared. A collection with no cross-collection projection population is not applicable, never a failure alone. No billed calls, but opt-in as a new audit surface, not a gate on the LATERAL/HNSW rewrite it checks |
