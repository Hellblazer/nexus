...
`nx doctor`'s `tuples.watch_permission` row (bead nexus-rml7o, MM-3.4 critic finding S5) checks this for you: it reads every settings file Claude Code consults for permissions — user `~/.claude/settings.json` (honouring `CLAUDE_CONFIG_DIR`), project `<root>/.claude/settings.json`, and project-local `<root>/.claude/settings.local.json` (`root` is the git top-level of the cwd, falling back to the cwd itself outside a repo) — never writing any of them, and reports whether a covering `permissions.allow` rule is present — the exact entry above, a genuinely broader `Bash(<prefix>:*)` ancestor such as `Bash(nx tuple:*)` or `Bash(nx:*)`, or a bare `Bash` rule — naming which file supplied it. A matching `permissions.deny` rule in ANY of the three files wins over a covering allow anywhere else, and the row reports that as denied, naming the denying file — the dangerous direction, since reporting covered while actually blocked would be worse than staying silent. A file that cannot be read is treated as carrying no rules, never a crash. On a box where the user-level Claude config directory (`CLAUDE_CONFIG_DIR`, or `~/.claude`) does not exist at all — Claude Code has never run there — the row reports `ok` with no warning instead: there is no permission surface to be missing a rule from yet (bead nexus-7zhag). Once that directory exists, the row is soft-informational, never fatal: `ok` when a covering allow rule stands unchallenged by any deny, a soft warning for a deny override or no covering rule at all. The row makes no claim about whether arming raises a permission prompt, in either direction — only whether a covering rule is present, which file supplied it, and what the rule is for.
