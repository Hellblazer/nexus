...
**Default routing by extension** (nexus-cvaw): `nx dt index --uuid X` without `--collection` picks the home based on file type. PDFs land in `knowledge__<corpus>-papers`, where `nx enrich aspects` routes each document by shape (nexus-kmbys): paper-shaped documents get `scholarly-paper-v1`, other prose gets `general-prose-v1` — a `knowledge__*` collection is never paper-only. Markdown notes land in `docs__<corpus>`, which gets no aspect extraction unless the collection is opted in through `aspects.docs_collections` (nexus-kk4ut; see [Configuration](configuration.md#aspects)), in which case its prose files get `general-prose-v1`. Pre-nexus-cvaw both extensions defaulted to `docs__default`, which stranded paper PDFs.
...
**An unknown collection name is refused, never reported clean (nexus-v1zdu).** Before computing anything, `nx t3 gc` (like `nx catalog audit-membership` and `nx catalog verify --collection`) routes `--collection` through a shared known-vs-unknown helper (`nexus.catalog.membership.refuse_if_collection_unknown`). A typo'd or never-registered name used to read identically to a genuinely empty, registered collection: both had zero manifest entries, so `--no-dry-run --yes` could exit 0 having deleted nothing, or worse, treat the empty result as license to condemn every live chunk it happened to find. The name is now checked against the catalog first: unknown refuses (exit 1, `ClickException`) naming the collection; known-but-empty falls through to the existing `--allow-empty-manifest-set` gate below.
...
Topics are auto-discovered after `nx index repo`, gated on the run actually having indexed files (an all-unchanged re-index skips the discover/kmeans/label/project/L1 pass entirely — a self-heal guard still runs discovery if a target collection has zero topics). A run that writes files refreshes co-occurrence links, topic links and the L1 context cache whenever its collections already have topics; the cross-collection projection pass runs only when discovery created topics, since the engine projects each new chunk as it is written. `nx taxonomy rebuild`, `merge` and `split` change one collection's topics in place and do not recompute other collections' projections onto them; each says so and names `nx taxonomy project --backfill --persist` as the refresh. The gate is per-collection: only collections whose own content kind (code / docs / rdr) wrote files this run enter the discover loop. Non-force discovery on a collection that already has topics returns instantly with `topics already exist … use `nx taxonomy rebuild` to re-discover` — previously the same no-op was decided only after fetching and clustering the full collection; incremental flush-grain assignment is what keeps existing topics current. Labeling with Claude haiku runs in a DETACHED background process spawned at the end of indexing (nexus-qqc1v) — the CLI exits immediately and labels land minutes later (progress: `~/.config/nexus/logs/deferred_labeling.log`; run `nx taxonomy label` manually if the spawn failed or you don't want to wait). Search results are grouped by topic and boosted when results share a topic cluster.
...
`nx index repo` defers taxonomy assignment, both the per-flush assign and this drain, when the engine's `/version` reports `process_uptime_seconds` under 600 (restarted within ten minutes: cold cache, the nexus-mg8gx loss), or when a run against the same engine lost an assignment in the last 15 minutes (recorded in `~/.config/nexus/taxonomy_assign_failed_at.<digest>`, one file per engine endpoint; delete it to stop the backoff early). `NX_TAXONOMY_DEFER_UPTIME_S` and `NX_TAXONOMY_FAILURE_BACKOFF_S` override the two thresholds. It prints `Taxonomy deferred: <reason>` once, and defers discovery in the same window while co-occurrence links, topic links and the L1 cache still refresh from the existing taxonomy (nexus-x3gig; a collection with no topics yet self-heals on the next run). Deferred chunks are not losses and do not fail the run; a later run's drain assigns them. Within one run, a batch still lost after the retry stops further assign calls for the rest of that run. An engine or edge that omits the uptime field defers nothing (the field only ever excludes). `nx taxonomy drain` and the MCP `store_put` path never defer (nexus-tawfg).

### `nx taxonomy acknowledge`

`nx taxonomy acknowledge CHASH [...] -c COLLECTION --note TEXT`, `--remove`, `--list`. The drain lists every unassigned chunk on every run, so a chunk the engine always refuses to assign would fail every `nx index repo`. (The other recurring exit checks already have escapes: index ignore patterns for a file that always fails to flush, `--allow-degraded-extraction` for a PDF the quality gate rejects.) After diagnosing such a chunk, acknowledge it with a note: the drain skips it in that collection instead of retrying it, prints `N acknowledged stuck (skipped)` on every run, and does not count it as a loss. Unacknowledged failures still fail the run. Acknowledgments are per collection (identical text in another collection is not covered) and live in T2 memory (project `nexus_taxonomy_ack`, title `<collection>/<chash>`), so every box and CI runner on the same tenant sees them. `--remove` withdraws one once the chunk is fixed (nexus-j7ae6).
...
| `project SOURCE` | Cross-collection projection: match chunks against other collections' centroids. `--against TARGETS` for explicit targets (default: sibling collections). `--threshold N` (optional; when omitted uses per-corpus defaults: `code__*` 0.70, `knowledge__*` 0.50, `docs__*`/`rdr__*` 0.55 — see [taxonomy-projection-tuning.md](exploration/taxonomy-projection-tuning.md)). `--top-k N` caps centroids considered per chunk (default: 3). `--use-icf` suppresses hub topics via Inverse Collection Frequency weighting (RDR-077); the corpus threshold always applies to the raw cosine and the ICF weight is clamped to 1, so it can only rank and suppress, never admit (GH #1528). `--persist` to write assignments. `--backfill` to project all collections against each other. `--prune-below-threshold` (recovery, GH #1528) deletes projection assignments whose stored raw cosine is below the corpus threshold: for the source collection given at its (or the explicit) threshold, or with `--backfill` for every corpus prefix at its default |
...
| `re-embed NAME --to MODEL` | In-place re-embed for Voyage models, `voyage-context-3` included since nexus-tysei (the engine embeds each chunk on its own, so an in-place CCE re-embed writes the vector a fresh index would; it also repairs stored vectors that no longer match their text). Service mode: same-model only — the computed vectors ride the verbatim passthrough; a cross-model `--to` fails loud (server-side embedding routes by the collection NAME's model segment; cross-model moves are the migration pipeline's job). `--no-dry-run --yes` to apply (6.3.1, nexus-c9xr2/u37lw) |
...
| `--check-assignments` | Cross-collection ("projection") topic-assignment audit (nexus-v4pj4): since engine-service-v0.1.132 the projection pass of `assign_from_chashes_<dim>` picks each chunk's nearest FOREIGN-collection centroid via an HNSW `LATERAL` (approximate) instead of an exact join — measured equal to exact under production insertion order, but a wrong pick would be silent. Samples chunks per collection that carry a stored `assigned_by='projection'` row, picks the MOST RECENTLY DECIDED row per chunk as the stored pick (never the highest-recorded-similarity row — a chunk reassigned to a different topic can leave a stale, higher-similarity row behind; `topic_assignments`' conflict key is per `(doc_id, topic_id)`, not per `doc_id`), re-fetches the stored vector and every live foreign centroid, and recomputes the exact nearest centroid RESTRICTED to foreign topics that already existed (by `created_at`) at or before that decision's own `assigned_at` — a topic discovered afterward is excluded, so healthy taxonomy growth since the assignment was made never reads as a wrong pick. Compares with the same tie-break the engine's own `ORDER BY distance, topic_id` uses. This eligibility-cutoff filter does not catch an EXISTING topic's centroid vector being later revised (rebuild/merge onto the same topic id) — closing that fully needs a same-moment engine-side ANN recompute this client-only check cannot add (the one read-only ANN route, `/v1/taxonomy/centroids/query`, runs different HNSW settings — `relaxed_order`/`ef_search=n_results` — than the cross pass's `strict_order`/`ef_search=400`, and `assign_from_chashes` has no dry-run form); tracked as the open half of nexus-v4pj4. At the default sample of 20, a systemic wrong-pick rate of 10% is caught with ~88% probability, 20% with ~99%; a rare, isolated bad pick under 1% of a collection's population may not land in any one run's sample. `--assignments-sample N` (default 20, max 300; the pool is sized from the collection's own known projection density, with a floor, rather than a flat oversample factor), `--assignments-collection NAME` (repeatable; default: every collection with a cross-collection projection assignment), `--assignments-seed` (default: today's UTC date). **Exit 1** when any sampled row disagrees beyond a float-noise tolerance, any collection could not be probed, or nothing was compared. A collection with no cross-collection projection population is not applicable; a collection with a known population this sample failed to reach is reported INCONCLUSIVE — neither counts as a failure alone. No billed calls, but opt-in: a full run fetches every foreign centroid tenant-wide per collection audited plus the full topic table once, real work at ~800-centroid scale, and its false-positive behavior against a real corpus is not yet observed |
...
| `--limit N` | Page size; omitted returns every matching subspace unpaged |
| `--after CURSOR` | Subspace-name cursor from a prior truncated page |
| `--json` | Output as a JSON array |
...
| `set-status STATUS` | Flip an RDR's `status:` frontmatter field and README row (refused unless the lifecycle table allows the transition); then append `needs-reexamination` to the T2 entry of every RDR joined to it by a `supersedes` edge (RDR-201 P3.3). A flip to `superseded` first writes the `superseded_by` successor's `supersedes` catalog edge itself (idempotent; the walk never waits for an index run), and `nx index repo` re-feeds an RDR whose content changed to the dependency link generator, so a frontmatter edit seeds its edges at the next index. `--date YYYY-MM-DD` sets `accepted_date`/`closed_date` (default today, UTC); `--root DIR` names the repo root (default git toplevel); `--reason TEXT` is required for `draft` → `closed` (the table's guarded `close-unaccepted` edge, for work that shipped without the record ever being accepted) and ignored on every other edge. Re-running the command for the status the file already holds completes the T2 mirror a T2-unreachable first run left behind, and only along an edge the table admits |
