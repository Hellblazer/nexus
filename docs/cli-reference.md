...
**Write-time guard census (7.22.0, nexus-41zr9).** Each mutation arm opens with `Write-time guard (playbook §5.4, as of <date>): <status> — <guard> [<where>]` naming the guard that prevents the population it is about to clean from recurring: `shipped`, `shipped-with-residuals` (with the residual beads listed), or `UNGUARDED`. Today two arms are UNGUARDED: `recount` (the chunk_count desync writer it repairs after, nexus-wu8s1, is still unfound) and `tombstone-vanished` (nothing keeps benchmark/gate debris collections — the dominant vanished population — out of the production namespace; the API delete/rename path is cascaded, debris never passes through it). A residual no bead owns is printed as `UNOWNED residual: … — no bead names it` on every run, never silently. The table in `reconcile_stale.py` is the record (the playbook points at it rather than restating it); the printed `as of` date says how old its claims are, and the shakedown's instrument-freshness census is what catches a row that has rotted. The census `--json` carries the table as `write_time_guards` plus `write_time_guards_as_of`.
