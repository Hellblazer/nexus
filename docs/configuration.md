...
**Embedding tiers**: Tier 0 (bundled MiniLM-L6-v2, 384d) is always available. Ask for tier 1 (bge-base-en-v1.5, 768d, better quality; downloads the model on first embed) when you install the CLI: `uv tool install "conexus[local]"`. The extra is fixed at install time — it is recorded in the generation's install receipt and [`nx self install`](cli-reference.md#nx-self-install) threads it into every later generation, so an upgrade never drops it; there is no verb that adds it to an existing generation install. (On a box still on the legacy uv-tool layout, `uv tool install --reinstall "conexus[local]"` still adds it; on a generation install that same command rebuilds the legacy uv tree over the nexus-owned shims instead.)
