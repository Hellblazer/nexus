...
**No renew, and no reply inside the ack.** A claim cannot be extended, so a mailbox claim lasts at most fifteen minutes, and this is safe by scope rather than by construction: nothing today holds a claim longer. A reply is a separate `out` after the take that claimed the request, so a reader that crashes between the two sees the request again at lease lapse and repeats the work, and no reply is lost because none was sent. Both a `renew` and an `ack` that carries its reply in one transaction are named as later candidates.
