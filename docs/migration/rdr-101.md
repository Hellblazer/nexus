...
runs `nx catalog prune-deprecated-keys` to drop eight legacy chunk
...
RDR-101 Phase 5c (`nexus-o6aa.13`) additionally removed these from
`ALLOWED_TOP_LEVEL`; pre-5c collections retain them until pruned:
...
For most catalogs, a single command runs the whole Phase 3 + Phase 4
sequence (synthesize-log + backfill + prune + drain backfill +
doctor):

```bash
nx catalog migrate --i-have-completed-the-reader-migration
```

This is the recommended path. The verb is idempotent at every stage;
re-running on a partially-migrated catalog picks up where it left off.

If you prefer per-step control (e.g. staging per-collection on a
large catalog or mid-stream debugging), the manual sequence is:
...
# 5. Drain the deferred class.
nx catalog t3-backfill-doc-id

# 6. Verify steady state.
nx catalog doctor --t3-doc-id-coverage --replay-equality
```
