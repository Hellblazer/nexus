...
**Action**: do **NOT** supersede this name — superseding sets `superseded_by`, which permanently excludes the name from resolution, and the revive path is identity-gated (only the collection a row was superseded FROM may revive it), so superseding a tombstoned name turns a reversible trash into an unreachable orphan (nexus-e1k14, the bug this symptom class exists to prevent). Run `nx catalog trash` to list the tombstoned document(s) that back this collection, then `nx catalog restore <tumbler>` for each one you want back (nexus-dkymw — `document_restore`, a PG function since `catalog-003-soft-delete.xml`, now has an operator-facing caller). Once restored, the chunks reappear in the live T3 view and the collection drops out of this report on its own — **provided the tombstone was made by `nx catalog delete` and nothing else has touched the chunks since** (review round 2, T2 [24834]). Past `nx catalog purge-trash`'s grace window the tombstoned row is physically reclaimed and restore is no longer possible — recovery at that point means re-indexing the source. A tombstoned document's chunks are now protected until purge-trash reclaims the row for `nx t3 gc`'s orphan sweep too (Sam's second 2026-09-07 ruling on nexus-dkymw: its alive-set protects a tombstoned-but-not-yet-purged document's chashes, superseding nexus-mqd6t's original immediate-exclusion filter for that one read), so its own chunk-`indexed_at` clock can no longer reap chunks inside the window above. Two paths still bypass it entirely: the MCP `store_delete` tool (tombstones the row and hard-deletes the T3 chunk in the same call — no window at all), and `nx collection delete`/`nx collection prune` (irreversible, never tombstones) — after either of those, `nx catalog restore` still reports success on the catalog row, but the chunks will NOT reappear; confirm with `nx catalog show`'s chunk count before assuming the content is back, and treat a still-empty collection after restoring as a signal to re-index rather than to retry restore.
