...
nx search "resolve_active_session_id" --lexical           # + exact-text hits (different flag, different job)
...
`search()` and `query()` share one corpus resolver (`_resolve_corpus_target` in `src/nexus/mcp/core.py`). A bare prefix (`code`, `knowledge,code,docs`, or `all`) expands to every live collection under that prefix. Within that expansion, a collection is dropped only when it is *both* below `_FANOUT_MIN_COLLECTION_CHUNK_COUNT` (3 chunks) *and* at least one other collection sharing that prefix is at or above it — a thin collection riding beside a populous sibling is pure fan-out noise. The floor is deliberately **relative, not a flat per-collection cutoff**: a prefix with only one live collection is never pruned (there is no sibling to be noise beside — a fresh install's first note, alone in `knowledge__knowledge` at 1 chunk, must still be found by a bare `knowledge` search), and a prefix where every collection is thin is left alone too (no healthy sibling marks any of them as noise by comparison).

**The count is chunks, not documents** — the exact tombstone-filtered row count `list_collections()` already reports, deliberately, even though the bead that requested this floor spoke in document counts (`code__1-4` at "1 doc"). A document-count source exists (`HttpCatalogClient.collection_doc_counts()`, one batched call), but was rejected: it needs `get_catalog()` to be non-`None` (a local/no-catalog install would see every count as unknown and the floor would never fire), and it only covers catalog-registered documents — a T3 collection with no catalog entry (a real category, see `classify_t3_orphan_collections`) would always read as unknown and never be excluded, which is backwards, since an orphaned collection is disproportionately likely to be exactly the thin, noisy kind this floor targets. Chunk counts cover every live collection unconditionally, with no catalog dependency, at zero extra round trip (the `list_collections()` call the resolver already makes to enumerate live collections). Practically: at a floor of 3 chunks, this excludes only a collection of roughly one file or less riding beside a healthy sibling — a two-file repo produces well over 3 chunks in practice and stays searchable.

**What this floor does *not* claim to fix**: the bead's other flagged collection, `code__1-43` (211 documents, 94.0% zero-hit), is nowhere near 3 chunks and is **not** excluded by this floor, nor was it meant to be — Sam's disposal note calls it a real, legitimately small corpus (`obsidian-smart-connections`, 121 files) whose zero-hit rate is expected noise from riding the default `code` fan-out's share, not a broken index. This floor only removes near-empty riders (roughly one file or less); it is not a proxy for "small enough to deserve exclusion" at any larger scale, and a collection like `code__1-43` continues to be searched, and to accumulate zero-hit telemetry, exactly as before.

Naming a collection explicitly — a full conformant name (`code__myrepo__voyage-code-3__v1`) or the short 2-segment form (`code__myrepo`) — always searches it regardless of size; the floor only prunes the bare-prefix branch.

**Visibility.** An excluded collection is named in the tool's rendered text result — a one-line footer, `[excluded below fan-out floor: name1, name2]`, appended after any result-capping truncation so it is never itself truncated away — on both `search()` and `query()`'s plain corpus-based path (the `query()` catalog-param path — `author`/`content_type`/`follow_links`/`subtree` — does not yet carry this footer). Each exclusion is also logged at `structlog` debug level (`corpus_fanout_excluded_below_floor`). `nx doctor`'s default supplementary-check sweep additionally runs a read-only fan-out-floor census (no CLI flag; always on, cheap — one `list_collections()` call, no per-collection round trip) naming every collection currently excluded across every prefix, because an excluded collection stops accumulating `search_telemetry` zero-hit data the moment it drops out of the default fan-out — a genuinely broken/partial index (this bead's own original hypothesis, before it was ruled out) would otherwise go quiet instead of visibly zero-hit.

**Staleness.** Collection names and counts are cached for up to 60s (`_COLLECTIONS_CACHE_TTL`) to avoid a `list_collections()` round trip on every call. `store_put`/`store_delete` invalidate this cache immediately (mirroring the existing page-turn cache invalidation), so a collection crossing the floor from a committed write in the SAME process is visible on the very next call. A write from a different process (the CLI, another MCP server instance) is still bounded by the 60s TTL.
...
Code chunks carry `section_type` too, set by the AST chunker at index time (`class`, `method`, or empty for module-level content). A chunk that is nothing but a file's package/module declaration plus its import statements, ignoring any comment (`section_type="imports"`, RDR-200 Phase 1c evidence hygiene, nexus-4jj40) is structural, not evidentiary: near-identical header boilerplate embeds close together across unrelated, self-indexed repos and crowds out real matches in cross-corpus semantic search. Filter it out explicitly the same way as `references`. This is implemented for exactly 12 named languages (java, python, javascript, typescript, tsx, go, c_sharp, kotlin, rust, cpp, c, scala), not "code" generally: a language outside this list never gets `section_type="imports"`, matching the same fail-conservative design as `_extract_context`'s `class`/`method` detection. A module/file docstring is NOT treated as a comment (it usually carries evidentiary content), so a chunk mixing a docstring with imports stays unstamped.
