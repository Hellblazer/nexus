...
**Finding**: The empirical ratio is 0.94–1.35x across 10+ stability runs (gate 2.0x). The inflation is real and explained: `cluster_and_persist` Phase A (`self._memory.get_all()`) briefly acquires `memory._lock` for the snapshot SELECT, and some fraction of concurrent `memory.search` calls land in that window. The long numpy clustering phase (Phase B) holds no locks, and the topic-write phase (Phase C) runs on taxonomy's own connection, so the bulk of the rebuild truly does not block searches.

**Classification note**: the gate is 2.0x (not 1.5x like the write-load tests). The write-load tests have no sibling store acquiring `memory._lock`, so their ratio should be ~1.0. The cluster-load test has a necessary Phase A `memory._lock` acquisition that inflates the p95 by a bounded amount. A 2.0x gate would catch any regression that kept `memory._lock` held across the full rebuild (that would be 10x+ inflation) while giving enough headroom for CI jitter. The RDR Success Criterion 2c is met: clustering does not block search *for its duration*, only for the brief snapshot read.
...
- **`cluster_and_persist` benchmark gap.** Initially listed as a carry-forward after the substantive-critic audit flagged Drift 2's "architecturally verified" framing as overclaim. Closed during the same session by adding `test_memory_search_under_cluster_and_persist_load` — see Drift 2 above. The test empirically demonstrates that Phase 2's per-store connection model keeps `memory.search` within 2.0x of baseline latency while the taxonomy runs full cluster rebuilds in a background loop (observed ratios 0.94–1.35x across 10+ runs). RDR Success Criterion 2c is now empirically verified, not just architecturally argued.
