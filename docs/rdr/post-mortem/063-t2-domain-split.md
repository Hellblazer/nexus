...
The overruns are honest reflections of domain scope, not bloat:

- **Facade** — the 367 LOC includes a ~60-line module docstring describing the architecture, imports, and thin delegate methods for 30+ public T2 operations. No implementation logic lives in the facade; the RDR's 50 LOC number assumed a minimal shim, not a backward-compatible surface.
- **memory_store** — absorbs memory CRUD + FTS5 sanitization + access tracking + TTL expire + RDR-057 consolidation helpers (`find_overlapping_memories`, `merge_memories`, `flag_stale_memories`) + migration guards. The RDR's 400 LOC estimate did not account for the consolidation-helper surface that RDR-057 had already grown into the monolith.
...
**Classification**: Minor. Numeric targets were aspirational. Functional modularity, import visibility, and test-site coverage are all in place. No refactor was skipped to hit a LOC budget.
...
### Drift 3 — Best-effort access tracking (discovered during Phase 2 review)
Not in the RDR plan. Phase 2 exposed a bug that Phase 1's global Python lock had masked: `memory.search(access="track")` does `SELECT` → `UPDATE access_count` → `commit` inside the memory store's lock. Under concurrent cross-domain writes, the `UPDATE` contended for SQLite's single writer lock and either (a) blocked the search for the full 5-second `busy_timeout` or (b) failed with `SQLITE_BUSY`. Both outcomes violated the acceptance gate.

**Fix** (commit `4871a75`, hardened in `aa5bcf9`): access tracking is now a best-effort statistical signal. The `UPDATE` runs with a temporary `PRAGMA busy_timeout = 0` (fail-fast) inside a try/finally that restores the 5-second default. `SQLITE_BUSY` is swallowed and logged at warning (`memory.access_tracking.skipped`); other `OperationalError` subclasses (SQLITE_CORRUPT, SQLITE_IOERR, SQLITE_CANTOPEN) still propagate. Under the under-load test, 5–10% of updates skip; the acceptance ratio dropped from "search fails or hangs" to 0.93–1.04x of the single-threaded baseline.

**Classification**: Scope creep, but necessary — the original behavior was only correct by accident under Phase 1's blanket serialization. This drift is a feature of the Phase 2 transition, not a gap in the plan.

## Carry-Forward Items (not addressed in this RDR)

1. **`memory.get()` contention.** `get()` has the same `SELECT` → `UPDATE access_count` pattern as `search()` but did not receive the fast-fail fix. Under heavy cross-domain write load, `get()` can still block for up to 5 seconds on `busy_timeout` before returning. Out of scope for `nexus-s8o5` (whose F5 gate was scoped to memory_search). A follow-up could apply the same fast-fail treatment, or a broader pattern where any best-effort write-side-effect on a read path uses a shorter timeout.

2. **CatalogTaxonomy.get_topic_docs Known Defect** (RDR-063 §Known Defect Option 3). T3-origin topics return `title=doc_id` as a fallback because the memory table doesn't hold those rows. Pinned by regression test `test_get_topic_docs_known_defect_project_collection_mismatch`. Documented, not fixed — the fix requires RDR-061 catalog integration work.
