...
Layer 2 (assumption audit via T2 research findings) was **never invoked** across all three RDRs. `/conexus:rdr-research` was never called. When Layer 2 ran during gate review, it printed "No research findings recorded" and moved on.

**Root cause**: The author forgot `/conexus:rdr-research` existed. This is a discoverability problem, not a process design problem. The tool was not surfaced to the user at the right moment in the workflow. Layer 2 wasn't dead because it's useless — it was dead because the tool wasn't discoverable.
...
**Problem**: Both Layer 1 and Layer 2 failed. Layer 1 should have blocked on absent research findings (per `workflow.md`) but didn't. Layer 2 was never invoked because the author didn't know `/conexus:rdr-research` existed.
...
**Option A — Enforce with discoverability**: Require at least one `/conexus:rdr-research` finding before `/conexus:rdr-gate` can run. Make `rdr-gate` print a clear prompt: "No research findings recorded. Run `/conexus:rdr-research add <id>` to record findings, or `--skip-research` to override." This addresses the discoverability problem directly.
