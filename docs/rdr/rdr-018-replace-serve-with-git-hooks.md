...
(`~/.config/nexus/locks/<repo-hash>.lock`) before indexing. Lock behaviour is controlled
by the `--on-locked={skip,wait}` CLI flag (default: `wait`):

| Caller | `--on-locked` | Behaviour |
|--------|---------------|-----------|
| Hook script | `skip` (explicit) | Exits immediately — concurrent hook fires during `git rebase -i` are silently dropped |
| `nx index repo` (interactive) | `wait` (default) | Blocks until the lock is released, then runs |
| `nx index repo --frecency-only` | lock bypassed | Frecency-only runs do not write chunks and cannot conflict |

The generated hook stanza always includes `--on-locked=skip`. Interactive invocations
use the `wait` default and are never passed `--on-locked` explicitly.
...
- [ ] `nx index repo` accepts `--on-locked={skip,wait}` (default: `wait`); hook scripts generated by `nx hooks install` include `--on-locked=skip`
- [ ] `index_repository` acquires a per-repo file lock; hook callers skip, interactive callers wait; frecency-only bypasses the lock
- [ ] `nx hooks install` sets the executable bit on newly created hook files (`chmod +x`)
- [ ] `index_repository` updates `head_hash` in the registry after a successful full-index run (not on frecency-only)
