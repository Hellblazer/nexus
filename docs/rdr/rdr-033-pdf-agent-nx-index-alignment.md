...
2. `/conexus:pdf-process` skill activated, which spawned `pdf-chromadb-processor` agent
...
1. **Skill invocation overhead**: `/conexus:pdf-process` skill spawns a haiku agent, which then tries to run bash commands that get sandboxed
...
- **Backward compatibility**: The agent name (`pdf-chromadb-processor`) and skill invocation path (`/conexus:pdf-process`) remain unchanged. Existing workflows that reference these names continue to work.
