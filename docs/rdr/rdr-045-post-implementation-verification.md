...
This check attempts to infer session context via a scratch proxy. It is explicitly **best-effort**: if the marker is absent (review-code was not invoked, or scratch is unavailable), the check **fails open** with an advisory warning: "No review marker found — consider running /conexus:review-code before closing." It does NOT block.
...
The `/conexus:review-code` skill must be modified to write a T1 scratch marker on completion:
...
|-------------|----------------|
| `/conexus:review-code` writes scratch marker on completion (see Layer 2b) | Enables review-presence tracking |
| `/conexus:review-code` flags deferred items → prompts "create beads for these" | Deferred items vanishing into RDR prose |
| `/conexus:substantive-critique` compares implementation against RDR stated goals | Category 3 semantic gaps (correct code, wrong behavior) |
...
**Tests-as-oracle caveat:** The mechanical gates assume the test suite is an independent correctness oracle. When tests and implementation are co-generated by the same agent in the same session, this assumption degrades — the tests may verify the wrong behavior. `/conexus:test-validate` addresses test quality concerns and should be invoked for critical work. The mechanical gate catches "tests don't pass," not "tests test the right thing."
