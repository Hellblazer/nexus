...
2. **Probe step adherence**: the agent/user running the plan actually executes `/conexus:composition-probe <id>` (Gap 3 is convention-based, not structural)
...
**CA-5 full deferral is doubly justified post-Phase-1b on the historical target set**: (1) cost — architectural rework is required to support cross-bead method-ownership lookup (Finding 4), (2) scope — even if CA-5 full were implemented, it would not improve the catch rate **on the historical target set** because RDR-036 is out-of-scope for both detection rules. The deferral can be closed as "not needed for the historical incidents" rather than "deferred pending cost amortization." **Scope qualifier (retro 2026-04-11, M4 from T2 id 743)**: the doubly-justified claim applies specifically to the 4 historical ART incidents. It is NOT a general claim that CA-5 full would never improve catch rate — future plan structures with novel composition patterns (e.g., a coordinator that semantically composes outputs without declaring `bd dep add` edges) could surface a need for the richer detection. CA-5 full remains valuable as an optional refinement for future plan structures, narrowed from "for all targets" to "for non-historical targets where authors forget to declare semantic composition edges via `bd dep add`."
...
### Finding 5 (2026-04-11, Phase 1a spike): CA-1/CA-2/CA-3 VERIFIED for nexus patterns — Read-only subagent sufficient on the verified target set, but the target set did NOT include typed-generic "hard case" cross-file composition

Source: `nexus_rdr/066-research-5-ca1-ca2-ca3-hard-case-spike` (Phase 1a runtime hard-case spike). Dispatched `nx:codebase-deep-analyzer` against `src/nexus/search_engine.py:149` (`search_cross_corpus`) with a fixed-shape minimal relay. The target composes outputs from 4 source modules (`db/t3.py`, `types.py`, `scoring.py`, plus the search_engine itself) — a real cross-file composition, but with `Any` at injection boundaries rather than typed generics.

**Scope honesty (2026-04-11 retro post-close, from T2 id 743 Phase 1b spike finding)**: CA-1's original definition is "hard cases (cross-file generics, protocols, third-party library types)." The chosen target (`search_cross_corpus`) is cross-file but uses `Any` at injection boundaries — it is not a typed-generic hard case in the sense CA-1 enumerates. The `READ-ONLY-SUFFICIENT` verdict is therefore accurate for **the dominant nexus pattern** (`Any`-at-injection, runtime dict-key contracts) but has NOT been verified against typed-generic targets like `typing.Protocol` / `TypeVar`-parameterized compositions that would require symbol resolution. A retroactive independent critique (2026-04-11, T2 id 743) surfaced this scope-conflation explicitly.

- **CA-1** `READ-ONLY-SUFFICIENT-FOR-NEXUS-PATTERNS; TYPED-GENERIC-HARD-CASE-UNVERIFIED`: probe generated from `Read`/`Grep`/`Glob` only. No Serena symbol resolution needed **for the verified target set**. The `Any` type at injection boundaries (`t3: Any`) is the dominant nexus pattern — contracts are expressed as runtime dict-key presence, not typed generics requiring inference. Conftest-fixture discovery heuristic handles test isolation. **Cross-project application (e.g., ART, which uses typed generics including `typing.Protocol` and parameterized return types) may hit the Read+Serena escalation branch.** The Read-only architecture decision is valid for shipping the conexus plugin's composition-probe against nexus coordinators; it is NOT a claim that Read-only generalizes to arbitrary Python codebases with richer type systems.
...
- **CA-3** `WITHIN-BUDGET`: execution 1.93 seconds (5 tests, real EphemeralClient + ONNX MiniLM, no mocks). Well under the 30-120s ceiling. Generation ~8 minutes wall-clock for the verified target.

**Phase 3 architecture decision**: Read-only subagent is sufficient **for the nexus target set**. Tool budget for the composition-probe skill: `Read + Grep + Glob + conftest-search heuristic`. Serena escalation is reserved for `typing.Protocol` / `TypeVar` cases. When the probe is applied to non-nexus coordinator targets (e.g., ART), a Phase 1a-equivalent spike against that project's dominant type patterns is warranted before trusting the Read-only verdict. This caveat is retro-added per the scope conflation finding.
...
- [x] **CA-1**: A composition probe can be generated automatically from a coordinator bead description + its dependency beads' declared outputs, for "hard" cases (cross-file generics, protocols, third-party types).
  — **Status**: `READ-ONLY-SUFFICIENT-FOR-NEXUS-PATTERNS; TYPED-GENERIC-HARD-CASE-UNVERIFIED` — verified by Phase 1a spike (Finding 5) against `src/nexus/search_engine.py:149` (`search_cross_corpus`, composing 4 source modules via `Any` at injection boundaries). Probe generated from `Read`/`Grep`/`Glob` only, no Serena symbol resolution required **for the verified target set**. The `Any` type at injection boundaries is the dominant nexus pattern — contracts are expressed as runtime dict-key presence, not typed generics. **Scope honesty** (retro 2026-04-11, T2 id 743): the verified target is cross-file but NOT a typed-generic hard case in the sense CA-1's original definition enumerates. The Read-only verdict is valid for shipping the probe against nexus coordinators; it is NOT a claim that Read-only generalizes to codebases with richer type systems (e.g., ART uses `typing.Protocol` / `TypeVar`-parameterized returns). A typed-generic spike remains unverified and is the escalation path for cross-project application. **T2**: `nexus_rdr/066-research-5-ca1-ca2-ca3-hard-case-spike`.
  — **Implication for §Technical Design**: CA-1 Read-only branch selected for the conexus plugin's composition-probe against nexus-shaped coordinators. Phase 3 ships with minimal tool budget (Read + Grep + Glob + conftest-fixture heuristic). Read+Serena branch is reserved for `typing.Protocol` / `TypeVar` cases and explicitly triggers on cross-project application to codebases with richer type systems (a Phase 1a-equivalent re-spike against that project's dominant type patterns should precede trusting the Read-only verdict for any non-nexus target).
...
## When to Use
- User invokes `/conexus:composition-probe <coordinator-bead-id>`
...
- **Positive**: catches **3/4 of the 4 historical audit incidents** at bounded reopen cost — the in-scope inter-bead composition failures (RDR-073 GroundedLanguageSystem coordinator, RDR-075 DevelopmentalCurriculum Phase 2 + CogEMEmotionalModulator, RDR-031 Full pipeline wiring). The 4/4 denominator is preserved explicitly: the 4th incident (RDR-036 FactualTeacher.query HashMap short-circuit) is an intra-class failure mode, dropped from the probe's scope rather than caught — it re-attributes to RDR-068 dimensional contracts. **Do not read "catches 3/4" as if the denominator shrank to match the numerator** — the audit still counted 4 historical incidents; the probe framework covers 3 of them by design and RDR-036 belongs to a different intervention class. Actual catch rate is bounded below by (coordinator tag precision) × (probe step adherence) × (probe run integrity). Probe run integrity is VERIFIED for the verified target set (Phase 1a spike, Finding 5; see scope honesty note for typed-generic caveats). See §Research Findings §Finding 3 for the 3/4 framing and the RDR-036 re-attribution
...
- **Negative** (over-tag cost, unquantified — retro 2026-04-11 per T2 id 743): the fallback heuristic over-tags fan-in beads that depend on many siblings without actually composing their outputs. Each spurious probe is 30-120 seconds of subagent runtime. On a hypothetical 50-coordinator plan with (say) 40% false-positive rate, that is 20-80 minutes of wasted runtime per plan. **This RDR does not estimate the base-rate false-positive rate** — the concrete number is unmeasured and will only surface from RDR-067 audit-loop telemetry post-ship. **Soft kill-switch recommendation**: if cumulative probe runtime for a single plan exceeds N minutes (suggested starting value: 30 minutes, tunable via config), downgrade the plan's coordinator tags to "static contract only" — probe generation is skipped and only the RDR-068 dimensional contract check runs. Not implemented in v1; tracked as a possible v2 enhancement. Without telemetry or a kill-switch, over-tagging is unfalsifiable and the CA-3 audit-loop backstop (RDR-067) is the only measurement channel
...
- **Probe step skipped** (user or agent ignores the text instruction in the enriched bead description) → no probe runs; caught only by RDR-067 audit loop or by RDR-069 critic at close time. Acknowledged as a known limitation of the convention-based Gap 3 closure; a future RDR may add a structural hook. **Measurement path retro 2026-04-11 (T2 id 743)**: probe-step adherence is currently unfalsifiable — there is no telemetry hook that records "probe ran for bead X at time T". A lightweight v2 enhancement would add a `metadata.probe_last_run: <ISO timestamp>` field to coordinator beads, asserted by plan-enricher when transitioning to the next bead. Without it, the "probe step adherence" factor in the catch-rate equation is an unfalsifiable claim until RDR-067 audit telemetry comes online.
- **Confidently-wrong attribution** (retro 2026-04-11, M5 from T2 id 743) — the probe runs `isinstance` assertions, all pass, then a downstream exception fires from a different code path and the subagent attributes the failure to the wrong dependency bead. The agent reads the attribution as authoritative, reopens the wrong bead, the actual issue stays hidden. CA-2's fallback ("unattributed mode") covers ambiguous failures but NOT confident-but-wrong attributions. **Mitigation**: when the probe fires `bd reopen <dep>`, the user should always verify the dependency was actually involved in the composition before accepting the suggestion. Future hardening: have the probe surface a confidence score or require corroborating evidence for high-confidence attribution. Not implemented in v1.
...
- Shipped as **3.8.5** on 2026-04-11 (after RDR-069's 3.8.3 and the surgical-reindex 3.8.4). Release commit `433b036` on main; see PR #148.
...
- 2026-04-11 (fifth iteration) — **Finalization gate run** (`/conexus:rdr-gate 066`) returned PASSED (0 Critical) but surfaced 2 Significant findings and 4 Observations via Layer 3 critic dispatch. Fixes applied: (a) §Research Findings §Finding 3 4/4 qualification tightened to four conditional factors including the new CA-5b; (b) §Finding 4 "fallback reaches same coordinator set" claim acknowledged as structurally unvalidated — added explicit under-tagging failure mode analysis; (c) CA-5b added to gating CA set ("the four historical ART coordinator beads each have ≥2 declared `bd` dependencies"); (d) §Trade-offs §Consequences "catches 4/4" unconditionally → "catches up to 4/4" with full conditional chain; (e) §Risks extended with RDR-067-deferred-no-correction-channel risk (OBS-3) + fallback-heuristic coordinator-set under-tagging risk (SIG-1); (f) Phase 1 split into 1a (runtime hard-case spike for CA-1/CA-2/CA-3) and 1b (retrospective ART plan graph lookup for CA-5b); (g) Phase 2 happy-path test + SIG-2 negative test for CA-4 silent-omission mitigation made explicit; (h) Test Plan gains Scenario 5b (silent-omission negative test) and Scenario 5c (CA-5b retrospective lookup); (i) §Finalization Gate §Assumption Verification fixes CA-4 phase misattribution (was "Phase 1 spike", now "Phase 2 implementation + Scenario 5b"); (j) Prerequisites now reflects RDR-069 shipped (satisfied) and CA-4 + CA-5 fallback verified by analysis, with remaining unchecked CAs explicit. Gate T2 record: `nexus_rdr/066-gate-latest`. Re-gate against the 5th iteration returned `justified` (0 Critical, 0 Significant, 3 informational Observations, all explicitly non-blocking).
- 2026-04-11 (sixth iteration) — **Phase 1 execution landed two material findings** via parallel runtime spike (Phase 1a, Finding 5) and retrospective ART plan graph lookup (Phase 1b, CA-5b resolution). Fixes applied: (a) §Research Findings §Finding 3 rewritten — the nexus audit's "4/4 catch" ceiling was over-scoped by conflating inter-bead composition failures (RDR-073, RDR-075, RDR-031 — probe-catchable) with intra-class short-circuits (RDR-036 FactualTeacher.query HashMap — out of probe framework scope). Revised ceiling is 3/4. RDR-036 re-attributed to RDR-068 dimensional contracts as the appropriate intervention; (b) new §Finding 5 (Phase 1a Read-only sufficiency spike against `search_engine.search_cross_corpus`) documenting the CA-1/CA-2/CA-3 verification and the Phase 3 tool-budget decision; (c) CA-1 flipped from Unverified → `READ-ONLY-SUFFICIENT`, CA-2 → `INTERPRETABLE`, CA-3 → `WITHIN-BUDGET` (1.93s execution), all verified by Finding 5; (d) CA-5b flipped from Unverified → `STRUCTURALLY ADEQUATE — 3/3 on in-scope targets` with explicit deeper finding about intra-class vs inter-bead failure mode taxonomy; (e) §Finding 4 trade-off language updated to reflect that CA-5 full is now *doubly* deferred (cost + scope — it wouldn't help on the historical targets); (f) §Trade-offs §Consequences first bullet: "up to 4/4" → "3/4 historical audit incidents" with the RDR-036 re-attribution note; (g) §Technical Design note locked on Read-only branch (was conditional on CA-1 outcome, now decided); (h) §Prerequisites checklist — all CAs now checked, with Phase 1a/1b outcomes and T2 record references inline. T2 research records: `nexus_rdr/066-research-4-ca5b-retrospective` (id 734), `nexus_rdr/066-research-5-ca1-ca2-ca3-hard-case-spike`. Beads closed: nexus-9ps, nexus-ssp. Ready set: nexus-3k9 (Phase 2) + nexus-2pl (Phase 3) now both unblocked.
- 2026-04-11 (seventh iteration — close cycle + retro pass) — **Close arc on this RDR was non-trivial.** First Phase 5c attempt was a manual walkthrough that auto-closed RDR-066 by reusing the Phase 5b Run 3 critic verdict as the gate signal — exact silent-scope-reduction failure mode this RDR exists to prevent, applied to its own close flow. Rolled back via commit `cbe7290`. A `rdr_close_gate_hook` was added then reverted (`6cf0703`) per user direction (the hook over-scoped). The real close ran through `/conexus:rdr-close 066 --reason implemented` with a fresh Step 1.75 critic dispatch (commit `44e9fe9`) — the fresh critic caught 3 stale `4/4` references in prose sections (executive summary, §"Why this supersedes", §Phase 2 acknowledged false-positive risk) that Phase 5b Run 3 had missed. All 3 fixed. Close T2 audit chain (which critic gated which step): `nexus_rdr/066-validation-5b-independent-critic` (id 738) ran 3 times pre-close; `nexus_rdr/066-validation-5c-real-self-close` (id 739) is the rollback record; `nexus_rdr/066-close-post-mortem` (id 740) is the actual real-close record with the fresh Step 1.75 dispatch verdict. The fresh Step 1.75 dispatch verdict was the load-bearing close-time gate signal, NOT the Phase 5b runs.
- 2026-04-11 (eighth iteration — **this one**, retro cleanup post-close from RDR-067 spike) — A live `claude -p '/conexus:substantive-critique 066'` invocation during the RDR-067 CA-3 Phase 1b spike (T2 `nexus_rdr/067-research-2-ca3-phase1b-spike-result` id 743) surfaced 5 Significant + 6 Minor drift findings against the closed RDR-066. Retro fixes applied without reopening the RDR: (a) **S1** CA-1 scope conflation — disposition revised from `READ-ONLY-SUFFICIENT` to `READ-ONLY-SUFFICIENT-FOR-NEXUS-PATTERNS; TYPED-GENERIC-HARD-CASE-UNVERIFIED` with explicit caveat that the verified target uses `Any` at injection boundaries, not typed generics; cross-project (ART) application requires a re-spike; (b) **S2** Gap 3 measurement path — added retro note in §Failure Modes acknowledging probe-step adherence is currently unfalsifiable until RDR-067 audit telemetry comes online; future v2 enhancement is a `metadata.probe_last_run` timestamp field; (c) **S3** over-tag cost unquantified — added negative bullet in §Trade-offs §Consequences with hypothetical 50-coordinator / 40% false-positive runtime cost example and a soft-kill-switch v2 enhancement recommendation; (d) **S4** validation circularity — this Revision History entry (and the seventh-iteration entry just above it) document which critic runs gated which step of the close, separating Phase 5b pre-close runs from the close-time fresh Step 1.75 dispatch; T2 IDs cited inline; (e) **S5** 4/4→3/4 framing — §Trade-offs §Consequences first bullet rewritten to preserve the 4/4 denominator explicitly: "catches 3/4 of the 4 historical audit incidents" with explicit "do not read this as if the denominator shrank" framing; (f) **M1** Finding 5 / Finding 4 ordering — swapped so Finding 4 (CA-4/CA-5 feasibility, the earlier work chronologically) precedes Finding 5 (Phase 1a spike, the later work); (g) **M2** Phase 4 release version "3.8.3 (or whatever)" → "Shipped as 3.8.5 on 2026-04-11 (after RDR-069's 3.8.3 and the surgical-reindex 3.8.4); release commit 433b036 on main; see PR #148"; (h) **M4** doubly-justified deferral scope qualifier — narrowed from "for all targets" to "on the historical target set"; future plan structures may still benefit from CA-5 full; (i) **M5** confidently-wrong attribution risk — added to §Failure Modes as a distinct mode from CA-2's unattributed-failure mode; mitigation is user-side verification before accepting `bd reopen` suggestions; (j) **M6** audit artifact location — `nexus_rdr/nexus-audit-2026-04-11` (cross-project T2 namespace) is the load-bearing evidence; cited inline where it was previously bare prose. **Not addressed**: M3 (velocity note in revision history) — explicitly skipped as low-value housekeeping. **No reopen** — RDR-066 stays `status: closed`. The retro is documentation cleanup against a shipped RDR. Surfaced from RDR-067 CA-3 Phase 1b spike T2 id 743.
