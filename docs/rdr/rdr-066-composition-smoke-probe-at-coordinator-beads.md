...
- **INT-2 composition probe catches 4/4 incidents** — all of RDR-073, 075, 036, 031 would surface under a runtime composition smoke test. This is because all 4 incidents involve a coordinator whose runtime behavior diverges from its declared API contract.
...
Each of these probes is ≤50 lines of test code. None requires special infrastructure. All could have been generated by a plan-enricher-class subagent given the coordinator bead description.

### Critical Assumptions

- [ ] **CA-1**: A composition probe can be generated automatically from a coordinator bead description + its dependency beads' declared outputs, for "hard" cases (cross-file generics, protocols, third-party types). The easy case (plain types in self-contained functions) is verified by Finding 2. The hard case is unverified.
...
- [ ] **CA-4**: The coordinator-bead identification convention (`metadata.coordinator=true`) is reliably applied by plan-enricher or by users. If it's missed, the probe doesn't fire. Mitigation: plan-enricher agent prompt update to require `coordinator:true` tagging on beads whose enrichment names ≥2 prior-bead outputs as inputs.
  — **Status**: Unverified — **Method**: update plan-enricher prompt; verify on a sample plan
...
- [ ] CA-1 through CA-4 verified via a hard-case spike
...
- **6a**: synthetic composition failure injected into a test coordinator, verify probe catches
- **6b**: independent code review (substantive-critic on this RDR pre-close)
- **6c**: real self-close of RDR-066 via the new close flow (RDR-069 active)
...
- **Scenario 6** (recursive 6a): synthetic retcon in a test coordinator → probe catches → this RDR's close passes
