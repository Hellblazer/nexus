...
- **`nx:deep-research-synthesizer`** (`conexus/agents/deep-research-synthesizer.md`) — the agent that ran the 2026-04-11 audit. Takes a research question, mines sources, produces a structured synthesis.
- **T2 `rdr_process` project** — cross-project memory namespace already in use. Contains `failure-mode-silent-scope-reduction` (ART) and `nexus-audit-2026-04-11` (nexus). Accessible via `mcp__plugin_conexus_nexus__memory_get` / `memory_put` / `memory_search` / `memory_list`.
...
**The Claude Code harness `schedule` skill is the PRIMARY candidate.** Available tools: `CronCreate`, `CronList`, `CronDelete`, `RemoteTrigger`. Description from the session start skill list: "Create, update, list, or run scheduled remote agents (triggers) that execute on a cron schedule." Purpose-built for recurring remote agents — direct match for periodic audit dispatch. One cron-scheduled trigger per project, firing `/conexus:rdr-audit <scope>` at the chosen cadence (90 days per the RDR design), with no user intervention needed.
...
Source: `nexus_rdr/067-research-2-ca3-phase1b-spike-result` (id 743). Live test via `claude -p --max-budget-usd 1.00 '/conexus:substantive-critique 066'`.
...
**This verifies CA-3 Option A** (external cron/launchd + `claude -p`) end-to-end: the cron wrapper invokes `claude -p '/conexus:rdr-audit <project>'`, the headless session loads the plugin, the audit skill fires, the research-synthesizer subagent dispatches, the audit writes findings to T2 via MCP, and the process exits.
...
- [x] **CA-3**: A usable scheduling mechanism exists for periodic audits. **VERIFIED 2026-04-11** via Research Findings 1 + 2 (T2 `nexus_rdr/067-research-1-ca3-scheduling-mechanism-survey` id 742, `nexus_rdr/067-research-2-ca3-phase1b-spike-result` id 743). Primary mechanism selected: **external cron/launchd + `claude -p '/conexus:rdr-audit <project>'`**.
  ...
  — **Future enhancement (out of scope for this RDR)**: wrap cron/launchd install + health-check behind an nx MCP tool — `mcp__plugin_conexus_nexus__schedule_audit(project, cadence, machine)` — to centralize the per-machine setup friction across multiple machines. Captured as a possible v2 enhancement if user scale warrants it. v1 ships plain cron/launchd templates.

- [x] **CA-4**: The `rdr_process` collection template (for project-filed incidents) is rich enough to capture the pattern without being burdensome. Too heavy and projects won't file; too light and filings won't aggregate.
  — **Status**: **VERIFIED 2026-04-11** via Phase 3 (nexus-dqp.6). Template shipped at `conexus/resources/rdr_process/INCIDENT-TEMPLATE.md` with 6 frontmatter fields (`project`, `rdr`, `incident_date`, `drift_class`, `caught_by`, `outcome`) + 8 required narrative sections. `drift_class` enum matches the canonical prompt taxonomy exactly so sibling project filings aggregate without translation. Synthetic filing test at `rdr_process/demo-incident-ca4-unwiring` (id 756) verified that filings are parseable via `memory_search` with frontmatter block intact in retrieval preview. Full CA-4 disposition at T2 `nexus_rdr/067-ca4-synthetic-filing` (ttl=0 permanent). Full live-dispatch validation inherited from Phase 1b spike runs (T2 ids 751, 752, 753) which demonstrated `deep-research-synthesizer` correctly parses and classifies rdr_process entries using the same schema the template formalizes.
...
1. **`nx:rdr-audit` skill**: new skill at `conexus/skills/rdr-audit/SKILL.md` that wraps the proven pattern. **Core audit dispatch** takes a project target (defaults to ART; accepts any project name as argument), dispatches `nx:deep-research-synthesizer` with the canonical prompt (pinned in the skill), persists result to `rdr_process/audit-<project>-<YYYY-MM-DD>`, surfaces summary to user. The skill also exposes **bare-bones management subcommands** so users and agents can inspect and manage scheduling from inside Claude Code without shelling out to OS primitives by hand:
    - `/conexus:rdr-audit list` — list all scheduled audits across projects by parsing `launchctl list | grep rdr-audit` (macOS) or `crontab -l` (Linux). **Read-only.**
    - `/conexus:rdr-audit status <project>` — show next-fire timestamp (from launchd/cron inspection) and last-run outcome (from the most recent `rdr_process/audit-<project>-*` T2 entry). **Read-only.**
    - `/conexus:rdr-audit history <project>` — list the last N audit findings for a project from T2 via `memory_list` + `memory_get`. **Read-only.**
    - `/conexus:rdr-audit schedule <project>` — **print** the platform-specific plist/crontab install commands for the user to review and run manually. Does NOT auto-execute the install (system-level installs are privileged).
    - `/conexus:rdr-audit unschedule <project>` — **print** the uninstall commands for the user to review and run manually. Does NOT auto-execute.
...
## When to use
- User invokes `/conexus:rdr-audit <project>` (default: current project) — runs the audit
- Periodic audit reminder fires (launchd/cron invokes `claude -p '/conexus:rdr-audit <project>'`)
...
**Collection template shape** (`conexus/resources/rdr_process/INCIDENT-TEMPLATE.md`):
...
**Scheduling**: primary mechanism is external cron/launchd + a shell wrapper that invokes `claude -p '/conexus:rdr-audit <project>'` from the user's local machine. Shape (macOS launchd example — ship a plist in `scripts/launchd/` or the equivalent for Linux cron):
...
<dict>
  ...
  <array>
    ...
    <string>-p</string>
    <string>/conexus:rdr-audit ART</string>
  </array>
...
```cron
0 3 1 */3 * /usr/local/bin/claude -p '/conexus:rdr-audit ART' >> ~/.local/state/rdr-audit-ART.log 2>&1
```
...
|---|---|---|
| Audit skill | `conexus/skills/rdr-audit/SKILL.md` | **New** (create) |
| Research subagent | `conexus/agents/deep-research-synthesizer.md` | **Reuse** — proven in the 2026-04-11 audit |
...
| Collection template | `conexus/resources/rdr_process/INCIDENT-TEMPLATE.md` | **New** (create) |
...
Run `/conexus:rdr-audit ART` via the new skill and produce a result that matches (within reasonable variance) the 2026-04-11 manual audit. If the canonical prompt is correctly pinned, the result should be structurally identical: same incidents, same frequency estimate, same VERIFIED recommendation.
...
- Create `conexus/skills/rdr-audit/SKILL.md` with the pinned canonical prompt and subcommand dispatch
...
- **Tests**:
  - Run `/conexus:rdr-audit ART` and compare to the 2026-04-11 manual audit (MVV)
  - Run `/conexus:rdr-audit list` with at least one audit scheduled → output table includes that entry
  - Run `/conexus:rdr-audit status ART` → shows next-fire + last-run
  - Run `/conexus:rdr-audit history ART` → shows the 2026-04-11 audit
  - Run `/conexus:rdr-audit schedule ART` → prints plist/crontab with correct project substitution
  - Run `/conexus:rdr-audit unschedule ART` → prints uninstall commands (verify nothing is actually unloaded)

### Phase 3: Collection template

- Create `conexus/resources/rdr_process/INCIDENT-TEMPLATE.md`
- Document the template in the skill's help
- Optional: extend `/conexus:rdr-close` to offer the incident template when closing with `close_reason: partial` (not required for MVV)
...
- **Phase 4 scope (minimal)**: ship a shell wrapper `scripts/cron-rdr-audit.sh` that invokes `claude -p '/conexus:rdr-audit <project>'` with a `PROJECT` environment variable, and document the per-platform setup:
...
- The `/conexus:rdr-audit schedule <project>` **management subcommand** (defined in Phase 2's skill scope) handles the print-the-install-commands step — users run the commands manually. System-level installs are never auto-executed.
...
- **5b**: substantive-critic on this RDR (via `/conexus:rdr-close` Step 1.75 close-time gate)
...
- **Scenario 1**: `/conexus:rdr-audit ART` — matches manual baseline (±1 incident) with same VERIFIED recommendation (CA-1 seeding run)
- **Scenario 2**: `/conexus:rdr-audit nexus` — CA-1 generalization test; verdict must satisfy acceptance criteria (a)–(d) from CA-1
- **Scenario 3**: `/conexus:rdr-audit ART` run twice in a row — CA-2 consistency test (±1-2 incident variance, same recommendation tier)
- **Scenario 4**: file a synthetic incident using the INCIDENT-TEMPLATE — verify it appears in the next audit (CA-4)
- **Scenario 5**: launchd/cron test entry fires — install a near-term launchd plist (or Linux cron line) that invokes `claude -p '/conexus:rdr-audit ART'`, wait for it to fire, verify the audit runs to completion, verify the finding lands in T2 as `rdr_process/audit-ART-<date>`, inspect the log file for any errors. Unload the test entry after verification. Expected latency window: ~3-5 minutes end-to-end including audit subagent dispatch.
- **Scenario 6** (management subcommands): with at least one schedule active, run in sequence:
  - `/conexus:rdr-audit list` → output table includes the active entry
  - `/conexus:rdr-audit status ART` → shows next-fire timestamp + last-run outcome
  - `/conexus:rdr-audit history ART` → shows recent audit findings including the 2026-04-11 baseline
  - `/conexus:rdr-audit schedule ART` → prints plist/crontab with correct project substitution (verify no OS-level install side-effect)
  - `/conexus:rdr-audit unschedule ART` → prints uninstall commands (verify nothing is actually unloaded)
...
MVV: `/conexus:rdr-audit ART` matches the 2026-04-11 manual baseline. Concrete, measurable, executable in Phase 2.
...
- `conexus/agents/deep-research-synthesizer.md` — the agent the skill wraps
...
- 2026-04-11 — **Research Finding 4 / CA-3 Phase 1b end-to-end spike — VERIFIED**. Live `claude -p --max-budget-usd 1.00 '/conexus:substantive-critique 066'` test in a fresh headless main-thread session confirmed: plugin loads, slash command resolves, skill body executes, `Agent`-tool subagent dispatch works end-to-end. External cron/launchd + local `claude -p` selected as primary mechanism (Option A); `schedule` skill and GitHub Actions reclassified as structurally ruled out for the full audit scope (no local-file access to ART Dolt archive; no nx MCP without connector registration). CA-3 marked VERIFIED. Sub-finding: substantive-critic did NOT emit the canonical Verdict block in headless mode — retro-cleanup item filed. T2: `nexus_rdr/067-research-2-ca3-phase1b-spike-result` (id 743). See `research/rdr-067-ca3-spike` branch / PR #150.
- 2026-04-11 — **Gate PASSED + drift cleanup + management surface added**. `/conexus:rdr-gate 067` result: Layer 1 PASS, Layer 2 PASS, Layer 3 partial (0 Critical, 2 Significant, 5 Observations) — T2 `nexus_rdr/067-gate-latest` (id 744). This pass addresses:
...
- 2026-04-11 — **Research Finding 4 / CA-3 Phase 1b spike + option A selection**. Live `claude -p` test verified headless mode supports plugin slash-command invocation + subagent dispatch end-to-end. Further analysis of the `schedule` skill revealed structural constraints (no MCP connectors attached, remote agents cannot access local files like `~/git/ART/.beads/dolt/ART/`, minimum 1h interval, triggers cannot be deleted programmatically) that make it unsuitable as primary for RDR-067's full audit scope. **Option A selected as primary**: external cron/launchd + `claude -p '/conexus:rdr-audit <project>'` shell wrapper running in the user's local context with full MCP + local file access. Secondary candidates retained for future scope expansion (CCR `schedule` skill for constrained remote-only audits; GitHub Actions `anthropics/claude-code-action@v1` for CI-triggered audits). Future enhancement noted: wrap cron/launchd install + health-check behind an nx MCP tool (`mcp__plugin_conexus_nexus__schedule_audit`) if per-machine friction becomes painful across a multi-machine user base — not in scope for v1. RDR sections updated to reflect option A as primary: CA-3, §Problem Statement Gap 3, §Context, §Proposed Solution, §Technical Design Scheduling (with launchd plist + crontab examples), §Existing Infrastructure Audit, §Trade-offs §Consequences, §Failure Modes (launchd/cron failure modes instead of schedule-skill modes), §Implementation Plan Phase 4 (shell wrapper + per-platform templates instead of CronCreate invocation), §Day 2 Operations, §Test Plan Scenario 5. CA-3 disposition upgraded from unverified to **VERIFIED**. Sub-findings from the spike: substantive-critic canonical Verdict block is NOT emitted in headless `claude -p` mode (retro backlog for RDR-069); the spike's test dispatch against closed RDR-066 surfaced 11 post-close drift findings (retro backlog for RDR-066). Both sub-findings are logged as retro cleanup targets, not blocking for RDR-067 CA-3 verification. T2: `nexus_rdr/067-research-2-ca3-phase1b-spike-result` (id 743).

- 2026-04-11 — **Phase 5a MVV + portability rewrite (post-gate scope addition)**. The Phase 5a MVV first dispatch mis-resolved `ART` → `arcaneum` because the canonical prompt's original `~/git/{project}/docs/rdr/post-mortem/*.md` template was interpreted as a relative path in the subagent context, dropping the `~/git/` prefix and resolving against the main session's CWD. The retry with explicit absolute paths succeeded cleanly: VERIFIED / HIGH / 3 incidents / unwiring-dominant against the 4-incident baseline (Δ=1, within CA-2 ±1-2 bound; {073,075,036} overlap with baseline {073,075,036,031}; drift-category reclassification on RDR-073 from `dim-mismatch` to `unwiring` consistent with RDR-069 finding-level variance precedent). **Post-gate portability rewrite applied** to prevent the path-resolution bug from recurring: the skill body now resolves an absolute `{project_path}` via a new precedence chain (explicit absolute path arg → `NEXUS_PROJECT_ROOTS` env var → default candidate roots `$HOME/git`/`$HOME/src`/`$HOME/projects`/`$HOME/code`/`$HOME/work`/`$HOME/dev`/`$HOME/Documents/git` — none authoritative, all common conventions → unresolved fallback skipping file-based layer). The canonical prompt at T2 `nexus_rdr/067-canonical-prompt-v1` was upserted to v1.1 with `{project_path}` as a new substitution parameter replacing `~/git/{project}/` throughout the §Scope section, plus an explicit `## IMPORTANT — path handling` warning and a sanity-check instruction ("if your results contain files from a project other than `{project}`, STOP"). This is a **material post-gate scope addition** documented here per the silent-scope-reduction prevention principle the RDR itself formalizes. Root-cause investigation + fix disposition at T2 `nexus_active/rdr067-persistence-root-cause-fix` (id 758). Drift bead `nexus-cwm` opened and closed in the same session once the fix was applied. **MVV comparison** at T2 `nexus_rdr/067-phase5a-mvv-comparison` (permanent, ttl=0).

- 2026-04-11 — **Research-class agent persistence softening (post-gate scope addition, side-effect fix)**. Phase 1b spike disposition originally framed the "subagents do not reliably self-persist" finding as an instruction-compliance issue (0/3 deep-research-synthesizer runs called `memory_put` despite explicit relay instructions). Follow-up investigation during Phase 5a root-caused the actual cause: `conexus/agents/deep-research-synthesizer.md` had a hardcoded "MUST store your research findings to T3 knowledge BEFORE returning" primary directive + a `<HARD-GATE>` enforcement block, both keyed to `store_put(collection="knowledge")`. The agent was correctly following its trained T3 pattern; the Phase 1b spike dispatches landed in T3 as `rdr067-audit-run1-*` instead of the requested T2 target. Five sibling research-class agents (`deep-analyst`, `codebase-deep-analyzer`, `architect-planner`, `debugger`, `strategic-planner`) had identical hardcoded T3 mandates. **Softening applied** to all 6 agents: primary directive and HARD-GATE blocks rewritten to "MUST persist ... unless the dispatching relay specifies an alternative storage target in its Input Artifacts, Deliverable, or Operational Notes". Default behavior (T3 via `store_put` for generic `/conexus:research`-style dispatches) is preserved; dispatching skills like `nx:rdr-audit` can now redirect findings to T2 without fighting the agents' trained pattern. Phase 5a MVV dispatches confirmed the softening works — both the first (mis-scoped) and retry dispatches used `memory_put` to T2, neither called `store_put` to T3. This is a **material post-gate scope addition** documented here per the same principle as the portability rewrite above. Not in the original RDR-067 Technical Design, but a necessary side-effect fix without which the `nx:rdr-audit` skill's persistence-ownership invariant would be redundant with (not complementary to) a conflicting agent directive. Full investigation at T2 `nexus_active/rdr067-persistence-root-cause-fix` (id 758). Files changed: `conexus/agents/deep-research-synthesizer.md`, `conexus/agents/deep-analyst.md`, `conexus/agents/codebase-deep-analyzer.md`, `conexus/agents/architect-planner.md`, `conexus/agents/debugger.md`, `conexus/agents/strategic-planner.md`.
