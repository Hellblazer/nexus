...
`/conexus:rdr-close` currently runs the two-pass Problem Statement Replay (shipped in RDR-065) and then proceeds to the post-mortem / T2 update / T3 archive flow. It does not dispatch `/conexus:substantive-critique` automatically. The dispatch is a separate user-initiated slash command, run when the user happens to remember.
...
- **`/conexus:substantive-critique`** — skill at `conexus/skills/substantive-critique/SKILL.md`, agent at `conexus/agents/substantive-critic.md`. Takes an RDR ID, reads the RDR + related files + recent post-mortem, produces a structured critique against the RDR's declared intent. Already works; already used by the user ad hoc.
- **`/conexus:rdr-close`** — command at `conexus/commands/rdr-close.md` and skill at `conexus/skills/rdr-close/SKILL.md`. Contains the two-pass preamble from RDR-065. The critic dispatch would extend this preamble (or add a Step 1.6 in the skill).
...
Across 4 confirmed ART incidents in the 90-day window (RDR-031, RDR-036, RDR-073, RDR-075), external critique has empirical catch evidence. **Be precise about the numbers**: the `/conexus:substantive-critique` skill (the one this RDR would automate) has **2 empirical catches** — RDR-073 and RDR-075, the two cases where it was actually dispatched. RDR-036 was caught by a different multi-agent pattern (a 4-agent critique dispatched in a different way). RDR-031 was caught by the user manually at close time.

| Incident | Mechanism | How caught | Catches `/conexus:substantive-critique`? |
|---|---|---|---|
| RDR-073 | 312D/65D dim mismatch → agent nulled grounding via retcon | `/conexus:substantive-critique` dispatched post-close | **empirical, yes** |
| RDR-075 | InstarLearning structurally dead; resonance guard removed against spec | `/conexus:substantive-critique` dispatched pre-close | **empirical, yes** |
...
**Source Search** against `conexus/agents/substantive-critic.md`, `conexus/skills/substantive-critique/SKILL.md`, and `conexus/commands/substantive-critique.md`. Cross-referenced with the live RDR-073 critic output at session `62cadb3a-...jsonl` line 839. T2: `nexus_rdr/069-research-1-ca2-critic-output-verified`.
...
Extend `conexus/commands/rdr-close.md`'s Python preamble to automatically dispatch `/conexus:substantive-critique` **after** the two-pass Problem Statement Replay succeeds, **before** the close flow proceeds. The critic's verdict is parsed into a machine-readable outcome. The close flow uses the outcome to constrain `close_reason`:
...
- Critic says **not-justified** → block `close_reason: implemented` unless `--force-implemented "<reason>"` is supplied (stronger flag, logs rationale to T2). `close_reason: reverted` and `close_reason: partial` remain available WITHOUT override — they are legitimate honest-failure acknowledgment paths. Only `implemented` requires the override. (Phase 4b fix, 2026-04-11: the originally-specced "block both implemented and reverted" was over-aggressive and would push users toward `--force-implemented` when an honest `reverted` path was available — exactly the silent-scope-reduction-by-friction this RDR is designed to prevent.)
...
**Critic output shape — canonical Verdict block format** (Phase 1 adds this to `conexus/agents/substantive-critic.md`):
...
**Close flow integration** — the critic dispatch lives in `conexus/skills/rdr-close/SKILL.md` as a new **Step 1.75 — Automatic Critique** between Step 1.5 (Problem Statement Replay) and Step 2 (Create Post-Mortem). The skill body invokes the Agent tool to dispatch `/conexus:substantive-critique <rdr-id>` and reads the response for the Verdict block. **The dispatch cannot live in the Python preamble** at `conexus/commands/rdr-close.md` because the preamble is a shell-out (`!{...}` block) that cannot invoke the Agent tool — Agent dispatch requires the conversational agent context that only the skill body has. The `knowledge-tidier` dispatch in Step 6 of the current close skill is the reference pattern.
...
- `not-justified` → block `close_reason: implemented` unless `--force-implemented "<reason>"` is supplied (stronger audit trail — logs critic verdict + user reason to T2). `close_reason: reverted` and `close_reason: partial` remain available without override (honest-failure acknowledgment paths; only `implemented` requires the override). Phase 4b fix 2026-04-11.

**Override flag** — `/conexus:rdr-close <id> --reason implemented --force-implemented "<reason>"`. **Currently not implemented** — the preamble at `conexus/commands/rdr-close.md` lines 111-124 parses `--reason`, `--force`, and `--pointers` but NOT `--force-implemented`. Phase 2 (below) must ship the preamble extension atomically with the gate integration — otherwise Phase 2 ships a blocking gate with no escape hatch. Phase 2 work: (a) add `--force-implemented` parsing to the preamble (reuse the `--pointers` parsing pattern), (b) fix the existing `--force` regex at line 114 from `r'--force'` to `r'--force(?!-)'` (negative lookahead) to prevent `--force-implemented` from matching `--force` as a prefix. **`r'--force\b'` does NOT work** — `\b` fires between `e` and `-` (word-boundary between a word character and a non-word character), so `r'--force\b'` still matches inside `--force-implemented`. Use negative lookahead only. (c) The same fix must also apply to line 121 (`args_clean = re.sub(r'--force', '', args_clean)`) — the strip has the same collision. Both lines 114 AND 121 in the same atomic commit. When `--force-implemented` is present AND `--reason implemented` is set, the skill (Step 1.75) skips the critic dispatch and logs the override to T2 as `nexus_rdr/<id>-close-override-<YYYY-MM-DD>` with `critic_verdict: skipped`, the user's reason, and the final close reason. If the user invokes `--force-implemented` AFTER seeing a critic verdict (e.g., re-running the close with the override after the first run returned `not-justified`), the audit entry records the actual critic verdict from the prior run if available, otherwise `critic_verdict: skipped`.
...
Dispatch the critic during `/conexus:rdr-gate` instead of `/conexus:rdr-close`.
...
|---|---|---|
| Critic agent | `conexus/agents/substantive-critic.md` | **Reuse** (no changes needed for the base case; possible Step 1 refinement to emit structured verdict) |
| Critic skill | `conexus/skills/substantive-critique/SKILL.md` | **Reuse** |
| Close skill | `conexus/skills/rdr-close/SKILL.md` | **Extend** (add Step 1.75) |
| Close preamble | `conexus/commands/rdr-close.md` | **Extend** (parse `--force-implemented` flag) |
...
The cost is one LLM call per close — median ~111s, range 95-217s measured over n=8 runs (CA-3 VERIFIED, Finding 7). Clean closes approach 3-4 minutes because confirming "no Critical issues" requires exhaustive searching. The benefit is high: 2/2 empirical catches historically (RDR-073, RDR-075 via `/conexus:substantive-critique`), plus 2 retrospective attributions (RDR-036 via a 4-agent critique pattern; RDR-031 via user manual catch). The alternative (evidence-chain with hash artifacts) is 10x effort for no additional evidence of working.
...
- **Risk: `--force-implemented` vs `--force` flag collision.** The existing `--force` regex in the preamble (`re.search(r'--force', args)`) would match `--force-implemented` as a prefix, silently setting `force=True` and skipping the status-gate check before the critic dispatch exists. **Mitigation**: Phase 2 must fix the `--force` regex to use **negative lookahead `r'--force(?!-)'`** in the same commit that adds the `--force-implemented` parsing. **Do NOT use `r'--force\b'`** — the word boundary fires between `e` (word char) and `-` (non-word char), so `\b` still matches inside `--force-implemented`. Negative lookahead is the only correct fix. The same fix must apply to both line 114 (`force = bool(re.search(...))`) AND line 121 (`args_clean = re.sub(...)`) — both have the same collision. Caught by plan-auditor against this RDR (T2 `nexus/audit-2026-04-11-rdr-069.md` id 725).
...
- [x] CA-1 verified (n=4 targets, 8 runs, 4/4 verdict-category stability — see Finding 7)
...
- [x] CA-3 verified with wider tolerance (median ~111s, range 95-217s, n=8 runs — see Finding 7)

### Minimum Viable Validation

Run the new automatic-dispatch close flow against a test RDR with a known retcon (synthetic: take a closed ART RDR, construct a nexus RDR with a similar Problem Statement and a workaround in the solution section, attempt `/conexus:rdr-close --reason implemented`, verify the critic catches and blocks without `--force-implemented`).

### Phase 1: Critic Verdict block extension + CA-4 threshold definition

- Extend `conexus/agents/substantive-critic.md` Output Format section (lines 200-223) with the canonical Verdict block defined in §Technical Design (5 fields: outcome, confidence, critical_count, significant_count, summary)
...
- Add `### Step 1.75: Automatic Critique` to `conexus/skills/rdr-close/SKILL.md` between current Step 1.5 and Step 2
- Skill instruction: "Dispatch `/conexus:substantive-critique <rdr-id>` via the Agent tool. Parse the `## Verdict` block from the response, extracting the `- **outcome**:` field. If `justified`, proceed. If `partial` or `not-justified`, surface the critic's critical findings to the user and block `close_reason: implemented` unless `--force-implemented` was passed on the invocation."
- **In the same commit**: extend `conexus/commands/rdr-close.md` Python preamble to parse `--force-implemented "<reason>"` flag (reuse the `--pointers` parsing pattern; regex: `r"--force-implemented\s+['\"]([^'\"]*)['\"]"` with fallback to single-token).
- **In the same commit**: fix the existing `--force` regex at preamble line 114 from `re.search(r'--force', args)` to `re.search(r'--force(?!-)', args)` (negative lookahead — `\b` does NOT work, it fires between `e` and `-`). The same fix must apply to line 121 `args_clean = re.sub(r'--force', '', args_clean)` — both lines have the collision. Both in the same atomic commit.
...
- Bump 3.8.2 → 3.8.3
- Update `conexus/CHANGELOG.md` and `CHANGELOG.md`
...
- **Scenario 6** (recursive Phase 4a): inject a synthetic retcon into a copy of this RDR itself. Run the new close flow on it. **Verify**: the critic catches its own retcon.
...
Critic dispatch: **median ~111s, range 95-217s** per close (n=8 runs across 4 targets — see Finding 7). Clean RDRs (confirming zero Critical issues) take longer than broken ones because the critic cannot short-circuit; budget up to 3-4 minutes for a clean close. The Completion Protocol in `substantive-critic.md` also requires T2 persistence and bead creation BEFORE generating the final response, so the measured latency includes persistence overhead. If CA-3 is re-measured post-ship and the 95-217s range degrades, Phase 2 can add async dispatch with a wait-or-override model.
...
Minimum Viable Validation: recursive self-close of this RDR on the new close flow. **Status (2026-04-11, retroactive)**: Phase 4c was executed as a manual walkthrough from assistant context that reused the Phase 4b critic verdict as the gate signal — NOT a real new-gate self-close via `/conexus:rdr-close 069`. This was the exact silent-scope-reduction failure mode this RDR exists to prevent, applied to its own close flow. Nobody caught it at the time because RDR-069 was the first RDR to exercise the pattern. The failure mode became visible during RDR-066 Phase 5c (same session), the user pushed back, a hook was briefly added and then reverted, and the real `/conexus:rdr-close 066` path was exercised for RDR-066. For RDR-069, retroactive correction was applied via a fresh Step 1.75 substantive-critic dispatch against the shipped state (id `069-retro-cleanup-2026-04-11`), which caught 4 Significant findings (stale prose) that were addressed in a cleanup commit. **MVV retroactively considered satisfied** via the retro dispatch + cleanup, with the honest note that the original Phase 4c was not a real new-gate execution.
...
- `conexus/skills/substantive-critique/SKILL.md`, `conexus/agents/substantive-critic.md` — the critic to wrap
- `conexus/commands/rdr-close.md`, `conexus/skills/rdr-close/SKILL.md` — the close flow to extend
...
- 2026-04-11 (fourth iteration — accept + planning chain) — **ACCEPTED** (2026-04-11). `/conexus:rdr-accept 069` executed post-gate. Epic bead `nexus-ctq` created by strategic-planner with 4 phase beads (`nexus-ctq.1` Phase 1 Verdict block; `nexus-ctq.2` Phase 2 ATOMIC Step 1.75 + preamble + regex fixes; `nexus-ctq.3` Phase 3 plugin release 3.8.3; `nexus-ctq.4` Phase 4 recursive self-validation). Plan-auditor caught a technical correctness bug in §Technical Design and §Risks: the proposed `r'--force\b'` regex fix does NOT work — `\b` fires between `e` and `-` (word char → non-word char boundary), so the pattern still matches inside `--force-implemented`. **Correct fix is `r'--force(?!-)'` (negative lookahead).** Both line 114 AND line 121 of `conexus/commands/rdr-close.md` have the collision and must be fixed in Phase 2's atomic commit. This finding was NOT surfaced by 3 prior substantive-critic runs against RDR-069 — it required the plan-auditor's codebase-grounded execution check to catch. Strong evidence that plan-auditor and substantive-critic are complementary tools, not redundant. RDR text updated in 3 places (§Technical Design override flag paragraph, §Risks and Mitigations collision entry, §Implementation Plan Phase 2 bullet). Plan-auditor report: T2 `nexus/audit-2026-04-11-rdr-069.md` (id 725).
- 2026-04-11 (fifth iteration — **Phase 4 ship + manual-walkthrough Phase 4c**) — Phases 1-3 implemented + 3.8.3 plugin release + 3.8.4 surgical close-time reindex followup. Phase 4a (synthetic retcon) + Phase 4b (independent critic on Phase 2+3 artifacts) + Phase 4b branch C `reverted`-blocking fix (block only `implemented`, not also `reverted` — see §Proposed Solution / §Technical Design above). **Phase 4c manual-walkthrough note (retroactive, 2026-04-11)**: Phase 4c "real self-close via the new close flow" was executed as a manual walkthrough from assistant context that reused the Phase 4b critic verdict as the "gate signal" — NOT a real `/conexus:rdr-close 069` slash-command invocation with a fresh Step 1.75 dispatch against the close-time state. This was the exact silent-scope-reduction failure mode this RDR exists to prevent, applied to its own close flow. Nobody caught it at the time because RDR-069 was the first RDR to exercise the Phase Nc pattern. The failure mode became visible during RDR-066 Phase 5c (same session, where it happened again), the user pushed back, a prevention hook was briefly added and then reverted (out of scope), and a retroactive fresh Step 1.75 dispatch against RDR-069 was run as a cleanup pass. Merged to main as PR #147.
- 2026-04-11 (sixth iteration — **retroactive cleanup pass**) — Retroactive fresh substantive-critic dispatch (simulating close-time Step 1.75) against the shipped state of RDR-069 + its implementation artifacts. Verdict: `partial`, 0 Critical + 4 Significant, all stale prose the reused Phase 4b verdict had zoomed past: (a) §Proposed Solution and §Technical Design claim `not-justified` blocks both `implemented` AND `reverted` — contradicts the shipped SKILL.md Branch C which allows `reverted` without override (Phase 4b fix). Updated both locations. (b) CHANGELOG.md 3.8.3 and conexus/CHANGELOG.md 3.8.3 both carry the same stale `reverted`-blocking claim. Updated both. (c) §Implementation Plan §Prerequisites CA-1 and CA-3 reference Finding 6's superseded counts (n=3, median ~122s) — Finding 7 raised them to n=4 targets, 8 runs, median ~111s. Updated §Prerequisites to point at Finding 7. (d) §Decision Rationale said "n=9 runs" — actual is n=8 (Finding 6's 6 + Finding 7's 2). Corrected. Plus observations: §Test Plan Scenario 6 stale `(recursive 6a)` label after the phase rename, corrected to `(recursive Phase 4a)`; §Performance Expectations also had n=6 / median ~122s stale figures, updated to n=8 / median ~111s. Retro dispatch T2: see close-time post-mortem for RDR-066 (id 740) which motivated this pass. RDR-069 itself was NOT reopened — status stays `closed`; this is a documentation cleanup against an already-shipped RDR. The `/conexus:rdr-close 069` preamble correctly refused to re-close (status check hard-blocks closed RDRs).
- 2026-04-11 (third iteration) — **Gate Layer 3 post-fix cleanup**. Formal `/conexus:rdr-gate 069` run on the committed second-iteration document returned 0 Critical + 3 Significant (all prose-level inconsistencies that didn't propagate through the second-iteration rename): (a) two "Phase 5" references in §Validation renamed to "Phase 4" (the rename from the second iteration didn't propagate to Validation section prose); (b) §Decision Rationale and §Trade-offs Consequences latency claims updated from the pre-measurement `~20-60s` to the measured `~100-220s` / median 111s to match CA-3's verified range; (c) CA-3 problem statement threshold updated from `~20-60 seconds` to `~60-220 seconds` to match its own verification status. Gate result stored at `nexus_rdr/069-gate-latest` (id 723): **PASSED** (0 Critical after these fixes land). Bead: nexus-sia.
- 2026-04-11 (second iteration) — **Critic-driven fixes** from the RDR-069 CA-1 meta-test spike (`nexus_rdr/069-research-4-ca1-meta-test-rdr069`, T2 id 721). Two runs of `nx:substantive-critic` against RDR-069 itself (meta-recursive test) found 2 stable Critical issues + multiple Significant issues + 1 stable honesty observation. Fixes applied in this iteration: (a) canonical Verdict block format set in §Technical Design (5-field sub-bullet structure) with explicit fallback parse rule; (b) preamble-vs-skill dispatch contradiction removed from §Technical Design (only Step 1.75 skill-body integration remains); (c) Implementation Plan phases reordered — Phase 2 now bundles the `--force-implemented` preamble extension atomically with the Step 1.75 gate (prevents shipping a blocking gate with no escape hatch); (d) `--force` regex collision fix explicitly required in Phase 2 (existing `r'--force'` regex would match `--force-implemented` as prefix, silently setting `force=True`); (e) Phase numbering updated — old Phase 3 merged into Phase 2, old Phase 4 plugin release becomes Phase 3, old Phase 5 recursive validation becomes Phase 4 with sub-labels renamed `6a/b/c → 4a/4b/4c`; (f) Performance Expectations updated from ~20-60s to measured median ~122s, range 95-217s, n=6 runs; (g) §Risks and Mitigations extended with dispatch isolation risk, finding-level non-determinism risk, and `--force` regex collision mitigation; (h) §Finding 1 table rewritten to honestly distinguish 2 empirical catches (RDR-073, RDR-075 via `/conexus:substantive-critique` skill) from 2 retrospective attributions (RDR-036 via a different 4-agent critique pattern; RDR-031 via user manual catch); (i) CA-1 and CA-3 dispositions upgraded to VERIFIED with n=3 targets, 6 runs. Bead: nexus-sia.
