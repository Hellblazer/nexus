...
The centroid labels generated by this RDR's pipeline are the community-partition substrate consumed by [RDR-098](rdr-098-abstract-question-plan-template.md)'s `abstract-themes` plan template. The plan's `groupby` step partitions search results by centroid label (`key: topic`), so the GraphRAG-style community step in the CheapRAG pattern (Zhou et al. VLDB 2025, `knowledge__hybridrag` tumbler 1.653.79, finding L4) is free: no community-report generation is needed at indexing time because BERTopic discovery already produced the topology. RDR-098 §RF-2 documents the empirical verification path for this assumption.
