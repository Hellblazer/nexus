...
Cached as a flat file at `~/.config/nexus/context_l1.txt` (not T2, because opening T2Database takes 1.7s due to 4 SQLite connections + migrations). The hook reads this file in <1ms. The file is regenerated by `nx taxonomy discover`, `nx index repo`, and `nx context refresh`.

### RF-072-4: Measured performance (2026-04-13)
