...
- **SubagentStart hook** (`nx/hooks/scripts/subagent-start.sh`) — for the ten retrieval-shaped agents (canonical list in `nx/retrieval-agents.txt`: strategic-planner, architect-planner, code-review-expert, substantive-critic, deep-analyst, deep-research-synthesizer, debugger, plan-auditor, codebase-deep-analyzer, query-planner), inject a "plan-match-first" preamble: *before decomposing any retrieval task, call `plan_match(query, min_confidence=0.85)`; execute the returned plan if match confidence clears the threshold.*
...
- **SC-7** — Session-start hook injects a "## Plan Library" block listing `plan_match`, `plan_save`, `plan_search`, and the five scenario names. SubagentStart hook injects the plan-match-first preamble for the ten retrieval-shaped agents registered in `nx/retrieval-agents.txt` (single source of truth; hook and test both read from it). Each agent's `nx/agents/<name>.md` cites the pattern independently (verifiable by grep).
...
- **2026-04-15** — Revision 8: gate-fix pass against `nexus_rdr/078-critique-gate` findings. Fixes 4 critical (C-1..C-4) and 5 significant (S-1..S-5) issues, all code-vs-design mismatches: (C-1) multi-seed `traverse` wrapper `Catalog.graph_many()` explicitly spec'd with node/edge dedup invariants; (C-2) `description` concept persisted via existing `plans.query` column — no schema rename; (C-3) SessionStart SQL replaced with correct `julianday()`-based TTL filter matching `plan_library.py:195-197`; (C-4) identity-dedup key made concrete via new `dimensions TEXT` column + UNIQUE `(project, dimensions)` index, canonical-JSON serialisation, `ON CONFLICT DO UPDATE` reseeding; (S-1) `Match.from_plan_row()` constructor spec'd for FTS5 fallback with `confidence=None` sentinel; (S-2) `$stepN.<field>` output contract resolved as design — retrieval steps emit `{tumblers, ids, distances}`, operators emit `{text, citations}`; (S-3) SC-10 now enforced at runtime via `PlanRunEmbeddingDomainError` + typed `traverse` signature + unit test; (S-4) `plan_run` execution model declared deterministic (no agent dispatch); (S-5) SC-1/SC-8 thresholds reference PQ-2 calibration rather than hardcoded numbers. Observations (O-3..O-5) addressed: references split correctly across RDR-010 + RDR-041 for T1 server lineage; `purpose:all-implementations` semantics clarified with `purposes_resolve` warn-and-drop on unknown types; `verb:debug` flat-scenario framing made definitive.
- **2026-04-15** — Revision 9 (this): post-ship honest-status annotation following substantive critique "Potemkin village" assessment. The infrastructure delivered here is real and tested (plan_match, plan_run, Catalog.graph_many, traverse, 9 YAML seeds, schema validator, 4-tier loader, hooks, skills, CI). The executable surface is narrower than SC-1..SC-19 imply: (a) core MCP tools return str, runner expects dict — 7/9 seeds fail at step 2 today; (b) the five operators (extract/rank/compare/generate/summarize) are not MCP tools, so 6/9 seeds hit `PlanRunToolNotFoundError` immediately; (c) `min_confidence=0.85` uncalibrated (PQ-2 deferred); (d) plan promotion lifecycle absent. Post-ship tests all inject dispatcher stubs — production execution was never exercised, and the str/dict bug survived weeks until code review. **RDR-078 status remains `accepted` because the infrastructure is correct and the gaps are scoped deferrals, not defects. End-to-end plan execution is deferred to RDR-079, which closes Gaps A–D with a `claude` streaming-RPC operator pool (`nexus-operators` MCP server) plus the tool-output contract fix.** This annotation is the canonical honest-status statement; SC-1..SC-19 as originally written describe the infrastructure contract, not end-to-end usability.
