...
| E | `extras` → fixed-column promotion mechanic: `promote_extras_field()` + `nx enrich aspects-promote-field` CLI + audit log | Shipped, 16 contract tests. **Runtime verb RETIRED 2026-07-25 (nexus-70x7y)**: it ran client-side `ALTER TABLE`, which the ALL-DDL-through-Liquibase directive forbids in every mode. Promotion is now one Liquibase changeset (ADD COLUMN + backfill + audit insert); the audit log and its `--history` read path survive on both substrates |
