...
gap_closures:
  Gap1: conexus/plans/builtin/hybrid-factual-lookup.yml:48
  Gap2: conexus/plans/builtin/hybrid-factual-lookup.yml:103
...
The plan exposes two optional bindings, `vector_budget_chunks` (default 6) and `graph_budget_chunks` (default 4), giving a 60/40 split per RF-6. The plan uses two `store_get_many` calls — one per stream — each with a scalar `limit_per_source` arg supplied by the corresponding budget binding. This requires a small extension to `store_get_many` (the `limit_per_source` arg) since the runner's binding-substitution language only resolves whole-token `$var` and `$stepN.field` references and would not interpret a Python-style `$step1.ids[:vector_budget_chunks]` slice expression — that text would pass through as a literal bogus ID. Bead `nexus-uwkw` ships the `store_get_many` extension as P1.0, a hard prerequisite for P1.1's plan YAML. Honesty addendum: **earlier drafts of this RDR claimed the YAML-slice convention worked; the planning-chain audit caught that it doesn't**, and the audit's own fallback ("scalar `limit` arg on store_get_many") also doesn't exist yet — the arg has to ship first. Pulling `nexus-uwkw` into P1 keeps Gap 4 addressed in this single phase rather than deferring all of Gap 4 to a future phase.
...
- **P1.0 — `store_get_many` per-stream `limit_per_source` arg (bead `nexus-uwkw`).** Add a `limit_per_source` parameter to `store_get_many` at `src/nexus/mcp/core.py:1006`. When `ids` is shaped as parallel lists (one list per source) the param truncates each list to its corresponding limit before hydration. Symmetric with the existing list-shaped `ids`/`collections` contract. **Hard prerequisite for P1.1**: without this arg there is no expressible per-stream budget mechanism (planning-chain audit confirmed neither YAML slice expressions nor a hypothetical existing `limit` arg on `store_get_many` would have worked). Test: `tests/test_plan_runner.py` parametrized case for two-stream truncation.
- **P1.1 — `hybrid-factual-lookup` plan YAML (depends on P1.0).** Create `conexus/plans/builtin/hybrid-factual-lookup.yml` with the five-step shape (search → traverse → store_get_many[vector] → store_get_many[graph] → operator_rank → operator_generate; or four-step if both hydrations bundle) parameterized by seed-or-question, depth, link-type purpose, and the two stream budgets via `limit_per_source`. Reuse `research-default.yml` and `citation-traversal.yml` as structural references. Run `nx catalog setup` against a clean DB to verify it loads without YAML errors.
- **P1.2 — `traverse-then-generate` plan YAML (depends on P1.0, P1.3).** Create `conexus/plans/builtin/traverse-then-generate.yml`, a three-step variant (traverse → store_get_many → operator_generate) for the explicit-seeds path. Uses `limit_per_source` to apply the graph_budget_chunks cap. Same YAML conventions as P1.1.
- **P1.3 — `factual-evidence` purpose.** Add a new purpose alias to `conexus/plans/purposes.yml` (the YAML registry — that's the source of truth; the `.py` file is a pure resolver) mapping `factual-evidence` → `["cites", "implements", "relates"]`, so both plans reference link types by stable name. Test via `nexus.plans.purposes.resolve_purpose("factual-evidence")`. *P1.5 validation: if `implements` fires zero times across the 5 test fixtures on `knowledge__hybridrag`, drop it from the alias before the PR lands. The `knowledge__*` corpus has sparse `implements` links by construction (those are usually code↔RDR pairs), so this is a real possibility.*
...
- `conexus/plans/builtin/citation-traversal.yml`, `conexus/plans/builtin/research-default.yml` — structural references for the new plan YAMLs.
