...
**RF-101-2 (Verified)**: Event log serialization is JSONL with **`{type, v, payload, ts}` envelope**. Existing JSONL records in `owners.jsonl`, `documents.jsonl`, and `links.jsonl` are projected as `v: 0` events of the matching type (the pre-greenfield sentinel where the raw legacy dict is the payload). New writes emit `v: 1` with canonicalized payloads. Projector dispatches on `(type, v)` pairs; unknown pairs log a structured warning and skip. One-pass replay; no dual-format runtime. Verified via `tumbler.py:127-265`: today's readers are typed only by file location and use `_filter_fields` for forward-compat; the one inline rename patch (`tumbler.py:254-255`, `created` → `created_at`) shows the maintenance cost of having no version dispatch.

**RF-101-3 (Verified)**: `DocumentDeleted` is a **tombstone event only**; T3 chunks remain in place. A separate `nx t3 gc --collection X --orphan-window 30d` verb deletes chunks whose `doc_id` no longer maps to a live document and whose `indexed_at` predates the window. Default window 30 days. `chash:` spans in surviving links continue to resolve via T3 directly during the window (RDR-053 stable-across-reindexing intent). Today's `T3Database.delete_by_source` (`db/t3.py:989-1002`) is the per-source primitive to extend. Verified that the existing `delete_document` path at `catalog.py:1402-1434` already does NOT touch T3 chunks; the comment at line 1405 explicitly documents the orphan-link convention.
...
**RF-101-5 (Probable)**: Deprecation window is **6 months minimum, measured in wall-clock time** not version count. Recent release cadence is 1.4 releases/day; counting in version numbers would compress the window to 1-2 weeks, too short for plugin authors to discover and migrate. Phase 4 ships plugin-marketplace telemetry (`direct_t3_metadata_read_total` vs `catalog_projection_read_total` counters) before deprecation warnings are operationally meaningful; without this, the flip-default decision is flying blind. Each phase ships independently; stalling at Phase 3 (dual-write) is an acceptable steady state with no regression vs today. Phase 5 default-on (4.24.x) is the only irreversible commitment, gated on telemetry confirming zero direct-T3-metadata reads in the target field set.
