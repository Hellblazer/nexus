...
- **Stale-detection where-filters (4 sites)** that already have doc_id-first fallback shape: `doc_indexer.py:109`, `indexer_utils.py:208`, `indexer.py:653,1098`. These continue to work; the source_path fallback branch becomes dead code post-prune-cycle. **Phase 5b CLOSED (2026-08-05)**: `doc_indexer.py`/`pipeline_stages.py` sites deleted by nexus-tbkk1; the sibling `indexer.py` (`_prune_misclassified_in_collection`'s legacy prune loop, `_run_index_frecency_only`'s legacy where-filter) and `indexer_utils.py` (`check_staleness`'s cached `StalenessCache.by_source_path` index and uncached where-filter) sites deleted by nexus-afudo, both backed by live-store existence probes (field>=! boundary-value test) finding zero `source_path` rows across 13 representative collections (~115k chunks). All four RDR-102-D2-named dead-code sites are now closed.
