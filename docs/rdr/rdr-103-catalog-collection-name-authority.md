...
- ~~Operator UX for the upgrade message: stderr line per collection is fine for ~10 collections. Operators with hundreds of collections (the current state for the project) may want a single summary line with a count plus a `--quiet` flag.~~ **Resolved 2026-05-04 (`nexus-yqnr.9`):** the migration loop iterates over `_MIGRATION_CONTENT_TYPES = ("code", "docs", "rdr")` so a single `nx index repo <path>` invocation emits at most 3 `Upgraded legacy collection X to Y.` lines, then 0 thereafter (idempotent). The "hundreds of collections" concern is the cumulative T3 store across many repos, not the per-invocation message count. Status quo kept; no `--quiet` flag added. Per-collection lines stay because they tell the operator which legacy names were renamed (information they care about during a one-shot upgrade).
