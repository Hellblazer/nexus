...
| `SubagentStop` | `agent_id`, `agent_type`, `effort: { level }`, `stop_hook_active` (bool), `agent_transcript_path`, `last_assistant_message`, `permission_mode` — verified 2026-05-14 (T2 `nexus_rdr/111-research-CA-6-payloads-SubagentStop`) | `{"hookSpecificOutput": {"decision": {"behavior": "allow"\|"stop"}}}` [inferred] |
...
| `PreCompact` | `trigger` ("manual"\|"auto"), `custom_instructions` (nullable; slot for `/compact <prompt>`) — verified 2026-05-14 (T2 `nexus_rdr/111-research-CA-6-payloads-PreCompact`). No `compact_summary`/`estimated_tokens`/`token_limit` in CC 2.1.x; original RF-1 inference was incorrect | Output ignored (`hooks.json` has empty `[]` for this type — not currently used) |
| `UserPromptSubmit` | `prompt` (the user's message text), `permission_mode` — verified 2026-05-14 (T2 `nexus_rdr/111-research-CA-6-payloads-UserPromptSubmit`) | `{"hookSpecificOutput": {"decision": {"behavior": "allow"\|"stop"}}}` [inferred] |
| `Notification` | `message`, `notification_type` — verified 2026-05-14 (T2 `nexus_rdr/111-research-CA-6-payloads-Notification`). Reliable trigger is **idle-wait** (~60-90s after agent turn), NOT permission prompts: CC 2.1.x auto-allows safe tools so the prompt path doesn't fire | Output ignored (informational only) |
...
| CA-6 | `SubagentStop`, `UserPromptSubmit`, `PreCompact`, `Notification` payload shapes match inferred schemas | **Resolved 2026-05-14** — all four captured via `tests/cc-validation/run-ca6-spike.sh`. Three matched/extended RF-1 inference; `PreCompact`'s inferred fields were wrong (CC 2.1.x emits `trigger` + `custom_instructions`, not `compact_summary`/`estimated_tokens`/`token_limit`). `SubagentStop` is significantly richer than inferred (`agent_id`, `agent_type`, `effort.level`, `last_assistant_message`). Per-type T2 memos: `nexus_rdr/111-research-CA-6-payloads-{type}`. | n/a — resolved | n/a — resolved |
