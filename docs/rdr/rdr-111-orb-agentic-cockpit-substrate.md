...
created: 2026-05-11
related_issues: []
related_rdrs: [RDR-105, RDR-110, RDR-112]
related_tests: []
...
| 2026-05-12 | Hal Hildebrand | Added forward dependency on RDR-112 (server-based coordination tiers). The cockpit panels and `_BindingWatcher` currently lean on direct SQLite file access to `tuples.db`; cross-host visibility — required for Cowork-VM sessions to see host hook events — needs RDR-112's sqld/libSQL transport. Single-host single-user installs are unaffected by this dependency in the short term, but Cowork as a deployment target requires RDR-112 to ship first. No structural change to RDR-111 in this revision; the SQL-bypass patterns will be revisited when RDR-112 specifies the Hrana raw-SQL surface. |
