...
- **RDR frontmatter generators** — `/conexus:rdr-create` skill builds YAML+JSON; the writing scripts in `nx/hooks/scripts/` are tolerant but produce silent malformed entries when LLM output is close-but-wrong.
