...
> **Amendment 2026-08-07 (nexus-37jha).** The retained `instructions`-channel banner implementation described above was subsequently DELETED: its sole producer (`mcp/_first_run.ensure_installed_and_running`, the T2-daemon autostart path) was removed at nexus-i711w Stage 2 sub-stage B, leaving both banner call sites no-ops on every boot. The banner subsystem and its tests are gone; first-run ownership lives in `nx init` (RDR-174). The §3 uninstall purpose remains served exactly as this Resolution records: the `daemon_uninstall` MCP tool plus `docs/desktop-deployment.md`. Marker-file utilities survive for `uninstall_daemon`.
...
> **Correction (2026-09-23, nexus-98cy4).** The shipped manifest's `entry_point` is `src/bootstrap.py`, not `src/server.py`. This is the RDR line that drifted, not the implementation: `src/bootstrap.py` was added by nexus-r433b as a resolve-with-retry wrapper for the PyPI propagation-window failure class (a fresh release's pin is unresolvable for ~10-25 minutes after publish), and it `exec`s `src/server.py` — the file this item names — once `uv sync` succeeds. `src/server.py` still exists and is still the real MCP entry point; it is just no longer the direct `mcp_config` target.
