...
- [x] **A1**: catalog carries every fact a consumer reads from `repos.json` except `status`. — **Status**: **Verified-with-gaps** — **Method**: Source Search.
  - Verdict: every collection-name + path field has a direct catalog equivalent. Single gap: `head_hash` (per-repo, used by indexer staleness skip) has no per-repo catalog home. **Locked mitigation (gate critique 2026-05-28)**: option (a) add `owners.head_hash` column. Option (b) "switch to `documents.source_mtime`" is NOT an equivalent substitute — a repo HEAD can advance without any tracked file's mtime changing (remote-only merge with no file changes, ff-only pull of tag-only commits, merge with all changes in submodule / untracked dir). The `source_mtime` path would silently pass staleness in those cases. (a) is a one-column addition + a single migration step in Phase 1.5; small cost preserves exact semantics. (b) is only acceptable as a documented downgrade if the schema change is vetoed, and the staleness regression class must be named explicitly when it is.
...
- [x] **A4**: Worktree-vs-main-repo dual registration is unintentional; collapse is acceptable. — **Status**: **Refuted-productively** — **Method**: Source Search (wave 2, 2026-05-28).
  - Verdict: dual-registration is **accidental**, not architectural — and the current code already deduplicates worktrees via `_repo_identity()` + `git rev-parse --git-common-dir` (registry.py: `_resolve_repo_collection` → `_repo_identity`; catalog.py: `ensure_owner_for_repo` lines 995-1024). Both paths derive the same `(basename, hash8)` tuple for any worktree of the same logical repo. Live owner `1.22` (qmrr-zm2n) is a **pre-RDR-103 historical artifact** from when callers used raw worktree-path hashes for `repo_hash`; a new indexing run from either path now resolves to owner `1.1` (nexus).
  - **Gate critique 2026-05-28 finding**: the dedup is correct for `repo_hash` BUT `ensure_owner_for_repo` (catalog.py:1022) and `_catalog_hook` (catalog.py:640) currently write `repo_root=str(repo)` — the raw input path — NOT the `main_repo` that `_repo_identity` computes internally. First-run from a worktree therefore leaves `repo_root` pointing at a transient worktree directory; after worktree deletion, `resolve_path` produces broken paths for all relative-path documents.
  - **Phase 4 sub-bead required**: change `_repo_identity` return signature from `(name, hash8)` to `(name, hash8, main_repo_path)` and thread `main_repo_path` through `ensure_owner_for_repo` + `_catalog_hook` to write `repo_root=main_repo_path` instead of `str(repo)`. Then the orphan-owner cleanup step Phase 4 already specifies sweeps the existing wrong-`repo_root` rows.
...
5. ~~Where does the `--corpus knowledge` routing preference live post-migration?~~ **Resolved 2026-05-28 (gate critique)**: option (b) infer from the existing collection-name prefix at read time. Lowest friction — the catalog already has `knowledge__*` vs `docs__*` as the physical collection name for a repo whenever the user opted in, so the reader can derive the preference without any schema change. **Phase 4 gates on this**: writer replacement cannot land until the catalog-backed reader implements the prefix-aware inference and a fixture-level test exercises `nx index repo . --corpus knowledge` end-to-end through the new path.
...
7. (**Gate critique 2026-05-28**) When does Phase 5's "doctor warns if `repos.json` still exists" upgrade-warning consume-and-delete vs warn-only? A user with a stale config copied from another machine should not have their file silently deleted on first `nx doctor`.
8. (**Gate critique 2026-05-28**) Phase 5 migration trigger needs naming: `nx upgrade` step, first `nx index repo` post-upgrade, or a dedicated `nx catalog migrate-repos` verb. The success criterion currently says "first session start following the migration" but SessionStart is not a migration trigger today.
9. (**Gate critique 2026-05-28**) The Phase 1 test-fixture inventory (A3 "noise") should partition the 20+ `RepoRegistry(tmp_path / "repos.json")` test files into (a) status-lifecycle tests that disappear with the registry and (b) path/collection-name parity tests that need rewriting against the catalog API. Output this partition before Phase 3 so test-migration scope is concrete.
10. (**Gate critique 2026-05-28**) DEBUG→WARN graduation in Phase 2 needs a mechanical threshold rather than editorial judgment: e.g., "24 consecutive hours with zero fallback fires in `mcp.log`" makes the promotion automated. Pick a threshold during Phase 2 implementation.
11. (**Gate critique 2026-05-28**) Phase 3 cutover completion cannot land until the `RepoRegistry` import is removed from the `catalog_docs.py:476-484` exemplar itself — otherwise Phase 5's lint guard fires against its own template. Explicit ordering: rewrite `resolve_path`'s fallback to use a non-RepoRegistry mechanism (or remove the fallback once Phase 1.5 backfill is universal) before declaring Phase 3 done.
