...
- [ ] **CA-1: Note-backed aspects are genuinely unrecoverable by any existing key** (not just the two join paths tried). — **Status**: Unverified — **Method**: Source Search (confirm no `source_uri`/title path can resolve a note aspect to a catalog tumbler, e.g. via T3 chunk metadata or a title join).
- [ ] **CA-2: Dropping unmapped aspects is non-destructive given regeneration** — `nx enrich aspects <collection>` re-extracts dropped aspects for live docs. — **Status**: Unverified — **Method**: Spike.
- [ ] **CA-3: The ongoing orphan stream originates at the aspect-write path for note-backed docs** (no doc_id resolvable at write time), not at a later step. — **Status**: Unverified — **Method**: Source Search (`aspect_worker.py` doc_id resolution).
- [ ] **CA-4: Tightening the aspect surface to exclude note collections (or only paper-like docs) does not regress intended RDR-089 coverage** for real paper collections (`dt-papers`, `augur-oracle-papers`, etc.). — **Status**: Unverified — **Method**: Source Search.
