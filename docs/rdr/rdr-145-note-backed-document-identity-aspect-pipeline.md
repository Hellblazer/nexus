...
- [x] **CA-5: RDR-172 (landed 2026-06-27) closes Gap-1's ongoing-orphan stream for the `store_put` path.** — **Status**: VERIFIED — **Method**: Source Search (2026-06-27). RDR-172 P1.1 (`nexus-pyn35`) changed `store_put` to forward `catalog_doc_id` — the real catalog tumbler returned by `catalog_store_hook` (`store_hook.py:85-90`, which registers note-backed docs with a `documents.tumbler` and returns it) — as `fire_document`'s `doc_id` kwarg (`mcp/core.py:2068`), instead of the chash. So **new** note aspects via `store_put` now carry a mappable tumbler `doc_id` at write time (no backfill needed; FK-satisfied in service mode). This is exactly root-cause fix **(A)** from the prior design-impact note, realized via the catalog tumbler rather than the title resolver. CA-3's "chash-as-source_path bypasses the resolver" defect is moot for `doc_id` (source_path stays the chash but is not the mapped key; the PK migration switches PK to `doc_id`, now populated).
...
1. **Gap 1 — note identity at write time: CLOSED by RDR-172 (no work here).** `store_put` now forwards the catalog tumbler as the aspect `doc_id` (CA-5), so new note aspects are mappable + FK-satisfied. RDR-145 inherits this; it does not re-implement it.
2. **Gap 3 — surface eligibility: DECIDED "shape-aware routing", already shipped by `nexus-kmbys` (CA-6).** Note-shaped `knowledge__` docs route to `general-prose-v1`, paper-shaped to `scholarly-paper-v1`. RDR-145's only Gap-3 work is a **verification test** that a representative `knowledge__knowledge` note routes to `general-prose-v1` (not the paper extractor) and lands a mappable, non-fabricated aspect — locking the decision against regression. (Cost lever "skip note extraction entirely" is explicitly deferred — not adopted; the shipped light extractor is the chosen behavior.)
3. **Gap 2 — absolute-path `source_path` contamination: BUILD.** Normalize `source_path` to the catalog's canonical/relative form at the aspect-write path (the `nexus-3e4s` normalization class) so file-backed aspects stop landing sibling-clone/absolute paths that miss the catalog join.
4. **One-time orphan cleanup: BUILD.** The ~296 pre-fix orphans (122 note-identity + 177 absolute-path) still block the RDR-108 Phase-1c PK migration. They are unmappable by any key and aspects are regenerable (CA-2), so hard-delete the unmapped orphans during the migration — aligned with RDR-108's documented intent — then let the now-correct write path (RDR-172 + kmbys) and `nx enrich aspects` repopulate live collections.
