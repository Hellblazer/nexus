...
Entity-record classification is the decisive design choice. Options (decide
...
**Hop-1 precision is on the critical path, not deferred.** The benchmark proved that
...
- **Risk**: The recursion finding: even perfect resolution doesn't close
  ...
  **Mitigation**: the hop-1 precision step (`operator_filter`, existing operator)
  is IN SCOPE in the plan template (Step 4), not deferred; it is the decisive
...
move. The `operator_filter` precision step is the decisive addition; without
...
operationalizes the joint dependency: shipping P2 alone flips only
