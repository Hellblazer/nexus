...
- 2026-09-24: **"PID-reuse immunity. No PID in the identity or liveness path"** (Decision, above) now carries one bounded exception. Sam DECIDED (bead nexus-wo6sc, `service_registry.py`'s `ServiceRegistry.discover()`) that a `storage_service` lease past its own TTL is still read as live when the reader can independently confirm it: the recorded `payload["supervisor_pid"]` alive AND the recorded port answering `/health` as the expected service — only when both fail does the reader treat it as down. This does not restore PID as the identity/liveness path in general: `discover()`'s DEFAULT for every tier stays lease-freshness-only (pid-reuse immunity fully intact), the exception is scoped to `TIER_READER_GRACE` (`storage_service` only — `aspect_worker` publishes no HTTP endpoint to probe, so the mechanism cannot fire there regardless), it only ever WIDENS a freshness miss into a hit (a fresh lease is never narrowed), and it is bounded by `STALE_LEASE_GRACE_MAX_TTL_MULTIPLE` (10× the lease's own TTL) so a lease cannot be resurrected indefinitely. Motivation: the 2026-09-12 heartbeat-stall incident, where the supervisor was alive and healthy throughout but two heartbeat ticks (19.098s / 31.622s) overran a 15s TTL, and every client read the endpoint as absent until the next stamp. See `src/nexus/daemon/AGENTS.md`'s "Third documented exception" for the mechanism's full guards.
