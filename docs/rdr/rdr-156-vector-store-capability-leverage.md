...
related_tests: []
related: [RDR-152, RDR-153, RDR-154, RDR-155, RDR-106, RDR-108, RDR-191]
decision_2_superseded_by: RDR-191
---
...
RDR-106 (draft) documented the near-miss: `prune-stale` about to drop 11,766 valid catalog entries on a classification bug, averted by operator intuition, band-aided with backup files + an undelete verb. The structural gap stands: there is no in-database trashed state, a delete fires FK cascades through the manifest/aspects/highlights chains, and recovery is file restore or full re-index. The SQLite/Chroma split made tombstones awkward (RDR-107 was superseded rather than shipped); Postgres makes them nearly declarative. *(Historical framing, 2026-08-18 cross-walk: the SQLite/Chroma split described here no longer exists — SQLite retired by RDR-152/RDR-158, Chroma by RDR-155 P4b. Gap 3 itself closed: Decision 6's soft delete shipped as catalog-003.)*
...
The deeper goal is not integrity hygiene. It is that co-residence makes **combined queries** possible: retrieval shapes that today require multi-store round-trips with application-side stitching collapse into single SQL statements the planner can optimize. The current `query` MCP tool's catalog-aware routing is the canonical example of the cost being paid today: catalog metadata lookup (SQLite), collection resolution, per-collection vector query (Chroma), then app-side re-join, interleave, and re-rank. *(Historical framing, 2026-08-18 cross-walk: SQLite and Chroma are both retired now — SQLite by RDR-152/RDR-158, Chroma by RDR-155 P4b — this describes the pre-Decision-5 starting point, not current architecture; see the Decision 5 annotation below for what actually shipped.)* Each of these becomes one statement:
...
- `catalog_collections` (the RDR-103 collection-name authority, with `embedding_model`) is **referenced by nothing**: not by `chunks_<dim>.collection`, not by `chash_index.physical_collection`, not by `topic_assignments.source_collection`. *(True state, 2026-08-18 cross-walk: this bullet and the one above are 2026-06-10 point-in-time evidence, not current schema — the FKs shipped (fk-002/Decision 1), and the per-dim `chunks_<dim>` tables themselves were later retired by RDR-191's unification into `nexus.chunks`.)*
...
- The production migration's cutover validation ran against **empty catalog tables** (the RDR-153 SQLite→Postgres data migration has not run), making the manifest-orphan check vacuous — evidence that integrity checks living outside the schema degrade silently. *(True state, 2026-08-18 cross-walk: the RDR-153 migration ran long ago and `chash_index` itself was later dropped by rdr187-001; the manifest-orphan check is now largely superseded by RDR-191's manifest FK — see the Decision 2/3 annotations.)*
...
1. **Schema-enforce the collection registry.** Add FKs, contingent on a unique key over `catalog_collections(tenant_id, name)`:
   ...
   > **True state (2026-08-18 cross-walk):** shipped (fk-002), but the per-dim FKs died with the vectors-004 `DROP CASCADE` when RDR-191 unified the chunk tables. Successor FK ships as fk-004 (RDR-191 P5). The `chash_index` FK is moot — rdr187-001 dropped that table outright. The FK spine was extended by RDR-164 (fk-003). RDR-194 F9 confirms zero `NOT VALID` stragglers remain.
...
2. **Do NOT FK the manifest to the chunk tables.** `catalog_document_chunks.chash` cannot directly reference chunks split across three dim tables. The trigger-maintained `chunks_registry` parent-table alternative is rejected today (see Alternatives): it adds a trigger outside RDR-154's "app-unfixable only" bar and imposes chunk-before-manifest write ordering on the hot indexing path. Instead, promote the orphan check to the database as a stored function (below) and keep the P2.1 fail-loud read backstop.
   ...
   > **SUPERSEDED (2026-08-18 cross-walk):** RDR-191 supersedes this decision (frontmatter `decision_2_superseded_by: RDR-191`). Splitting chunks across three dim tables is no longer true — RDR-191 unified them into one `nexus.chunks` table, which makes the manifest FK expressible. The FK deployed with engine v0.1.76; dangling count is 0 (catalog-030).
...
3. **Specialized functions** (replacing generated-SQL-string artifacts and hand-assembled reconstruction):
   ...
   - `nexus.hybrid_search(...)` — server-side RRF fusion over HNSW + tsvector (+ trigram) in one round-trip. **No longer interlocked (2026-08-18 cross-walk):** the conexus xr7.8.9 production-scale recall + hybrid-parity gate PASSED and CLOSED 2026-06-12 (T2 `conexus/result-xr789-gate-PASSED`); the re-pointed cutover dependency completed with the local→managed cutover. Hal lifted the standing block 2026-08-18 — an owner call; no formal cutover-window-closed artifact exists nexus-side, stated as such rather than implied. P5 is now ordinary ready work. **Narrowed-value caveat:** nexus-lcogi already shipped a standalone selectivity-aware two-phase fix hitting the target recall (6/6, <2s) without full RRF fusion, so the go/no-go (word_similarity 0.6 cross-check included) is still real but has moved from P5's entry to its exit — P5.G must measure the round-trip win of server-side fusion against lcogi's fix before proceeding.

   > **True state (2026-08-18 cross-walk):** `manifest_orphans`/`manifest_backfill` shipped, then RETIRED by catalog-030 once the manifest FK (RDR-191) made the orphan check structural rather than a callable. `document_text` is live (vectors-005, bead .726). `hybrid_search` is open — see the annotation above and the P5 rewrite in §Approach.
   >
   > **True state (P5.2 as-built, 2026-08-18):** `nexus.hybrid_search_<dim>` shipped (vectors-007) with a deliberate deviation from this bullet's "over HNSW" wording: the vector leg is computed EXACTLY over the text-gated candidate set (rank by `<=>` within a materialized gate CTE), never through an HNSW index probe — for the selective gates this function exists to serve (the lcogi regression class), exact-over-gate is the correct shape and HNSW cannot starve it; `HybridSelectiveGateTest` establishes the same precedent Java-side, and the suite's EXPLAIN test asserts the no-HNSW plan deliberately. Consequence: unlike lcogi, the function has NO dense-gate escape valve (`SELECTIVE_GATE_MAX`-style HNSW-first dispatch) — a very large gate pays three full window-function sorts. That cost axis was measured by P5.G's 100k-chunk run (2026-08-18, T2 `nexus_rdr/156-P5-gate-latest`): the fn/java ratio grows with gate size — 0.97x at gate=6, 2.57x at 1830, 6.50x at 25,000 (683ms vs 105ms) — while the selective/lcogi case holds full parity (6/6 in 100k, fn 638ms, marginally faster than java). **P5.G verdict: GO with the dense-gate tradeoff tracked as nexus-76352** (in-function dispatch vs accepted-tradeoff note); until it resolves, dense-workload callers belong on the java lcogi path.
...
4. **Views, under the RDR-154 `security_invoker` standing rule:** `collection_vector_stats` (per-collection chunk count, dim, last write — replaces remote count calls in doctor/status and the migration runbook's hand psql).
   ...
   > **True state (2026-08-18 cross-walk):** shipped and live (catalog-005) — `collection_vector_stats` is consumed by `doctor.py` and `collection_state.py`.
...
5. **Combined-query read shapes as the unification deliverable.** Define the canonical cross-store retrieval shapes (metadata-scoped search, topic-scoped search, aspect-filtered search, graph-hop + rank, frecency-boosted rank) as `security_invoker` views or set-returning functions, and progressively repoint the Python `query`/search composition layers at them, deleting the app-side stitching they replace. Each shape lands with an EXPLAIN-verified plan (the HNSW scan must survive the join — a filter that defeats the index is a regression, not a simplification) and a parity test against the stitched path it retires. **The parity fixture MUST include a narrow-collection scenario (collection size smaller than `hnsw.ef_search`, semantically distant query vector) with an exact-recall assertion (`== N`, not `>= threshold`)** — Research Finding 5b shows medium-selectivity fixtures pass while narrow-collection inputs silently under-return at the `max_scan_tuples` ceiling; the exact-count gate proves the selectivity-strategy switch actually fires (mirroring the RDR-155 P3.E recall@10 == 1.0 pattern). Stitching deletion happens in a **separate commit from the repoint**, so `git revert` is the rollback mechanism if a regression surfaces post-deletion.
   ...
   > **True state (2026-08-18 cross-walk):** three of the five combined-query shapes shipped and are live — metadata-scoped, topic-scoped, and graph-hop (catalog-006/007/008/012), retargeted onto the unified `nexus.chunks` table post-RDR-191 (vectors-005). Aspect-filtered and frecency-boosted shapes were decided but never beaded — zero children existed for them until this cross-walk; now tracked as nexus-ubnwk. The stitching-delete this decision requires (P4.2c) is open at nexus-2bqpn, FACT-blocked (not dep-blocked) on the operator-`where` disposition — see that bead's notes.
   >
   > **Disposition (2026-08-28, owner decision on nexus-ubnwk):** the two unbuilt shapes split. **Frecency-boosted rank is RETIRED from this decision's shape list** — it was never a combined query: `frecency_score` is a git-derived per-file number computed at index time (`src/nexus/frecency.py`), stamped into chunk metadata for `code__` corpora, and applied as a weighted re-rank over the top-K of a single vector call (`src/nexus/scoring.py`). There is no second store to join and no round trip to remove, and Research Finding 5 rules out a composite ORDER BY reaching the HNSW index, so a SQL form would be the same top-K-then-re-rank relocated. **Aspect-filtered search is BUILT** under nexus-ubnwk (rescoped) on the shipped template — `security_invoker` set-returning function, EXPLAIN-verified plan with the vector as argument, parity against the plan-runner `search -> operator_filter` SQL fast path it collapses, and the narrow-collection exact-recall fixture this decision requires. Its precondition is an identity fix: `document_aspects.doc_id` defaulted to `''` on every row written before nexus-x1de2 (2026-08-28, go-forward stamping only), so the join has no data behind it until the existing rows are attributed.
...
6. **Soft delete, finally done right (absorbs RDR-106's catalog-projection scope onto the PG substrate).** RDR-106 (draft, 2026-05-08) was motivated by a near-miss mass deletion (`prune-stale` about to drop 11,766 valid entries, nexus-6ims); its SQLite-era answer was backup files + an undelete verb — physical delete remained the only delete. On Postgres this becomes almost declarative:
   ...
   > **True state (2026-08-18 cross-walk):** shipped (catalog-003), but the "single enforcement point" claim two bullets above was FALSIFIED in production — nexus-3ck2g (closed 2026-08-03) found five unfiltered read sites bypassing the view/function filter, repaired by catalog-019/022/026. Treat that bullet as aspirational-then-corrected, not as-built.
...
7. **Declarative hygiene now cheap because the tables are empty** (do before RDR-153 lands data — these are free on empty tables and expensive after):
   ...
   > **True state (2026-08-18 cross-walk):** temporal typing (`created_at`/`superseded_at`) shipped. The `length(chash) = 32` CHECK constraints are moot — RDR-180 moved chash to `bytea` (32-byte octet length enforced at the type level, not a text-length CHECK). `source_uri` UNIQUE shipped via catalog-016 (nexus-78n33).
...
8. **Capability-selection discipline:** every choice above is recorded against RDR-154 P3's boundary (declarative FK > function > view > trigger-only-if-app-unfixable). The chunks_registry trigger is this RDR's recorded "NOT worth it" entry; soft delete adds zero triggers (tombstone is a plain column + partial indexes + view filters).
   ...
   > **True state (2026-08-18 cross-walk):** shipped (70r3c.19, PR #1182) — the capability-selection discipline entry lives in `src/nexus/db/AGENTS.md`.
...
5. **P4 — combined-query shapes, one at a time, evidence-led.** Start with the two highest-traffic stitches: metadata-scoped search (the `query` MCP tool's catalog dance) and topic-scoped search. Each: view/function + EXPLAIN plan check (HNSW survives the join, query vector as argument per Finding 5) + parity test vs the stitched path + repoint + delete the stitching. Aspect-filtered and graph-hop shapes follow on the same template once the first two prove the pattern.
   ...
   > **True state (2026-08-18 cross-walk):** the P4 phase-review-gate cross-walk PASSED 2026-06-14 (T2 `156-P4-gate-latest`); gate bead (70r3c.16) closure itself waits on nexus-2bqpn's disposition (see the Decision 5 annotation above). Two residues remain open: nexus-zekpl (tumbler-aware plan-runner hydration for metadata-scoped ids — documented in the tool docstring only, not yet fixed) and nexus-0zcn9 (Research Finding 5b's `max_scan_tuples` narrow-collection hazard is STILL UNDEFENDED on the shipped combined-query shapes — said plainly, not glossed over).

6. **P5 — `hybrid_search` function.** No longer world-blocked (2026-08-18 cross-walk — see the Decision 3 annotation): the conexus xr7.8.9 gate closed 2026-06-12 and Hal lifted the standing block 2026-08-18. P5 now runs as ordinary ready work in sequence `.17 -> .18 -> .20` (bead nexus-70r3c). The go/no-go is still real but now sits at the gate (.20), not the entry: MEASURE the round-trip win of server-side fusion against nexus-lcogi's already-shipped standalone selectivity-aware fix (6/6 recall, <2s) and re-justify `hybrid_search` on that measurement, including the `word_similarity` 0.6 cross-check. P5.3 (Decision 8's capability-selection rationale in `src/nexus/db/AGENTS.md`) already shipped (70r3c.19, PR #1182), independent of P5.1/P5.2.

   > **True state (2026-08-18, P5 executed):** P5.1 + P5.2 shipped on develop @ `8ed76f28e` (vectors-007 `hybrid_search_384/768/1024` + `HybridSearchFunctionParityIntegrationTest`, stacked review clean). The bead's original harness premise was false — `DualRunHarnessIntegrationTest`/`HybridParityIntegrationTest` died with the Chroma leg at RDR-155 P4b — so the suite was modeled on `CombinedQueryParityIntegrationTest`, carrying the DualRun parameters. Dense-regime parity was redefined evidence-first (fixed-K denominator was unachievable by construction; exact-set recall is the wrong bar for a deliberately-reordering fusion): gate containment + selective-subcase exact equality + oracle quality-non-regression (fn 0.9841 vs java 0.9735, never worse per-query). **P5.G PASSED 2026-08-18 — GO** (T2 `nexus_rdr/156-P5-gate-latest`): lcogi regression case 6/6 in 100k at 638ms both paths; dense-gate cost tracked as nexus-76352 (see the Decision 3 as-built annotation). Sibling tombstone-predicate sweep: nexus-2rtge.
...
4. **Parity benchmarking home** (VERIFIED, T2 `nexus_rdr/155-P3-gate-result`): extend the existing Java `DualRunHarnessIntegrationTest` / `HybridParityIntegrationTest` (RDR-155 P3.E, commit 4c6be055, `-Dnx.dualrun.*` params; baseline recall 1.0 over 20 queries k=10, p95 < 250ms) for function-vs-service-fusion parity. The production-scale go/no-go (incl. `word_similarity` 0.6 calibration cross-check) was conexus xr7.8.9. **True state (2026-08-18 cross-walk):** xr7.8.9 PASSED and CLOSED 2026-06-12; it no longer gates P5's entry. It re-surfaces at P5's exit as the re-justification bar against nexus-lcogi's standalone fix — see the Decision 3 and Approach-P5 annotations.
...
- ~~Where does `hybrid_search` parity benchmarking live~~ — RESOLVED (Finding 4): extend the P3.E Java harness; production go/no-go was conexus xr7.8.9, which PASSED and CLOSED 2026-06-12 (2026-08-18 cross-walk: no longer a P5 entry gate — it re-surfaces at P5's exit against nexus-lcogi's standalone fix; see the Decision 3 / Approach-P5 annotations).
...
- ~~RDR-106 disposition~~ — DONE (verified 2026-08-18 cross-walk against RDR-106's own frontmatter): RDR-106 is marked `status: superseded`, `superseded-by: RDR-156` (its catalog-projection tombstone scope is absorbed here; its SQLite-era backup/undelete mechanism remains shipped history; its event-sourced projector-verb scope died with events.jsonl in RDR-152 — see Decision 6 scope boundary).
