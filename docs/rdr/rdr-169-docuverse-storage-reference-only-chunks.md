...
- [x] **A reference-only chunk is still embeddable.** **Status**: Verified-feasible.
  **Method**: source spike (research-1). The precomputed-vector primitive already exists:
  ...
  `retention='reference-only'` plus the schema slice, not a blocker.
...
1. **Cross-repo lockstep: the decisive paired edit is NOT the column line.** The cutover
   ...
   chunks silently never copy. (b) is the non-obvious critical-path change.
...
4. **DTO strictly additive + nullable, no framing change** (conexus A4). The edge proxy
   ...
   through untouched. The DTO addition MUST NOT change `Content-Type` or response framing,
   though; add `chunk_text` (nullable) + `retention` to the existing response shape only.
