...
- `GET /v1/stats/service`: operator JSON with PG size totals (`pg_total_relation_size` per table), Hikari pool gauges, uptime, version/changesets, and rolled-up request metrics. Local mode: the user is the operator.
...
2. **Client-side aggregation (nx queries each store endpoint).** Rejected: that is the status quo that failed, with N round-trips, no bytes, no history, and RLS footguns when it falls back to psql.
