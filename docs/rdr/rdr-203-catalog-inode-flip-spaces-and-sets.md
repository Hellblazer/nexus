...
1. **Physical partition.** The engine picks the embedder and the vector width by splitting the name on `__` and looking the third field up in hard-coded maps (`EmbedderRouter.java:302-323`, `PgVectorRepository.java:409-426`; the Python mirror is `reconcile.py:177-183`). The chunk table's primary key is (tenant, collection name, chash). Fifteen or more tables key on the name string; five carry a validated foreign key to it.
2. **Classification.** Content type and owner ride in the first two fields. Search routing (`corpus=` on every MCP search tool, `--corpus` on the CLI) is prefix matching on the same string (`corpus.py:876-905`), and the "all" expansion exists in three non-identical copies in `mcp/core.py` (lines 1915, 2332, 3425; the third uses a raw part where the others normalise).
3. **Schema version.** The trailing `v<n>`.

Ted Nelson's rule for tumblers, which nexus adopted in RDR-049, is that an address is independent of subject and category, and that "categorizing directories" belong to the user, never to the system level. The collection name is a categorising directory at system level. The tumbler already says where a document is; the name repeats the owner in a second, lossy encoding (`1.7.42` becomes `1-7`), adds a category the catalog also holds as a column, and welds both to a storage detail the engine should own.
...
| RDR-108 (closed), RDR-180 (closed) | Precedent | Made chunk identity content-addressed and put document structure in the manifest, not in chunk metadata. This RDR is the same move one level up: document organisation in the catalog, not in a storage name. Chash and manifest are untouched. |
...
Engine: Java, Postgres 17 with pgvector 0.8.2 bundled, Liquibase changelogs under `service/src/main/resources/db/changelog/`, jOOQ generated DSL only (no SQL strings, Sam 2026-09-03). Client: Python 3.12, `src/nexus/`. Catalog tables in `catalog-001-baseline.xml`: `catalog_owners` (line 37), `catalog_documents` (62), `catalog_links` (127), `catalog_document_chunks` (171), `catalog_collections` (199, holding content_type, owner_id, embedding_model, model_version, with a tuple index at 221). Chunks in `vectors-004-unify-chunks.xml:262-276`: (tenant_id, collection, chash bytea, chunk_text, embedding_384, embedding_768, embedding_1024, chunk_tsv, metadata, created_at), primary key (tenant_id, collection, chash). Four embedding models in use: voyage-code-3 and voyage-context-3 at 1024, bge at 768, test-embed at 768. Cloud estate: one environment, 19465 live documents, 286707 chunks, 75 owners.
...
- **Verified**: the three "all" expansions in `mcp/core.py` differ (line 3440 uses the raw part). Three copies of one routing rule is the shape that drifts.
...
- [ ] Every existing collection maps to exactly one space with no split or merge. **Status**: Unverified. **Method**: Spike, a census query over `catalog_collections` grouped by (tenant, embedding_model, model_version) before the seed changeset is written.
- [ ] The chunk primary key can move from (tenant, collection, chash) to (tenant, space_id, chash) with identical uniqueness, that is, no two collections that become one space share a chash. **Status**: Unverified. **Method**: Spike, the same census, counting chash collisions across collections that collapse.
...
- [ ] No client outside this repository reads collection names off the wire. **Status**: Unverified. **Method**: Source Search of conexus and the plugin surfaces for `__` splitting.
...
**Set.** A document of content type `set`, registered under its owning curator, tumbler-addressed like any document, so it can be linked, cited, superseded, tombstoned and audited by existing machinery. Membership is a `member-of` row on `catalog_links` with an optional integer position. One document in many sets; a set in a set. `--set` on `nx store put` and `nx index pdf`; `set=` on search alongside `subtree=` and `content_type=`, combinable. Sets carry no model policy.
...
- Liquibase changesets: create `spaces`; seed one row per distinct (tenant, embedding_model, model_version) in `catalog_collections`; add nullable `space_id` to `nexus.chunks` and to each name-keyed table, populate by join through `catalog_collections`, then set NOT NULL and add the FK; add `space_id` to `catalog_collections` so the old name row points at its space. The name column stays, valid, for the whole release.
...
- **Risk**: two collections collapse into one space and share a chash, breaking the new primary key.
  **Mitigation**: the census spike before the seed changeset; a collision is an identical chunk text under two names and collapses to one row by RDR-108's own rule, so the migration dedups with a manifest rewrite, not a failure.
...
- [ ] Census spike results recorded in T2 and attached to this RDR
...
To be completed at gate; the four Critical Assumptions above each name their method.
...
To be judged at gate; the Alternatives section may shrink once the census spike lands.
