...
- Liquibase changesets: create `spaces`; backfill `catalog_collections.embedding_model` and `model_version` from the name's model segment where blank (201 of 265 live rows, census [24466]), asserting the width of that collection's chunks matches the model's dimension; seed one row per distinct (tenant, embedding_model, model_version); dedup chunk rows that share (tenant, prospective space, chash) across collapsing collections (3,369 live rows); add nullable `space_id` to `nexus.chunks` and to each name-keyed table, populate by join through `catalog_collections`, then set NOT NULL and add the FK; add `space_id` to `catalog_collections` so the old name row points at its space. The name column stays, valid, for the whole release.
