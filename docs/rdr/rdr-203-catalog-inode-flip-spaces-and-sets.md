...
- **Verified**: the three "all" expansions in `mcp/core.py` differ: the prefix logic at 3436-3440 is identical across the three sites, but the third appends the raw part (`target.append(part)`, 3439-3440) where the other two pass it through `t3_collection_name(part, t3=t3)`. Three copies of one routing rule is the shape that drifts.
