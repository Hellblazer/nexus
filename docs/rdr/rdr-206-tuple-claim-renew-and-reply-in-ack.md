...
| `TupleHandler` | Yes | Body helpers take any `Map<String,Object>`, so a nested `reply` object parses with the same code `handleOut` uses; `/renew` is one switch case; all ten typed errors and statuses already cover both operations (research-1 §3). |
...
claimed with no reply row (atomicity). The order of the two calls inside the
transaction is not pinned, because it is immaterial to the outcome. This
executes Critical Assumption 2.
