...
- 2026-09-14: Phase 1 (engine) landed: the `directory/<name>` template and `address_kind: session` on the mailbox template (nexus-galkv.1 to .5), in engine-service-v0.1.119. Close record T2 `nexus/rdr-208-phase1-close-2026-09-14`.
- 2026-09-14: Phase 2 (client) landed: `mailbox_send` resolves a name to its holder's session id at send time and refuses a name held by two live sessions; the `/clear` cleared record and one-time drain; directory leases re-sent every 60 s with a 300 s TTL (nexus-galkv.6 to .16). Shipped in conexus 7.46.0 (R2). Close record T2 `nexus/rdr-208-phase2-close-2026-09-14`.
- 2026-09-15: MVV (nexus-galkv.19). Cloud mode ran on real Claude Code sessions: steps 1 to 5 passed; step 6 (`/branch`) failed on 7.46.0 because `/branch` fires no SessionStart, was fixed in 73938b94b, and passed live on 7.46.1; step 7 passed in both directions with a second host. A fork re-arm defect found on the re-run was fixed in 08298f6db (7.47.0) and re-verified live (nexus-galkv.30). Local mode was validated by the container harness `tests/e2e/rdr208-mvv/run.sh` (42/42 on published 7.46.1), which runs stand-in claude processes over the real hooks, `mailbox_send`, drain and watcher, never a real Claude Code session. The Minimum Viable Validation's "two real sessions" therefore holds for cloud mode only.
- 2026-09-18: The Fork section records a defect in the accepted design, fixed in 9c730421f: a fork fired no hook, so the parent's waiter pushed the parent's mail into the fork. A session handoff now moves the channel waiter, and a fork counts as one. The directory-lease infrastructure row follows RDR-211's deletion of the CLI watcher.
- 2026-09-24: Phase 3 (transition) landed after the 7-day retention window. Client half (nexus-galkv.20): the drain hook and `tuple_subscribe` stop treating an instance-name mailbox as delivered; a subscribed name only arms its directory lease. Shipped in conexus 7.60.0 (R3). Engine half (nexus-galkv.24): the mailbox template refuses `address_kind: instance`, and rows already stored with it stay readable, claimable and ackable. Shipped in engine-service-v0.1.131, paired with conexus 7.61.0 (R4) and deployed armed at the client tag. Close record T2 `nexus/rdr-208-phase3-close-2026-09-24`.
