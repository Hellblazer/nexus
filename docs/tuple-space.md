...
**Across `/compact`, `/clear` and `/resume`.** A Monitor survives all three inside one Claude Code process: the harness task, its child process and event delivery all carry across; only the model's own context is replaced. `/compact` keeps the same session id, so a surviving watcher is still watching the right mailbox. `/clear` and `/resume` mint a new session id for the conversation while the old watcher keeps probing the OLD one, so `nx hook session-start` writes a marker naming the new id on both, and the watcher's own per-cycle check against that marker is what stops it, never a model-issued `TaskStop`, which cannot work across a real `/clear`: the fresh conversation running the new SessionStart arm instruction has no memory of the old Monitor's harness task id to stop. The new arm instruction re-arms into whatever address the old watcher's exit just freed. See [`nx tuple watch`](cli-reference.md#nx-tuple-watch) for the lock and self-stop mechanics.
