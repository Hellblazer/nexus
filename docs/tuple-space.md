...
**The watcher pings, never claims.** `nx tuple watch` is armed as a Monitor's `command` from a SessionStart instruction (`nexus.mailbox_arm`, emitted by `nx hook session-start` on every source, silent when the tuple surface is not answering). Arming needs a one-time `~/.claude/settings.json` permission-allowlist entry, since a Monitor's command runs under the same permission machinery as `Bash` (see the CLI reference). It probes `mailbox/<session-id>` and, when given `--instance NAME`, `mailbox/NAME` too, in one process sharing one emit budget: a second Monitor for the second address would double the ping rate against a throttle counted per Monitor. A hit prints one line naming the sender, kind, correlation id and tuple id, and the exact drain call to make, never the body; a miss prints nothing, so an idle mailbox costs the Monitor's harness nothing. It never calls `in`, so a lost or suppressed ping never loses a message: the row still sits in the mailbox for its full retention window whether or not anything was ever printed about it.
