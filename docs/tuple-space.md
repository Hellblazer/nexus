...
| `tuples.channel_delivery` (RDR-211 Phase 1 Step 3, bead nexus-rplay.13) | The `claude/channel` push-delivery waiter's own status for THIS session, read cross-process from a per-session on-disk record the waiter publishes at every wake/renew/release (`nx doctor` runs in the CLI process, the waiter in the session's `nx-mcp` process — no shared memory, no engine call). No record for the resolved session (no active session, or no `nx-mcp` has run the waiter here) is informational, never a warning. A record with `proof: "none"` (declared but never proven live — the channel is opt-in) is also informational, never a warning, and states the `released` count plainly if nonzero. `proof: "argv"` or `"probe"` with the waiter alive and a `last_wake` within 75s (3x the engine's 25s park cap) is OK, naming proof, wake age, `unacked` (0 or 1) and `released`. The waiter not alive, or a `last_wake` older than 75s, is a WARN naming the fix (`/mcp` to restart) — the drain hook still delivers at the next prompt either way |

## Push delivery (RDR-205, ping-then-pull)

The mailbox gets push delivery through a Claude Code `Monitor`, split into two pieces that are never merged: `nx tuple watch` pings, `conexus/hooks/scripts/mailbox_drain.py` delivers. See [CLI Reference: `nx tuple watch`](cli-reference.md#nx-tuple-watch) for the full flag reference and [Push delivery: ping then pull](tuple-space-walkthroughs.md#push-delivery-ping-then-pull) for the sequence.

**The watcher pings, never claims.** `nx tuple watch` is armed as a Monitor's `command` from a SessionStart instruction (`nexus.mailbox_arm`, emitted by `nx hook session-start` on every source, silent when the tuple surface is not answering). Arming needs a one-time `~/.claude/settings.json` permission-allowlist entry, since a Monitor's command runs under the same permission machinery as `Bash` (see the CLI reference); `nx doctor`'s `tuples.watch_permission` row reads the settings files Claude Code consults for permissions and names the entry to add when none of them cover it (bead nexus-rml7o; see the CLI reference for the full file list and deny-override handling). It probes `mailbox/<session-id>` and, when given `--instance NAME`, `mailbox/NAME` too, in one process sharing one emit budget: a second Monitor for the second address would double the ping rate against a throttle counted per Monitor. A hit prints one line naming the sender, kind, correlation id and tuple id, and the exact drain call to make (`tuple_in` then `tuple_ack`/`tuple_nack`, the same MCP tools the arm instruction and mailbox skill already use — nexus-dyfg8), never the body; a miss prints nothing, so an idle mailbox costs the Monitor's harness nothing. It never calls `in`, so a lost or suppressed ping never loses a message: the row still sits in the mailbox for its full retention window whether or not anything was ever printed about it.

**Why probe, not park.** `rd` with `timeout_s=0` short-circuits straight to the engine's one-shot query, before the code path that acquires a park slot ever runs. A parked `rd` wakes faster per call (a signalled wake beats a sleep interval), but holds one of the engine's sixteen global park slots (`DEFAULT_PARK_CAP_GLOBAL`, an engine constant with no config surface today) for as long as it waits. With on the order of fifteen peer sessions on one box each holding a watcher parked continuously, that budget is gone before the mailbox skill's own cross-instance `ack` park ever gets a slot. A probe on a sleep cadence holds no slot at all, at the cost of latency bounded by `--interval` (3 seconds by default) instead of a near-instant wake. Raising the global cap is an engine change gated behind its own release; probing sidesteps the question entirely.

**Never the end state.** The Monitor tool supports a `ws:` source that a server pushes to directly, with no poll and no park slot at all. The engine has no WebSocket endpoint to push through today, so this design cannot use it yet; a future RDR is the right place to add one. Until then, the probe loop above is the interim, not the destination.

**Across `/compact`, `/clear` and `/resume`.** A Monitor survives all three inside one Claude Code process: the harness task, its child process and event delivery all carry across; only the model's own context is replaced. `/compact` keeps the same session id, so a surviving watcher is still watching the right mailbox. `/clear`, and `/resume` into a different session, give the conversation a new session id while the old watcher keeps probing the OLD one. `nx hook session-start` writes a marker naming the current session id for its Claude process on every SessionStart (every source, so a marker left by a dead process that owned the same pid is overwritten), and the watcher's own per-cycle check against that marker is what stops it when the ids differ, never a model-issued `TaskStop`, which cannot work across a real `/clear`: the fresh conversation running the new SessionStart arm instruction has no memory of the old Monitor's harness task id to stop. The new arm instruction re-arms into whatever address the old watcher's exit just freed. See [`nx tuple watch`](cli-reference.md#nx-tuple-watch) for the lock and self-stop mechanics.

**The floor underneath it.** `mailbox_drain.py` runs on every `UserPromptSubmit`, claims, acks and renders whatever is waiting, whether or not a watcher was ever armed, ever ran, or ever printed a ping for a given row. Arming a Monitor is a request the model can decline or forget; this hook is not. A dead-lettered row is the one case neither half fully owns: it is unclaimable by construction, so the drain hook cannot consume it, but it still gets surfaced once, from each side's own local seen-file, because the alternative is a session that never learns the message existed at all.
...
**The directory lease.** `nx tuple watch --instance NAME` arms a `directory/<name>` entry alongside its existing per-session instance registration, written at arm time with `ttl_seconds=300` and re-sent every 60 seconds; a fresh nonce is minted before the entry's `created_at` plus the template's 7-day retention would otherwise stop a re-send from moving its expiry further (§ Templates and subspaces above). On the marker-mismatch self-stop (the watcher discovering a `/clear`, `/resume` or `/branch` changed this conversation's session id out from under it; `/branch` runs no SessionStart, so the fork's first prompt moves the marker through `nx hook mailbox-arm`), the watcher releases its CURRENT entry (the nonce in use at stop time, which after a rotation is not the arm-time nonce) by re-sending it with `ttl_seconds=1`, so it lapses within about a second rather than riding out a full TTL while the replacement watcher re-arms. A plain exit or `SIGTERM` releases nothing: the entry rides out its normal TTL, matching a `/resume` inside that window still finding the name resolving.

**The refusal rule.** `rd` on `directory/<name>` returns only live rows: the engine's own `expires_at > now()` filter excludes a lapsed entry automatically, so `mailbox_send` never has to distinguish "lapsed" from "never armed." Zero live rows is an error naming the name; more than one distinct `session_id` among the live rows is an error naming every holder, and `mailbox_send` writes nothing either way. Several live rows for the SAME session (a re-armed watcher's new nonce beside its old row, mid-rotation) are one holder, not a conflict. `nx tuple directory NAME` shows the same live rows from the CLI (each holder's `session_id`, `created_at`, `expires_at`) before a send, or to find the session id to resend to directly after a refusal.

**`/clear` and the drain.** On `source=clear`, SessionStart reads the previous session id from `tuple-watch/session.<claude pid>` before overwriting it with the new one, and writes `<config>/tuple-watch/cleared.<new session id>` naming the id a live mailbox watcher just lost. `conexus/hooks/scripts/mailbox_drain.py` reads its own session's `cleared.*` record on every `UserPromptSubmit`, inside the same budget as its own mailbox, and drains every mailbox the record names. It deletes the record only once every named mailbox's claim loop ended empty, a read paged to the end shows no row except dead-lettered ones, and that mailbox's pending file is empty; any other outcome (the budget running out, a refused ack, a row still under lease) keeps the record for the next prompt. A record older than 7 days is pruned regardless, matching the mailbox template's own retention. A second `/clear` before the first record's drain finishes carries every earlier id forward into the new record.

**Fork.** `/branch` and `--fork-session` mint a new session id while the parent session still exists and can be resumed, so the parent's mailbox stays with the parent: a fork writes no cleared record. The forked session starts with an empty mailbox of its own and becomes reachable by name once its own watcher arms a `directory/<name>` entry.

**The stopped-watcher trade-off (gate Significant a).** A watcher that stops (killed, crashed, or simply never re-armed) leaves its `directory/<name>` entry to lapse on its own, within at most 300 seconds (one TTL) of its last re-send. During that window `mailbox_send` still resolves the name; past it, `mailbox_send` refuses the name outright rather than silently misdelivering to a session nobody is draining for. The session itself is never unreachable during or after this window: mail sent directly to its session id always arrives, watcher or no watcher. Only NAME-based addressing depends on the lease being current.
