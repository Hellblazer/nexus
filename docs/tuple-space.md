...
- **Ten `tuple_*` MCP tools** — `tuple_out`, `tuple_rd`, `tuple_in`, `tuple_ack`, `tuple_nack`, `tuple_renew`, `tuple_release`, `tuple_registry`, `tuple_list`, `tuple_stats` (`rd`/`in`'s own `timeout_s=0` default covers the probe case; there are no separate `tuple_rdp`/`tuple_inp` tools). `tuple_ack` takes an optional `reply` object (RDR-206) with the same fields as `tuple_out` minus the nonce. `tuple_release` (RDR-211) is `tuple_nack`'s non-failure counterpart. See [MCP Servers — Tuple space](mcp-servers.md#tuple-space-t2-adjacent-rdr-205) for signatures and the routing rule of thumb.

`wait` and the park-slot report (RDR-211 Phase 1 Step 1) are internal `HttpTupleStore` methods only — `wait` for the session's own MCP server lifespan waiter, the park report for a later `nx doctor` row — with deliberately no MCP tool or CLI verb (Sam's decision, RDR-211 Open Question 6): nothing outside those two future consumers calls them directly.
