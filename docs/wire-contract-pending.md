...
- `d0291180f2cbb098140f9833c54c3d652dcef10c` -- bead nexus-mlu3k -- engine tag `engine-service-v0.1.89 (uncut; same pairing as the fgxmk lines below)` -- NEW read-only route `GET /v1/aspects/list_without_catalog_document` (engine half: AspectRepository.listWithoutCatalogDocument + AspectHandler) and its client `HttpDocumentAspectsStore.list_without_catalog_document` + `nx enrich aspects-without-catalog`. Direction safety: NEW client + OLD engine = the route 404s and the verb exits 2 naming the missing route (never an empty census); OLD client + NEW engine = an extra route nothing calls. Additive, no existing request shape changed.
