...
**The [RDR process](rdr-overview.md) is the foundational use case.** Every design decision starts as an RDR document, gets researched with findings stored in the knowledge base, is accepted and implemented in code, and eventually closed with a post-mortem. Without the link graph, these artifacts are isolated chunks scattered across `rdr__`, `code__`, `knowledge__`, and `docs__` collections. With it, an agent can start from an RDR, follow `implements` links to the code that realizes it, follow `cites` links to the research that informed it, and follow `supersedes` links to the decisions it replaced. The RDR lifecycle skills create these links automatically as they work — [rdr-close](rdr-workflow.md) links the post-mortem, [rdr-research](rdr-workflow.md) links findings, and the developer agent links implementations.

**Agents create links beyond RDRs too.** A research synthesizer creates `cites` links to source papers. Debugger and analyst agents create `relates` links between findings. The knowledge tidier creates `supersedes` links when consolidating duplicates. Agents can link at the chunk level when search results provide specific passages, but most links today are document-to-document.
