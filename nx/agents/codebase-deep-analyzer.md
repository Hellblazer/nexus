...
mcp__plugin_nx_nexus-catalog__catalog_link(from_tumbler="...", to_tumbler="...", link_type="relates", created_by="codebase-deep-analyzer", from_span="chash:...", to_span="chash:...")
```
...
- **Catalog Links** (if catalog tools available): After storing architecture maps or pattern catalogs:
  1. `mcp__plugin_nx_nexus-catalog__catalog_search(query="{scope} architecture", content_type="knowledge")` — find related prior analyses
  2. For related architecture maps on interconnected modules: `mcp__plugin_nx_nexus-catalog__catalog_link(from_tumbler="{this-map-title}", to_tumbler="{related-map-title}", link_type="relates", created_by="codebase-deep-analyzer")`
  3. When replacing a stale analysis: `mcp__plugin_nx_nexus-catalog__catalog_link(from_tumbler="{new-analysis-title}", to_tumbler="{old-analysis-title}", link_type="supersedes", created_by="codebase-deep-analyzer")`
