...
def main() -> None:
    ...
    output_lines.append("")
    output_lines.append("Search: `search` MCP tool — `where=\"KEY>=VALUE\"` for metadata filtering (results include `chunk_text_hash` metadata)")
    ...
    output_lines.append("Catalog: `catalog_search`/`catalog_links`/`catalog_link`/`catalog_link_audit` MCP tools — metadata-first routing; link creation with `chash:` spans (content-addressed, preferred)")
