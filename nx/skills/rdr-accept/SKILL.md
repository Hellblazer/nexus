...
7. **Planning handoff** (default: yes) — Auto-detect complexity:
   ...
   - **If yes — execute the full chain (3 sequential dispatches + catalog enrichment, orchestrated by this skill):**
     ...
     5. **Catalog links** (if catalog initialized): Search for related RDRs and create `relates` links:
        - `mcp__plugin_nx_nexus__catalog_search(query="<rdr-title-keywords>", content_type="rdr")`
        - For each result that is NOT this RDR: `mcp__plugin_nx_nexus__catalog_link(from_tumbler="<this-rdr-title>", to_tumbler="<related-rdr-tumbler>", link_type="relates", created_by="rdr-accept")`
