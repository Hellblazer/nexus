...
Gates on open bead status before closing. Optionally delegates post-mortem archival to the **nx_tidy** MCP tool. See [registry.yaml](../../registry.yaml).
...
Before calling nx_tidy, seed link-context so the post-mortem auto-links to the RDR:
```
...
Call `nx_tidy` MCP tool for post-mortem archival if the post-mortem contains substantial divergence analysis that benefits from knowledge organization.
...
When calling the nx_tidy MCP tool for post-mortem archival, use this exact structure:
...
Outputs generated by the nx_tidy MCP tool (post-mortem archival only):
