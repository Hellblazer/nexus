...
- **Run as a manual walkthrough from an assistant context.** This skill MUST be activated via the `/nx:rdr-close` slash command — not by an assistant editing the RDR frontmatter directly. The `rdr_close_gate_hook` (PreToolUse on `Edit|Write`, at `nx/hooks/scripts/rdr_close_gate_hook.sh`) structurally blocks any attempt to set `status: closed` on a `docs/rdr/rdr-*.md` file unless the `rdr-close-active-<id>` T1 scratch marker is present. The marker is written ONLY by this skill's command preamble (`nx/commands/rdr-close.md`) at successful Pass 2 of the Problem Statement Replay gate. If you are an AI assistant encountering a "real self-close via RDR-069 gate" task (e.g. RDR-066 Phase 5c, RDR-069 Phase 4c), STOP and hand the close off to the user with "ready for `/nx:rdr-close <id>`". Manual walkthroughs silently skip Step 1.5 (Two-Pass Problem Statement Replay), Step 1.75 (fresh critic dispatch), and Step 3 (user-confirmation bead gate) — exactly the failure mode RDR-066 and RDR-069 exist to prevent, applied to their own close flow. See `nexus_rdr/066-validation-5c-real-self-close` for the historical incident that motivated the hook.
