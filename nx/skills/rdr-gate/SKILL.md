...
- First, try catalog (structured metadata): `mcp__plugin_nx_nexus-catalog__catalog_search(query="relevant terms from problem statement", content_type="rdr")`
  - If results found, use `mcp__plugin_nx_nexus-catalog__catalog_links(tumbler="<result>", direction="both")` to discover related RDRs in the graph
