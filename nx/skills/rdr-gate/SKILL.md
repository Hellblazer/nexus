...
1. Write gate result to T2:
   ```bash
   nx memory put - --project {repo}_rdr --title {id}-gate-latest --ttl permanent --tags rdr,gate <<'EOF'
   outcome: "PASSED"
   date: "YYYY-MM-DD"
   critical_count: 0
   significant_count: N
...
- **T2 memory**: Gate result record via `nx memory put - --project {repo}_rdr --title {id}-gate-latest --ttl permanent --tags rdr,gate` (outcome: PASSED or BLOCKED)
