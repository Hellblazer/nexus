...
6. **Catalog links** (if catalog initialized): Search for this RDR in the catalog and display graph relationships:
   ```
   mcp__plugin_nx_nexus-catalog__catalog_search(query="<rdr-title>", content_type="rdr")
   ```
   If found, call: `mcp__plugin_nx_nexus-catalog__catalog_links(tumbler="<tumbler>", depth=1)`
