...
- **T3 knowledge**: Critique findings via `printf "# Critique: {artifact}\n{findings}\n" | nx store put - --collection knowledge --title "critique-{component}-{date}" --tags "critique,review"`
- **T2 memory**: Session findings via `nx memory put "content" --project {project}_active --title critique-{date}.md --ttl 30d`
- **T1 scratch**: Critique notes during analysis via `nx scratch put "Critique note: {finding}" --tags "critique,review"` (promoted to T2 on completion)
