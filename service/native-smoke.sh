...
#
# The two "real Python client" probes below (T1, then memory/plans/taxonomy/
# chash) live in service/smoke-probes/*.py, not inline here (nexus-cm5km) --
# tests/test_native_smoke_client_probes.py runs the IDENTICAL files against
# a self-provisioned engine substrate, so a regression in either probe is
# caught by the unit suite, not only by a native-image release build.
set -uo pipefail
cd "$(dirname "$0")"
...
#
# Deliberately a SEPARATE `uv run python <probe file>` subprocess from the T1 block
...
if command -v uv >/dev/null 2>&1 && [ -f "$REPO_ROOT/pyproject.toml" ]; then
  ...
  PY_OUT=$(cd "$REPO_ROOT" && NEXUS_CONFIG_DIR="$T2_PY_TMPDIR" NX_SERVICE_URL='' \
    ...
    $TIMEOUT_CMD uv run python "$REPO_ROOT/service/smoke-probes/t2_real_client.py" 2>&1)
  rm -rf "$T2_PY_TMPDIR"
  if echo "$PY_OUT" | grep -q "^OK$"; then
