 ...
 *   <li>{@code output_dtype: "float"} — matches Python default</li>
...
public final class CceEmbedder implements Embedder {
     ...
     *
     * <p>Like {@link #embed} but returns {@code double[]} without float32 truncation.
     * Used by the parity gate ({@code /v1/vectors/embed}) to avoid float32 round-trip
     * precision loss so that Java and Python produce cosine == 1.0 exactly.
     */
    ...
    private String buildJson(String text) {
        // inputs = [[text]] — exactly as Python: one document, one chunk
        ...
        // Note: NO "truncation" field — CCE API does not accept it
        try {
    ...
    /**
     * Parse CCE response body for a single-text call (inputs=[[text]]).
     *
    ...
    private float[] parseOneFloat(String body) throws Exception {
        ...
        // Sort outer by index (API may return out-of-order, though single-doc shouldn't)
        ...
        List<Number> rawEmb = (List<Number>) innerData.get(0).get("embedding");
        if (rawEmb == null) throw new RuntimeException("CCE response: chunk missing 'embedding'");
    ...
    /**
     * Parse CCE response body preserving full double precision (no float32 truncation).
     */
    @SuppressWarnings("unchecked")
    private double[] parseOneDouble(String body) throws Exception {
        Map<String, Object> root = mapper.readValue(body, Map.class);
        List<Map<String, Object>> outerData = (List<Map<String, Object>>) root.get("data");
        if (outerData == null || outerData.isEmpty()) {
            throw new RuntimeException("CCE response missing data array: " + body);
        ...
        for (int i = 0; i < rawEmb.size(); i++) vec[i] = rawEmb.get(i).doubleValue();  // no float32 truncation
        return vec;
    }
}
