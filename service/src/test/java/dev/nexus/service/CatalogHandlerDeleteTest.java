...
class CatalogHandlerDeleteTest {
    ...
    void startAll() throws Exception {
        ...
        // registered, out from under this suite's own delete assertions).
        ...
        // Seed two registry rows to delete (one per route-shape test). RDR-204
        // nexus-ft04v.4/.5: routed through PgContainerHelper.insertCollection.
        try (Connection su = pg.createConnection("")) {
            PgContainerHelper.insertCollection(DSL.using(su, SQLDialect.POSTGRES), TENANT, "hdel__by-name");
            PgContainerHelper.insertCollection(DSL.using(su, SQLDialect.POSTGRES), TENANT, "hdel__by-alias");
        }
    }
