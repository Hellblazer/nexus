...
The plugin handles this in three places, all keyed on the `cwd` field of the hook payload:
...
- **PreToolUse / PermissionRequest, cwd-is-a-worktree case**: every Serena write tool (`replace_in_files`, `replace_symbol_body`, `insert_*`, `rename_symbol`, `jet_brains_rename`/`move`/`safe_delete`/`inline_symbol`, `*_lines`, `replace_content`, memory writes) is denied with a reason naming the worktree. Read tools are approved as before.
- **PreToolUse / PermissionRequest, RELOCATED-session case (nexus-ebx0s)**: a session that reaches a worktree by absolute path without its own cwd ever moving there is, from cwd alone, indistinguishable from a session legitimately working in the primary — the case above cannot catch it, because its cwd never IS a linked worktree. `session_start.py` records the git working-tree root of each session's cwd at `SessionStart`'s `source == "startup"` only (per-session, in a small state file under `XDG_STATE_HOME` or `~/.claude/sn`); a later write call whose cwd resolves to a DIFFERENT working tree than that record is denied too, naming both trees. Fails open (allows, and logs to stderr) whenever no session_id, no record, or an unresolvable cwd makes the comparison itself impossible — the guard would rather miss a relocation than misfire on legitimate primary work with no way to check.
