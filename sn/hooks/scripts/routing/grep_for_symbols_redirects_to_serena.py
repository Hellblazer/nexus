...
def _redirect_message(pattern: str, files: list[str]) -> str:
    ...
    return (
        ...
        "  1. Serena (best for symbols):\n"
        f"     - Definition: mcp__plugin_sn_serena__jet_brains_find_symbol("
        f"name_path_pattern=\"{pattern}\")\n"
        f"     - Callers:    mcp__plugin_sn_serena__jet_brains_find_referencing_symbols("
        f"name_path=\"{pattern}\", relative_path=\"{file_arg}\")\n"
        f"     - File map:   mcp__plugin_sn_serena__jet_brains_get_symbols_overview("
