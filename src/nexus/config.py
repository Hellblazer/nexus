...
def _coerce_bool(
    value: Any, *, key: str, default: bool, event: str = "telemetry_config_malformed"
) -> bool:
    """Coerce ``value`` to bool, warn + fall back to ``default`` on malformed input.

    *event* names the structured-log event. It defaults to the telemetry
    name this helper shipped with (asserted by nothing, but documented in
    CHANGELOG) so the telemetry caller's log line is byte-identical; a
    second caller passes its own rather than emitting a warning that names
    the wrong subsystem.
    """
...
#: Env override for the per-commit reviewer's on/off switch (nexus-jh86x).
#: Sam ruled "env var + config key" on 2026-09-02: the env var is the
#: ad-hoc, per-shell silencer; ``.nexus.yml#commit_review.enabled`` is the
#: persistent one that survives a new terminal. Env WINS when both are set,
#: so silencing a noisy afternoon never requires editing a file.
COMMIT_REVIEW_ENV_VAR = "NX_COMMIT_REVIEW"

#: Sam's ruling, 2026-09-02: $0.25 per commit, hard refuse. ``claude -p``'s
#: own ``--max-budget-usd`` is a HARD abort at cap (spawning denied, exit 1,
#: no graceful completion), which is the behaviour this value was chosen
#: for -- a commit too large to review under the cap is SKIPPED and says so,
#: never reviewed against a silently truncated diff.
#:
#: Burst arithmetic, raised by the sibling session nexus-13 on 2026-09-02:
#: a release cut is not one commit, it is a release commit plus a back-merge
#: plus any fix-forward, landing in quick succession. This cap is PER
#: DISPATCH, so a five-commit burst has a $1.25 ceiling. The hook's pgrep
#: guard serialises the burst rather than firing five concurrent dispatches;
#: it does not reduce the total. If a rolling budget is ever wanted, that is
#: a separate decision with its own evidence, not a default to assume here.
COMMIT_REVIEW_DEFAULT_BUDGET_USD = 0.25

#: The reviewer's model. NOT an optimisation -- the feature does not work
#: without it, measured 2026-09-02.
#:
#: With no ``--model`` the child inherits the parent's, which on this box
#: is Opus. An isolated ``claude -p`` child was measured consuming 26,068
#: cache-creation input tokens BEFORE the model emitted a token: that is
#: the CLI's own floor (system prompt, tool list, schema), independent of
#: the diff, and at Opus cache-write rates it is roughly $0.49. So every
...
#: fit this call; ``tests/test_commit_review.py`` pins the two equal
...
class CommitReviewConfig:
    ...
    - ``timeout_seconds``: subprocess kill deadline. Well under
      ``claude_dispatch``'s 300s default because the input here is one
      commit diff, not an analytical workload.
    - ``max_diff_bytes``: diffs larger than this are truncated, and the
      truncation is stated in the prompt AND in the record.
    - ``ttl_days``: review records age out. A per-commit writer with a
      permanent TTL is an unbounded accumulator.
    """

    enabled: bool = True
    max_budget_usd: float = COMMIT_REVIEW_DEFAULT_BUDGET_USD
    timeout_seconds: float = 180.0
    model: str | None = COMMIT_REVIEW_DEFAULT_MODEL
    max_diff_bytes: int = 200_000
    ttl_days: int = 90


def _coerce_float(value: Any, *, key: str, default: float) -> float:
    """Coerce *value* to a positive float; warn + fall back on malformed input.

    Deliberately mirrors :func:`_coerce_bool`'s contract: a stray string in
    ``.nexus.yml`` must never silently disable the cap (which would leave a
    per-commit dispatch with no spend ceiling at all).
    """
    try:
        coerced = float(value)
    except (TypeError, ValueError):
        coerced = float("nan")
    if coerced > 0 and coerced == coerced and coerced != float("inf"):
        return coerced
    _log.warning(
        "commit_review_config_malformed",
        key=key,
        value=value,
        fell_back_to=default,
    )
...
def get_commit_review_config(
    repo_root: Path | None = None,
    *,
    cfg: dict | None = None,
) -> CommitReviewConfig:
    """Load the ``commit_review`` config section into a typed struct.

    Precedence for ``enabled``: :data:`COMMIT_REVIEW_ENV_VAR` (when set to
    anything recognisable) beats ``commit_review.enabled``, which beats the
    ``True`` default. The env var is read at CALL time, not import time, so
    a hook process picks up the environment it was actually spawned with.
    """
    section = (cfg if cfg is not None else load_config(repo_root=repo_root)).get(
        "commit_review", {}
    )
    if not isinstance(section, dict):
        _log.warning("commit_review_config_malformed", key="commit_review", value=section)
        section = {}

    enabled = _coerce_bool(
        section.get("enabled", True),
        key="commit_review.enabled",
        default=True,
        event="commit_review_config_malformed",
    )
    raw_env = os.environ.get(COMMIT_REVIEW_ENV_VAR, "").strip().lower()
    if raw_env in ("0", "false", "no", "off"):
        enabled = False
    elif raw_env in ("1", "true", "yes", "on"):
        enabled = True

    # Default APPLIED HERE, not left to the dataclass: this loader builds
    # the struct field by field, so a bare ``.get("model")`` would hand
    # back None and silently defeat COMMIT_REVIEW_DEFAULT_MODEL for every
    # caller that goes through config (which is all of them).
    model = section.get("model", COMMIT_REVIEW_DEFAULT_MODEL)
    return CommitReviewConfig(
        enabled=enabled,
        max_budget_usd=_coerce_float(
            section.get("max_budget_usd", COMMIT_REVIEW_DEFAULT_BUDGET_USD),
            key="commit_review.max_budget_usd",
            default=COMMIT_REVIEW_DEFAULT_BUDGET_USD,
        ),
        timeout_seconds=_coerce_float(
            section.get("timeout_seconds", 180.0),
            key="commit_review.timeout_seconds",
            default=180.0,
        ),
        model=model if isinstance(model, str) and model.strip() else None,
        max_diff_bytes=int(
            _coerce_float(
                section.get("max_diff_bytes", 200_000),
                key="commit_review.max_diff_bytes",
                default=200_000.0,
            )
        ),
        ttl_days=int(
            _coerce_float(
                section.get("ttl_days", 90), key="commit_review.ttl_days", default=90.0
            )
        ),
    )
