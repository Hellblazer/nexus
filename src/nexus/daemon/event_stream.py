...
async def handle_event_stream(
    ...
    try:
        ...
        while not stopping_fn():
            ...
            if await _client_closed(reader):
                return
...
async def _client_closed(reader: asyncio.StreamReader) -> bool:
    """Non-blocking check: return True if the client has closed the connection.

    Uses ``asyncio.wait_for`` with timeout=0 to attempt a peek without
    blocking the event loop.  An empty read (b"") indicates EOF; a timeout
    means nothing arrived yet (connection still open).
    """
    try:
        data = await asyncio.wait_for(reader.read(1), timeout=0)
        return data == b""  # EOF
    except asyncio.TimeoutError:
        return False  # no data — still open
    except (ConnectionResetError, OSError):
        return True  # transport gone
