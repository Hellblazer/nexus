...
SECTION_PATTERNS: dict[str, re.Pattern] = {
    ...
    "related_work":     re.compile(rf"^{_NUM}(related\s+work|background|prior\s+work|literature\s+review)", re.IGNORECASE),
    "methods":          re.compile(rf"^{_NUM}(methods?|materials?\s*(and|&)\s*methods?|methodology|approach|model|framework|architecture|system\s+design|algorithm)", re.IGNORECASE),
    "results":          re.compile(rf"^{_NUM}(results?|evaluation|experiments?|experimental\s+(results?|evaluation|study)|empirical\s+(results?|study|evaluation)|findings)(\s*(and|&)\s*discussion)?$", re.IGNORECASE),
    "discussion":       re.compile(rf"^{_NUM}discussion", re.IGNORECASE),
    "conclusion":       re.compile(rf"^{_NUM}(conclusions?|summary|future\s+work)", re.IGNORECASE),
