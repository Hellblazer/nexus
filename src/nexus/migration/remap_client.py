...
# ── RDR-155 P4b P0e rehome (decision D-D): rekey surface split out ─────────
# The REKEY surface (submit /v1/remap/rekey + poll) SURVIVES the combined
# 7.0.0 wave (consumer: the chash_rekey ladder rung) and now lives in
# ``nexus.db.t2.rekey_client`` — :class:`HttpRemapStore` below EXTENDS
# :class:`~nexus.db.t2.rekey_client.HttpRekeyClient` with the WIRE-REID
# LEDGER surface (record_batch/clear_leg/membership/count/pairs/entries/
# source_collections), which DIES with RDR-187 .11's 410 set — this whole
# file deletes at P2. The imports below are thin re-export SHIMS so dying
# consumers (substrate_etl, storage_cmd, the pre-repoint tests) stay
# untouched; surviving consumers import from ``nexus.db.t2.rekey_client``.
from nexus.db.t2.rekey_client import (  # noqa: F401 — re-export shims for dying consumers
    HTTP_CONFLICT,
    HTTP_GONE,
    HttpRekeyClient,
    RekeyJobFailedError,
...
class HttpRemapStore(HttpRekeyClient):
    ...
    P0e split (decision D-D): the surviving REKEY surface is inherited from
    :class:`~nexus.db.t2.rekey_client.HttpRekeyClient`; every method DEFINED
    on this class is the WIRE-REID LEDGER surface, which DIES WITH RDR-187
    .11 (the engine's 410 set) — this file deletes whole at P2.
    """
