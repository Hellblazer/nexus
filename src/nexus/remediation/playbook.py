...
def _chash_poison_forensics(store_state: StoreState) -> Playbook:
    ...
    return Playbook(
        ...
        steps=(
            ...
            "session)",
            "interpret: any non-zero count in the first two statements "
            "(nexus.chunks — the RDR-191 unified relation — and "
            "catalog_document_chunks) means width-non-conformant rows "
            "exist in that table — unhealed upgrade-ladder debt. v0.1.48+ "
            "engines tolerate these rows at boot "
