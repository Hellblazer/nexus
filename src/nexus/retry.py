...
class VectorUpsertTimeoutError(RuntimeError):
    """Raised for an upsert (write) call that timed out while
    ``retry_on_timeout=False`` (nexus-8hdg9 phase 1).

    ``http_vector_client``'s transport (``_request_once``) opens the
    connection AND reads the response under one socket-level timeout
    (``_keepalive_opener().open(req, timeout=timeout)``), so this fires for
    a genuine connect-phase stall exactly as it does for a read-phase one --
    urllib raises the same bare ``TimeoutError`` either way and this
    codebase has no cheaper way to tell them apart (splitting the phases
    would mean a second, upsert-specific transport, out of scope for this
    fix). For either phase, an upsert timeout is refused rather than
    retried: on the read-phase case specifically the request body may
    already be sent and the engine may still be embedding this exact batch
    server-side, so blindly re-POSTing it would stack a second embed pass
    on top of the first; refusing is also just simpler than a second
    connect-vs-read distinction the transport cannot make. This is
    surfaced loud rather than retried or dropped, and is CONTAINED per file
    by the indexer (``_contain_transient_upsert``, indexer.py) rather than
    aborting a whole ``nx index repo`` run.
    """


def _is_bare_timeout(exc: BaseException) -> bool:
    """True if *exc* is (or directly chains to, one level via
    ``__context__``/``__cause__``) a bare ``TimeoutError`` -- from EITHER
    the connect or the read phase; see :class:`VectorUpsertTimeoutError`'s
    docstring for why this codebase's transport cannot distinguish them.
    ...
    (or the connect attempt itself) was confirmably refused or reset --
    that family stays retryable regardless of ``retry_on_timeout``.
    """
...
def _vector_with_retry(
    ...
    *max_attempts* times (default 5).  Backoff starts at 2 s, doubles each
    ...
    ``retry_on_timeout`` (default True, unchanged behaviour for every
    ...
    is a different exception family and keeps retrying either way). The
    name is deliberately NOT ``retry_read_timeout``: ``http_vector_client``
    uses one socket timeout for connect AND read (see
    :class:`VectorUpsertTimeoutError`'s docstring), so this flag governs a
    bare ``TimeoutError`` from either phase, not specifically a read
    timeout -- naming it otherwise would claim a distinction the transport
    cannot make.
    ...
    while True:
        ...
        try:
            ...
            if not _is_retryable_vector_error(exc):
                raise
            if not retry_on_timeout and _is_bare_timeout(exc):
                _log.warning(
                    "vector_upsert_timeout_not_retried",
                    attempt=attempt,
                    error=str(exc)[:200],
                )
                raise VectorUpsertTimeoutError(
                    "Upsert timed out (connect or read -- this transport "
                    "cannot tell which); the request may already have been "
                    "sent and the engine may still be embedding this batch "
                    "server-side. Check GET /v1/status for in-flight "
