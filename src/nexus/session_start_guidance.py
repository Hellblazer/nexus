...
GUIDANCE_IMPERATIVE = (
    ...
    "before producing any other response.** This is a hard rule, not a "
    "hint or a preference — skipping a matching skill is a defect, not "
    "an optimization. False positives are cheap; misses cost real time.\n"
    "\n"
    "Route (invoke `Skill`) BEFORE doing the work yourself whenever any "
    "of these hold:\n"
    "- starting a turn, or about to implement with no design of record\n"
    "- something is broken: a failure, exception, or 2 failed fix "
    "attempts without routing\n"
    "- work spans multiple steps, or needs cross-module design before "
    "code\n"
    "- code, a plan, or tests are ready and need a quality gate\n"
    "- an answer must be reduced from many documents, not looked up as "
    "one fact\n"
    "- about to explore or research without first checking T1/T2/T3 for "
    "prior work\n"
    "- a finding is about to go unstored, or a phase/RDR boundary is "
    "being crossed\n"
    "\n"
    "The routing menu (which skill for which destination), Essential MCP "
    "Tools, the Common Mistakes table, and the Red Flags table live in "
    "this Skill's own body, not here — invoke `Skill` on `using-nx-skills` "
    "to read them. Skills evolve; don't rely on memory of a prior version.\n"
)
