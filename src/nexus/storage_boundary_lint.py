...
T2DATABASE_CONSTRUCTION_ALLOWLIST: dict[str, int] = {
    ...
    "src/nexus/commands/enrich.py": 10,    # read-only T2 access (+ routed writes); was 10 with the aspects-without-catalog census (nexus-mlu3k) + --sweep recovery-target probe (nexus-kkumv) — both RETIRED (hygiene-001, nexus-tk070.p6a follow-on): document_aspects.doc_id NOT NULL means an aspect row without a live document can no longer exist, so the census/sweep have nothing to count. 8 -> 9 at nexus-vnz3d (Sam decision 2026-09-20): aspects-backfill-uri, repairing rows the batch aspect builder wrote with an empty source_uri — the key the aspect_sql operators match byte-equal, so those rows matched nothing and the miss surfaced as "does not match" rather than as an error (481 of 2232 rows, all in knowledge__ collections). 9 -> 10 at nexus-3foc9: `nx enrich delete --all`'s paged listing of every aspect row in a collection, ahead of the bulk delete.
    "src/nexus/commands/index.py": 3,      # read-only probes (+1 nexus-iygza: the unassigned-chunk drain's listing, on the command's shared client); writes via t2_index_write
