...
def assign_topic(db: "T2Database", doc_id: str, topic_id: int) -> None:
    ...
    db.taxonomy.assign_topic(doc_id, topic_id)  # epsilon-allow: deprecated shim, no external callers; T2 write is db/-internal (its last caller, db/migrations.py, died in RDR-158 P4 Stage 4)
