...
# Session-marker contract (session_marker_path, write_session_marker,
# _read_session_marker, cleared_record_path,
# record_clear_and_write_session_marker) moved to :mod:`nexus.session_marker`
# (RDR-211 nexus-rplay.24), ahead of this module's own deletion. Re-exported
# here, thin, so the watcher's own self-stop check below and every existing
# caller/test that imports these names from `nexus.tuple_watch` keep working
# unchanged; `nexus.tuple_directory` and `nexus.hooks` import the new home
# directly. See that module's docstring for why it is a separate module
# rather than folded into `nexus.session` or `nexus.db.t1`.
