...
from nexus.daemon.t2_daemon import T2Daemon, write_frame, read_frame
...
async def test_backfill_correctness(
    ...
    events = list(client.event_stream("tuples/backfill", since_cursor=0))
    client.close()
...
async def test_live_streaming(
    ...
    try:
        ...
        arrived = await asyncio.get_event_loop().run_in_executor(
            None, lambda: ready.wait(timeout=1.0)
        )
        assert arrived, "live event did not arrive within 1 second"
        stop_flag.set()
        t.join(timeout=2.0)
...
async def test_cursor_restart_no_duplicates(
    ...
    client = _make_client(daemon)
    gen = client.event_stream("tuples/restart", since_cursor=0)
    ...
    # Reconnect with since_cursor = last seen
    client2 = _make_client(daemon)
    remaining = list(client2.event_stream("tuples/restart", since_cursor=last_cursor))
    client2.close()
...
async def test_failure_category_demux(
    ...
    client = _make_client(daemon)
    # Subscribe with category filter = 'timeout'
    events = list(
        client.event_stream(
            "tuples/demux",
            since_cursor=0,
...
async def test_connection_close_cleanup(
    ...
    for _ in range(10):
        client = _make_client(daemon)
        gen = client.event_stream("tuples/cleanup", since_cursor=0)
        # Read first event (backfilled) then close
        try:
            next(gen)
...
async def test_backfill_cap_1500(
    ...
    client = _make_client(daemon)
    events = list(client.event_stream("tuples/bigbatch", since_cursor=0))
    client.close()
