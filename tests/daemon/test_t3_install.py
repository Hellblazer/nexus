...
Shell-out to ``launchctl`` / ``systemctl`` is mocked; template
...
class TestTemplatesShipped:
    def test_t3_plist_template_present_with_placeholders(self) -> None:
        ...
        # Launchd template must pass --foreground so the supervisor sees a
        # long-running foreground process and can trigger KeepAlive.Crashed
        # on chroma death.
    ...
    def test_t3_service_template_present_with_placeholders(self) -> None:
        ...
        # Systemd unit must pass --foreground so Restart=on-failure /
        # SuccessExitStatus=143 actually fire.
...
class TestSymlinkGuard:
    """Install refuses to overwrite through a symlink (FS-7 TOCTOU mitigation)."""
