...
class TestQuarantineRegistrationKwargs:
    """RDR-204 Phase 3 fix round (nexus-ft04v.28 C1): the quarantine
    sibling's REGISTRATION kwargs must carry the ORIGIN's own
    content_type/owner_id/embedding_model -- never the synthetic
    "quarantine-<content_type>" string embedded in the sibling's NAME
    (that string is not one of the four canonical content types, and
    deriving embedding_model from it via effective_embedding_model_for_writes
    raised ValueError on every cloud-mode/local+voyage install, per the
    code-review-expert C1 finding on a6d0a389b..79cb39d84)."""
    ...
    def test_reads_content_type_owner_model_from_the_row(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from nexus.catalog.chunk_quarantine import quarantine_registration_kwargs

        assert quarantine_registration_kwargs("code__nexus-1-1__voyage-code-3__v1") == {
            "content_type": "code",
            "owner_id": "nexus-1-1",
    ...
    def test_content_type_is_never_the_quarantine_prefixed_string(
        self, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """The exact C1 regression: content_type must be the origin's
        REAL type ("code"), never "quarantine-code" -- the string
        quarantine_collection_name embeds in the sibling's NAME."""
        import nexus.mcp_infra as mi
        ...
        kwargs = quarantine_registration_kwargs("code__myrepo__voyage-code-3__v1")
        assert kwargs["content_type"] == "code"
        assert not kwargs["content_type"].startswith("quarantine")
    ...
    def test_preserves_origins_model_even_under_cloud_mode(
        self, cloud_mode: None, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """The sibling's model is the origin's PRESERVED model (its
        chunks move with their embeddings, no re-embed) -- never
        recomputed from the current write-intent. Under cloud_mode,
        effective_embedding_model_for_writes("code") would return
        "voyage-code-3"; the row says the origin was actually written
        under bge (a pre-migration collection), and that must survive
        unchanged."""
        import nexus.mcp_infra as mi
        ...
        monkeypatch.setattr(
            mi, "get_collection_row",
            lambda name: {
                "content_type": "code", "owner_id": "old-repo",
                "embedding_model": "bge-base-en-v15-768", "lifecycle_state": "live",
            },
        )
        from nexus.catalog.chunk_quarantine import quarantine_registration_kwargs

        kwargs = quarantine_registration_kwargs("code__old-repo__bge-base-en-v15-768__v1")
        assert kwargs["embedding_model"] == "bge-base-en-v15-768"

    def test_missing_row_falls_to_collection_registration_kwargs(
        self, monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """No row for *origin* -- the coordinator's own named exception
        for "chunk_quarantine's sibling minting IF IT MUST" (ruling
        2026-09-08): falls to the SAME mint-time seam every bare
        register_collection call site reuses, derived from origin's OWN
        ...
        from nexus.catalog.chunk_quarantine import quarantine_registration_kwargs
        from nexus.corpus import collection_registration_kwargs

        origin = "docs__proj__bge-base-en-v15-768__v1"
        assert quarantine_registration_kwargs(origin) == collection_registration_kwargs(origin)
