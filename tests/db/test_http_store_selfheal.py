...
CASES: list[SelfHealCase] = [
    ...
    SelfHealCase(
        ...
        do_write=lambda store, tag: store.save_plan(verb="query", query=f"selfheal query {tag}", plan_json="{}"),
        write_count_key="POST /v1/plans/save",
        assert_write_effect=_assert_plan_library_write,
        seed_for_read=lambda store: store.save_plan(verb="query", query="findme-plan", plan_json="{}"),
