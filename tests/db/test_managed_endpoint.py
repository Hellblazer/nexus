...
def _version_body(
    ...
    mode: str = "voyage",
    release_version: str | None = "0.1.39",
) -> dict:
...
def _trimmed_version_body(release_version: str = "0.1.39") -> dict:
...
def test_probe_below_release_floor_is_incompatible():
    ...
    assert "0.1.5" in str(exc.value) and "0.1.39" in str(exc.value)
...
def test_probe_below_release_floor_exception_carries_structured_versions():
    ...
    assert exc.value.required_version == "0.1.39"
...
def test_probe_snapshot_app_version_is_not_gated():
    ...
    def fake_get(url: str, timeout: float) -> httpx.Response:
        return _resp(200, _version_body(app_version="1.0-SNAPSHOT", release_version="0.1.39"))

    caps = probe_managed_service(base_url="https://x", http_get=fake_get)
    assert caps.app_version == "1.0-SNAPSHOT"
    assert caps.release_version == "0.1.39"
...
def test_probe_at_release_floor_passes():
    def fake_get(url: str, timeout: float) -> httpx.Response:
        return _resp(200, _version_body(release_version="0.1.39"))
...
def test_probe_trimmed_payload_passes_and_defaults_optional_fields():
    ...
    def fake_get(url: str, timeout: float) -> httpx.Response:
        return _resp(200, _trimmed_version_body(release_version="0.1.39"))
