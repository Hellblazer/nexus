...
export GIT_AUTHOR_NAME="nexus acquire"
export GIT_AUTHOR_EMAIL="acquire@nexus.local"
...
if printf '%s\n' "$PROBE_TEXT" | nx store put - --collection acquire-gate --title "acquire-gate-probe" 2>&1 | sed 's/^/       /'; then
