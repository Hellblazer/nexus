...
1. ``wait_for_t1_lease`` -- the polling primitive -- against REAL files via
   ``nexus.db.t1.publish_t1_session_lease``/``read_t1_session_lease`` (no
   fakes for the signal itself: it is pure filesystem, so there is no
   reason to mock it). Covers: ready immediately, ready after N polls,
   never ready (times out), and a lease published under a DIFFERENT
   session id is never mistaken for readiness.
...
from nexus.hooks.mcp_connect_wait import run, wait_for_t1_lease

# -- wait_for_t1_lease: the polling primitive, against real lease files -----


class TestWaitForT1Lease:
    def test_ready_immediately(self, tmp_path: Path) -> None:
        publish_t1_session_lease("sess-A", "tok-A", tmp_path, ttl_seconds=3600)
    ...
    def test_ready_after_n_polls(self, tmp_path: Path) -> None:
        ...
        # real lease file after the third poll -- so the loop's own
        # read_lease call (the real, unmocked function) is what discovers it.
        calls = {"n": 0}
        clock = {"t": 0.0}
        ...
        def fake_sleep(interval: float) -> None:
            ...
            clock["t"] += interval
            if calls["n"] == 3:
                publish_t1_session_lease("sess-B", "tok-B", tmp_path, ttl_seconds=3600)
    ...
    def test_wrong_session_id_is_never_read_as_ready(self, tmp_path: Path) -> None:
        """A lease published for a DIFFERENT session id must not satisfy the wait.

        Real ``read_t1_session_lease`` keys the lookup on the exact session
        id (the lease file path names it), so this is really a proof that
        the wiring passes the right id through -- a wrong id here would
        silently pass the barrier for a session that never connected.
        """
        publish_t1_session_lease("sess-OTHER", "tok-OTHER", tmp_path, ttl_seconds=3600)
        ready, elapsed = wait_for_t1_lease(
    ...
    def test_expired_lease_reads_as_not_ready(self, tmp_path: Path) -> None:
        """A lease past its own expiry is ABSENT per read_t1_session_lease's
        ...
        publish_t1_session_lease("sess-D", "tok-D", tmp_path, ttl_seconds=-10.0)
        ready, elapsed = wait_for_t1_lease(
...
class TestRunMatcherSourceGate:
    def test_non_startup_source_is_a_fast_noop(self, tmp_path, monkeypatch) -> None:
        ...
        for source in ("resume", "clear", "compact", "fork", "", None):
            result = run({"session_id": "sess-E", "source": source})
            assert result.stdout is None
            assert result.exit_code == 0
        # Nothing was ever published, and the fast no-op path never reads
        # nexus.config or nexus.db.t1 -- the config dir stays empty.
...
class TestRunWaitsOnStartup:
    def test_lease_already_published_returns_immediately(self, tmp_path, monkeypatch) -> None:
        monkeypatch.setenv("NEXUS_CONFIG_DIR", str(tmp_path))
        ...
        publish_t1_session_lease("sess-F", "tok-F", tmp_path, ttl_seconds=3600)
        result = run({"session_id": "sess-F", "source": "startup"})
        assert result.stdout is None
        assert result.exit_code == 0
    ...
    def test_bound_and_poll_overrides_are_actually_honoured(
        ...
        run({"session_id": "sess-H", "source": "startup"})
