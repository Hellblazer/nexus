...
class FakeT2Client:
    ...
    def put(self, project: str, title: str, content: str, tags: str = "", ttl: int | None = None) -> int:
        self.put_calls.append((project, title, content))
...
def test_t2_leg_apply_writes_mapped_status(mod, rdr_tree: Path) -> None:
    ...
    _, title, new_content = client.put_calls[0]
    assert title == "007"
    assert "status: abandoned" in new_content


def test_t2_leg_diff_with_no_existing_t2_status_actually_writes_one(mod, rdr_tree: Path) -> None:
    """A reported ``None -> <status>`` diff must not be a lie: applying it
    must actually produce the status line, not silently no-op."""
    file_results = mod.compute_file_results(rdr_tree, apply=False)
    client = FakeT2Client({
        "001": {"content": "title: x\nno status line here\n", "tags": "rdr"},
    })
    report = mod.run_t2_leg(client, "nexus_rdr", file_results, apply=True)
    assert report.diffs[0].old_status is None
    assert report.diffs[0].new_status == "draft"
    _, _, new_content = client.put_calls[0]
    assert mod.extract_status_from_content(new_content) == "draft"
