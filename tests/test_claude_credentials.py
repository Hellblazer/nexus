...
"""Unit coverage for the Claude Code automation-token exec helper
(``tests/e2e/lib/claude_credentials.py``, nexus-galkv.19; RDR-219 P1.1).
...
These tests exercise `run`/`status` against a fake `security` (and, for
--remote, a fake `ssh`) — no real Keychain access, no real subprocess to
the OS. The module's old `pick`/`check FILE` modes (and the picker logic
that backed them) were removed at nexus-wauo1.18 once every caller had
migrated to `run`.
"""
...
def test_main_rejects_pick(cc, capsys) -> None:
    """RDR-219 P1.2 (nexus-wauo1.18): `pick` was removed once every caller
    migrated to `run` -- the mode must now be rejected as unrecognized,
    never dispatched."""
    assert not hasattr(cc, "_cmd_pick")
    rc = cc.main(["claude_credentials.py", "pick"])
    assert rc == 2
    assert "unknown mode" in capsys.readouterr().err.lower()


def test_main_rejects_check(cc, tmp_path, capsys) -> None:
    """Same removal for `check FILE`."""
    assert not hasattr(cc, "_cmd_check")
    target = tmp_path / "creds.json"
    target.write_text("{}")
    rc = cc.main(["claude_credentials.py", "check", str(target)])
    assert rc == 2
    assert "unknown mode" in capsys.readouterr().err.lower()
