...
class TestCheckChashConformanceReport:
    ...
    def test_fully_routable_collections_still_render_plain_clean(self, monkeypatch) -> None:
        ...
        Neutral model token on purpose (RDR-109 mode lint): routability is
        keyed on dim_for_model_token's fixed 5-entry table, not on mode --
        minilm-l6-v2-384 is as routable as any voyage token and carries no
        cloud-mode claim."""
        cat = self._cat()
        t3 = self._t3(collection_names=[
            "knowledge__x__bge-base-en-v15-768__v1",
            "code__y__minilm-l6-v2-384__v1",
        ])
        r = self._run(monkeypatch, cat, t3=t3)
        assert r.ok is True
        assert "clean" in r.detail
...
class TestCheckTopicsDocCountDrift:
    """Topics whose cached doc_count disagrees with the real topic_assignments
    ...
    def _run(self, monkeypatch, store) -> "object":
        ...
        return h._check_topics_doc_count_drift()[0]
    ...
    def test_drifted_topic_is_named(self, monkeypatch) -> None:
        ...
        assert r.ok is False and r.warn is True
        assert "500" in r.detail and "over-counted" in r.detail
        assert "doc_count=5" in r.detail and "actual=2" in r.detail
    ...
    def test_connect_failure_skips(self, monkeypatch) -> None:
        ...
        r = h._check_topics_doc_count_drift()[0]
        assert r.ok is True
    ...
    def test_route_missing_404_reads_as_unread_not_clean(self, monkeypatch) -> None:
