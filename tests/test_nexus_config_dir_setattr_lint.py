...
# total by 1.
_TOTAL_BY_VALUE_IMPORT_CEILING = 89  # -1: load_service_credentials_into_env deleted (d667cebdd, no callers)  # +1: nexus.hooks.mcp_connect_check (nexus-veh77), function-scoped deferred  # +3: nexus.hooks.mcp_connect_wait and two mcp/core.py _t1_lifespan connect-marker sites (nexus-veh77), each function-scoped deferred  # +1: nexus.hooks._routing_lib._default_config_dir (nexus-t9klx), function-scoped deferred -- the routing guards mirrored this resolution in a plugin-side stdlib copy until they moved into the wheel  # +1: mcp/core.py's tuple_subscribe helper (nexus-rplay.11), function-scoped deferred  # +1: _session_end_launcher._sweep_local_garbage (nexus-fjwk7), deferred  # -1: _session_end_census.capability_census_log_path deleted at nexus-gjv9b PART 3; +1: upgrade_finish.py aspect-worker respawn (nexus-06fu4/restart-stale fix); the 3o4lt bump to 79 was reverted by switching doc_indexer to a module import  # +2: health.py's two new RDR-205 tuple doctor rows (nexus-em75s.10), each a function-scoped deferred `from nexus.config import nexus_config_dir`  # +2: RDR-211 Phase 1 Step 3 channel-delivery doctor row (nexus-rplay.13) -- health.py's `_check_tuple_channel_delivery` and mcp/core.py's `_start_channel_waiter`, each a function-scoped deferred `from nexus.config import nexus_config_dir`  # +1: installer.py's `_render_for_service` (nexus-cd1k0.3 review round 3 finding 4b), function-scoped deferred -- resolves the config dir a generated unit's `--config-dir` argv carries  # +1: nexus.hooks.tuple_ledger_project.project (bead nexus-b5ugt, the in-process RDR-205 ledger tuple projector port), function-scoped deferred  # +1: beads_prime.manage_status (nexus-i4odo follow-up, T2 review-wave2-daemon-2026-09-23), function-scoped deferred -- resolves config.yml's path so the "could not read <path>" message names it
