...
The arm's candidate set is every zero-count NOTE-SHAPED row (no file_path,
a recorded chash -- the predicate ``--store-put-integrity`` scans by) from
any collection, narrowed PER ROW at execution time by two facts it
re-proves itself: the manifest is still empty, and T3 has no chunk under
the note's chash OR its title
...
RDR_COL = "rdr__1-1__voyage-context-3__v1"


class _Entry:
    def __init__(
        self, tumbler, title, *, chash="", chunk_count=0, file_path="", source_uri="",
        collection=COL,
    ):
...
def world():
    ...
    entries = [
        ...
        _Entry("1.11.2", "gone-no-chash"),                               # NOT note-shaped (no chash): left alone
        ...
        _Entry("1.11.7", "file-backed", file_path="/p.md"),             # not note-shaped
        # A store_put note registered into an rdr__ collection: not the
        # RDR-145 shape class, still note-shaped, still a ghost. The first
        # cut of the arm left exactly two of these behind on production.
        _Entry("1.11.8", "gone-in-rdr-collection", chash="f" * 32, collection=RDR_COL),
    ]
...
def test_confirmed_tombstones_exactly_the_proven_ghosts(world, monkeypatch) -> None:
    ...
    assert result.exit_code == 0, result.output
    assert {str(t) for t in writer.deleted} == {"1.11.1", "1.11.8"}, writer.deleted
...
def test_every_candidate_is_probed_in_t3_before_the_write(world, monkeypatch) -> None:
    ...
    assert {"gone-by-both-keys", "kept-title-chunks", "probe-fails", "gone-in-rdr-collection"} <= probed_titles
    ...
    # A row with no chash is not note-shaped and is never a candidate.
    assert "gone-no-chash" not in probed_titles
