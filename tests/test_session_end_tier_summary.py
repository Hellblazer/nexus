...
"""Phase 1C: SessionEnd launcher tier-write summary (nexus-a52i / nexus-ov13k).

The summary prints AFTER the fork dispatch, from the parent — a network
read must never sit ahead of the cleanup dispatch (review Critical: the
retrying transport has a 20-50s worst case), so the printer uses a
pinned-endpoint, single-attempt read. This test suite locks in:
...
- Failures (service unreachable, older engine) are swallowed — telemetry
  must never break session close.
- main() orders fork dispatch → service summary.

The pre-fork LOCAL-mode printer (fast sqlite read of the local
tier_writes) died with the =sqlite opt-out (RDR-158 P3, nexus-7bomn);
...
class TestServiceTierSummary:
    ...
    def test_service_summary_silent_when_no_session_resolvable(
        self,
        isolated_t2: Path,
        capsys: pytest.CaptureFixture,
    ) -> None:
    ...
    def test_main_prints_service_summary_after_fork_dispatch(
        ...
        # happen BEFORE the service summary attempt. (The pre-fork local
        # summary that used to lead this ordering died with the =sqlite
        # opt-out.)
