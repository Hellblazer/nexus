...
class TestStoreGetManyCategories:
    """nexus-4jj40 (RDR-200 Phase 1c evidence hygiene): ``category``
    metadata is already fetched into ``entry`` on the same batch call as
    ``content`` -- surfacing it costs no extra round trip and lets the
    plan runner's rank/candidate hydration tell a genuine paper hit from
    a co-mingled code-fixture hit (see ``nexus.plans.runner.
    _filter_hydrated_contents``).
    """

    def test_categories_aligned_with_contents(self):
        from nexus.mcp.core import store_get_many

        ids = ["paper-1", "code-1"]
        store = {
            "paper-1": {"content": "a real paper passage", "category": "paper"},
            "code-1": {"content": "operator test fixture JSON", "category": "code"},
        }
        mock_t3, _ = _make_stub_t3({"*": store})
        ...
        assert result["categories"] == ["paper", "code"]

    def test_missing_id_gets_empty_category(self):
        from nexus.mcp.core import store_get_many

        ids = ["exists-1", "missing-1"]
        store = {"exists-1": {"content": "a", "category": "prose"}}
        mock_t3, _ = _make_stub_t3({"*": store})
    ...
    def test_no_category_metadata_yields_empty_string(self):
        from nexus.mcp.core import store_get_many

        ids = ["doc-1"]
        store = {"doc-1": {"content": "no category stamped"}}
        mock_t3, _ = _make_stub_t3({"*": store})
