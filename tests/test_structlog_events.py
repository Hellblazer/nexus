...
# ── clustering_skipped_partial_failure (R4-2) ───────────────────────────────


def test_clustering_skipped_partial_failure_event(monkeypatch):
    """R4-2: clustering fall-through on partial failure emits observable event."""
    ...
    class _PartialT3:
        ...
        def search(self, query, collection_names, n_results=10, where=None):
            ...
            return [{"id": "c", "content": "t", "distance": 0.1}]
    ...
    events = [e for e in logs if e.get("event") == "clustering_skipped_partial_failure"]
    assert len(events) == 1
