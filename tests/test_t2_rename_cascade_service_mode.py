...
# ---------------------------------------------------------------------------
# Per-store isolation classes DELETED (nexus-i711w Stage 2 sub-stage A).
#
# TestCascadeChash/DocumentAspects/AspectQueue/DocumentHighlights/Telemetry
# ServiceMode each put ONE store on the service backend and let every other
# leg run its SQLite arm. That premise is unreachable now: the SQLite
# telemetry/chash/queue/highlights stores are deleted, their cascade legs
# route through the Http twins UNCONDITIONALLY, and the per-store
