...
else in the audit surface re-derives the pick and compares. This probe
samples chunks that carry a stored projection assignment, re-fetches
their stored vector and every live foreign centroid, recomputes the exact
nearest centroid (the same tie-break as the engine's own
``ORDER BY distance, topic_id``), and reports a disagreement.
...
def _plant_wrong_projection(tax, ids: list[str], topic_a: int, topic_b: int) -> tuple[str, int, int]:
    ...
    real cosine of exactly 1.0 -- nothing can out-similarity that). The
    real row's similarity for a non-centroid chunk is provably < 1.0, so
    a planted similarity of 0.999999 reliably wins the engine's
    GREATEST(similarity)-wins conflict resolution as the CANONICAL
    projection pick for this chunk without needing to read the real
    value first. Returns ``(victim, real_topic, wrong_topic)``.
    """
    victim = ids[2]
    real_topic = tax.get_assignments_for_docs([victim])[victim]
    wrong_topic = topic_b if real_topic == topic_a else topic_a
    tax.assign_topic(victim, wrong_topic, "projection", similarity=0.999999, source_collection=_SRC)
...
def test_report_flags_a_disagreement_with_its_gap() -> None:
    ...
    r.disagreements.append(Disagreement(
        doc_id="a" * 64, stored_topic_id=1, stored_similarity=0.5,
...
def test_an_empty_probe_is_named_not_fatal_alone() -> None:
    """An 'empty' probe result (the sampled candidate ids carried no
    projection row for THIS collection) is reported, not silently
    dropped, and does not by itself flip an otherwise-clean run to
    failing."""
    empty = CollectionAssignmentDrift(collection="quiet", size=2, projection_count=2, compared=0)
    clean = CollectionAssignmentDrift(collection="busy", size=2, projection_count=2, compared=2)
    lines, ok = format_report([empty, clean], sample=20, seed=1)
    assert ok
    assert any("NOT CHECKED" in line and "quiet" in line for line in lines)
...
def test_probe_collection_flags_a_planted_disagreement_with_fake_stores() -> None:
    ...
    projection topic is provably NOT the nearest one."""
...
def test_a_planted_wrong_projection_is_flagged(t2_service_env) -> None:
    ...
    # topic_assignments' conflict key is (tenant, doc_id, topic_id) -- this
    # INSERTS a second row rather than overwriting the real one; the audit
    # reads the highest-similarity 'projection' row per doc_id as the
    # stored pick, the same rule the engine itself applies on a re-assign
    # conflict.
